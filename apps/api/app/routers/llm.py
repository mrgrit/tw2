...
def _build_system_prompt(user: User, ctx: AskContext) -> str:
    ...
    parts = [
        ...
        "**기본은 핵심만 한 줄(한 문장)로 답한다.** 학생이 '상세히/자세히/길게/풀어서/단계별/예시/설명해줘' "
        "등으로 더 자세한 답을 명시적으로 요청한 경우에만 여러 줄로 길게(단계·예시 포함) 답한다.",
        "한국어로 정확하게 답하고, 보안 실습 맥락(공격/방어 기법, 로그·룰·페이로드 해석)을 이해해 설명한다.",
