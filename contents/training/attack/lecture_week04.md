...
공격은 내 공격 VM `ssh att@192.168.0.202`(비번 1, 자연 URL) 또는 방어 로그 확인은 WAF 장비
`ssh ccc@10.20.32.80`(비번 1)로 실행한다. **인가된
실습 환경에서만 수행한다.**
...
> **결과 해석** `command -v sqlmap` 가 경로를 출력 + whatweb 이 도달(코드 응답)하면 정상.
