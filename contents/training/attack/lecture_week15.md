...
5. el34 가 **SNAT 를 하지 않아 출처 IP(10.20.30.202)가 전 계층에 보존**됨을 이용해, 정찰·침투·
...
캡스톤도 시험이지만, 본 강의 시간은 시험 직전의 **14주 통합 복습 + PTES 판단 프레임워크 정립 +
실기 평가**로 구성한다.

| 시간        | 내용                                                                       | 유형     |
|-------------|----------------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — 왜 단일 기법이 아니라 PTES 완주인가 (캡스톤 채점 관점)                | 강의     |
| 0:20–0:55   | 이론 — PTES 7단계 + W01–W14 기법을 각 단계에 배치(한 침투의 지도)            | 강의     |
| 0:55–1:05   | 휴식                                                                        | —        |
| 1:05–1:35   | 이론 — el34 진입 경로(fw nftables + web Apache) + 출처 IP 보존 + 킬체인 재구성 | 강의/토론 |
| 1:35–2:05   | 실기 — 점검(전 도구) + ② 정찰(nmap+ffuf) + ④ 취약점 분석(SQLi/강제브라우징) (lab 1–3) | 시험 |
| 2:05–2:40   | 실기 — ⑤ 익스플로잇(셸 51515) + ⑥ 권한 상승 열거(SUID/sudo) (lab 4–5)        | 시험     |
| 2:40–2:50   | 휴식                                                                        | —        |
| 2:50–3:35   | 실기 — ⑥ persistence(pt15ghost)+유출 staging + 방어 킬체인 재구성 (lab 6–7)  | 시험     |
| 3:35–4:20   | 실기 — ⑦ PTES 보고서 + 전 흔적 self-clean 확인 (lab 8–9)                     | 시험     |
| 4:20–4:40   | 정리 + 수료 안내 + 침투 테스터로서의 다음 단계                              | 정리     |
...
```mermaid
graph TD
    ATK["외부 침입자(레드팀)<br/>출처 10.20.30.202"]
...
이번 시험의 시나리오는 한 외부 침입자(`el34-attacker`, 출처 IP `10.20.30.202`)가 fw 의
게이트웨이(`10.20.30.1`)를 통해 el34 의 웹 자산(`juice.el34.lab` / `dvwa.el34.lab`)을 노려
PTES 7단계를 완주하는 것이다. el34 는 SNAT 를 하지 않으므로 출처 IP `10.20.30.202` 가 모든
단계·모든 계층에 그대로 보존된다(§4).

> ⚠️ **인가된 실습만.** 이 트랙의 모든 공격은 **인가된 실습 환경(el34)** 안에서, 정해진 대상
> (`el34-attacker` → el34 내부 vhost / `el34-web` 호스트)에 한해서만 수행한다. RoE(범위·시간·
...
(`nmap`/`ffuf`/`sqlmap`)가 `el34-attacker` 에 준비됐는지, (2) 대상 vhost 가 응답하는지. 도구가
...
게이트웨이(`10.20.30.1`)의 80/443 등을 스캔하고 vhost 별로 경로를 퍼징해 "어디로 들어갈 수
...
```mermaid
graph TD
    P1["① 공격 경로 개요<br/>출처 10.20.30.202, PTES 완주"]
...
> 정반대다. **DNAT** 는 **목적지**를 바꾼다 — el34 의 fw 가 공개 주소 `10.20.30.1` 로 온 요청을
...
```mermaid
graph TD
    ATK["공격자 진짜 출처<br/>10.20.30.202"]
    FW["fw: DNAT 만 함<br/>SNAT 안 함<br/>→ 출발지 그대로 통과"]
    A["WAF modsec_audit<br/>remote_address = 10.20.30.202<br/>(정찰 913 / 침투 942)"]
...
fw 가 SNAT 를 하지 않으므로, 안쪽의 모든 장비가 공격자의 **진짜 출처 IP `10.20.30.202`** 를
...
```bash
# 포트 스캔
docker exec el34-attacker nmap -p 80,443,8001 10.20.30.1 -T4 --max-retries 1 2>/dev/null | grep -E "open|PORT"
# 경로 퍼징 (juice vhost)
docker exec el34-attacker sh -c 'printf "admin\nlogin\nftp\n" > /tmp/pt15.txt; ffuf -u http://10.20.30.1/FUZZ -w /tmp/pt15.txt -H "Host: juice.el34.lab" -mc 200,301,403 -s 2>&1 | head -5; rm -f /tmp/pt15.txt'
```

무엇을 보나 — 어떤 포트가 `open` 이고 어떤 경로가 응답하는가. 열린 포트·경로가 다음 단계의
익스플로잇 후보다. 임시 단어 목록(`/tmp/pt15.txt`)은 쓰고 바로 지운다.

### 5.2 ④ 취약점 분석 — el34-attacker / curl SQLi + 강제 브라우징 — W04·W06

SQLi 는 입력에 SQL 문법을 주입(W04), 강제 브라우징은 경로에 직접 접근(W06)하는 기법이다.

```bash
# SQLi (dvwa=차단 모드 → 403 이 정상)
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'sqli=%{http_code} ' -A sqlmap/1.7 -H 'Host: dvwa.el34.lab' \"http://10.20.30.1/?id=pt15%27%20UNION%20SELECT%201--\""
...
```bash
docker exec el34-web sh -c 'nohup python3 -m http.server 51515 >/dev/null 2>&1 & echo shell'
sleep 1
docker exec el34-web osqueryi --json 'SELECT pid,port FROM listening_ports WHERE port=51515;'
docker exec el34-web sh -c 'pkill -f "[h]ttp.server 51515"; true'
```
...
```bash
docker exec -u ccc el34-web sh -c 'find / -perm -4000 -type f 2>/dev/null | head -5; id'
```
...
```bash
docker exec el34-web sh -c 'userdel -r pt15ghost 2>/dev/null; useradd -m -s /bin/bash pt15ghost; tar czf /tmp/pt15_exfil.tar.gz /etc/passwd 2>/dev/null; echo "persist+exfil"'
docker exec el34-web osqueryi --json 'SELECT username FROM users WHERE username="pt15ghost";'
```
...
docker exec el34-web sh -c 'sudo tail -120 /var/log/apache2/modsec_audit.log | grep -oE "9[0-9]{5}" | sort -u | head'
# 호스트 발판: 백도어 계정
docker exec el34-web osqueryi --json 'SELECT username FROM users WHERE username="pt15ghost";'
```

무엇을 보나 — WAF 흔적에 913(정찰)·942(SQLi)가 보이고, osquery 에 발판 흔적(pt15ghost)이
보이는지. 이 흔적들을 출처 IP `10.20.30.202` 로 묶으면 정찰→침투→발판→유출의 한 킬체인이 된다.
...
>
> **무엇을 알 수 있는가?** `el34-attacker` 에 정찰(nmap/ffuf)·익스플로잇(sqlmap) 도구가 모두
...
>
> **무엇을 알 수 있는가?** nmap 으로 fw 게이트웨이(10.20.30.1)의 열린 포트를, ffuf 로 vhost 의
...
> 장비는 이 호스트 내부 발판을 못 본다**. 계정은 다음 헌팅 미션을 위해 남긴다.
>
...
> `10.20.30.202` 하나로 한 침투의 한 킬체인임을 입증.
>
...
- **인가된 실습만.** 모든 공격은 인가된 실습 환경(el34) 안에서, 정해진 대상(`el34-attacker` →
