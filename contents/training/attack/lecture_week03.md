...
```bash
curl -s http://juice.el34.lab/rest/products/search?q=test | head -c 200
```
...
curl -s -H 'Content-Type: application/json' -d @/tmp/sqli.json http://juice.el34.lab/rest/user/login | head -c 150
rm -f /tmp/sqli.json
```
...
> 외부 공격자 VM `192.168.0.202`(공격) / `ssh ccc@10.20.32.80`(방어 로그 확인)로 실행한다. **인가된 실습
