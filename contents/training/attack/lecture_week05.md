...
```bash
# 진짜 XSS 스캐너 dalfox로 Reflected XSS 발사 (dvwa = 차단 모드 → WAF 403 지문 예상)
dalfox url --url 'http://dvwa.el34.lab/?q=FUZZ' 2>&1 | sed -r 's/\x1b\[[0-9;]*m//g' | grep -iE 'WAF|reflected|XSS found'
```

`dalfox` 는 `<script>alert(1)</script>` 을 포함한 수백 개 XSS payload 를 자동으로 던진다. 자연 URL
`http://dvwa.el34.lab/`의 호스트명이 web Apache의 vhost 라우팅에서 어느 가상호스트로 보낼지 지정한다
(att `/etc/hosts` 등록). 출력에 `WAF ... (HTTP 403) detected` 가 뜨면 ModSec이 차단한 것이다(`XSS found 0` = 막혀서 성립 못 함).
...
```bash
# 이벤트 핸들러 변형(img onerror·svg onload)만 dalfox custom-payload로 발사 (<script> 없이도 성립)
printf '<img src=x onerror=alert(1)>\n<svg onload=alert(1)>\n' > /tmp/eh.txt
dalfox url --url 'http://dvwa.el34.lab/?q=FUZZ' --custom-payload /tmp/eh.txt --only-custom-payload 2>&1 | sed -r 's/\x1b\[[0-9;]*m//g' | grep -iE 'WAF|reflected|XSS found'
rm -f /tmp/eh.txt
```

> **el34 실측 — payload 인코딩은 dalfox가 알아서 처리한다.**
> 수작업 raw 요청에서는 `<img src=x onerror=alert(1)>` 의 **공백**이 요청 줄을 깨뜨려 `%20`으로
> 인코딩해야 했지만, 진짜 XSS 스캐너 dalfox 는 `--custom-payload` 로 넣은 이벤트 핸들러 변형의
> 인코딩·전송을 자동으로 처리한다. web Apache는 디코딩해 원형으로 받고, ModSec의 **941 룰군은
> `onerror=`·`onload=` 같은 이벤트 핸들러 패턴(941160 NoScript)까지 탐지**해 `403`으로 차단하므로
> dalfox 는 `WAF ... (HTTP 403) detected` + `XSS found 0` 로 보고한다 — WAF는 정규화 후 검사하기 때문이다(§4).
...
```bash
# dalfox가 대소문자(<ScRiPt>)·URL 인코딩(%3Cscript%3E) 등 수백 개 우회 변형을 자동 발사 — 정규화로 무력
dalfox url --url 'http://dvwa.el34.lab/?q=FUZZ' 2>&1 | sed -r 's/\x1b\[[0-9;]*m//g' | grep -iE 'WAF|reflected|XSS found'
```

dalfox 는 대소문자(`<ScRiPt>`)·URL 인코딩(`%3Cscript%3E`, `%3C`=`<`·`%3E`=`>`) 등 수백 개 우회 조합을
자동으로 던진다. 공격자는 인코딩하면 WAF가 못 알아볼 것을 기대하지만, ModSec은 디코드 후 검사하므로
전부 `403`으로 막히고 dalfox는 `WAF ... (HTTP 403) detected`로 방화벽을 지문한다(`XSS found 0`).
...
ssh ccc@10.20.32.80 'sudo tail -150 /var/log/apache2/modsec_audit.log | grep -oE "94[0-9]1[0-9]{2}" | sort | uniq -c'
```
...
```bash
# 같은 XSS, 다른 WAF 모드 — dvwa는 dalfox가 WAF 403 지문(차단), juice는 whatweb 200(탐지만)
echo '[dvwa 차단]'; dalfox url --url 'http://dvwa.el34.lab/?q=FUZZ' 2>&1 | sed -r 's/\x1b\[[0-9;]*m//g' | grep -iE 'WAF|XSS found'
echo -n '[juice 탐지만] '; whatweb -a1 --color=never 'http://juice.el34.lab/?q=%3Cscript%3Ealert(1)%3C/script%3E' 2>&1 | grep -oE '\[[0-9]{3}[^]]*\]' | head -1
```

**결과 해석**: dvwa는 dalfox가 `WAF ... (HTTP 403) detected`(차단), juice는 whatweb이 `[200 OK]`(탐지만
통과)로 갈린다. 이는 버그가 아니라 **의도된 운영 정책**
...
`ssh ccc@10.20.32.80`(로그 분석)로 실행한다. **인가된 el34 실습 환경 안에서만** 수행한다.
