...
echo "xss=$(echo -en 'GET /?q=<script>alert(1)</script> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```

위 명령에서 `-o /dev/null`은 본문을 버리고, `-w 'xss=%{http_code}\n'`은 응답 코드만 출력
하라는 뜻이다. 자연 URL `http://dvwa.el34.lab/`의 호스트명이 web Apache의 vhost 라우팅에서 어느
가상호스트로 보낼지 지정한다(`/etc/hosts` 등록). 출력이 `xss=403`이면 ModSec이 차단한 것이다.
...
echo "img=$(echo -en 'GET /?q=<img%20src=x%20onerror=alert(1)> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
echo "svg=$(echo -en 'GET /?q=<svg%20onload=alert(1)> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
...
echo "case=$(echo -en 'GET /?q=<ScRiPt>alert(1)</ScRiPt> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
echo "enc=$(echo -en 'GET /?q=%3Cscript%3Ealert(1)%3C/script%3E HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
...
echo -n "dvwa=$(echo -en 'GET /?q=<script>alert(1)</script> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}') "
echo "juice=$(echo -en 'GET /?q=<script>alert(1)</script> HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
