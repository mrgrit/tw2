...
본 주차의 명령은 사용하는 두 장비에 직접 접속해 실행한다 — SIEM `ssh ccc@10.20.32.100`(비번 1,
`/var/ossec` 은 sudo), 표적 web `ssh ccc@10.20.32.80`(비번 1). 두 장비의 역할은 다음과 같다.
...
> **실습 진행 원칙.** SIEM `ssh ccc@10.20.32.100`(/var/ossec 은 sudo)·표적 web `ssh ccc@10.20.32.80`
> 에 직접 접속해. **인가된 실습 환경(el34)에서만** 수행한다. 보강 룰은 `wazuh-logtest` 로
