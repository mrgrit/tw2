...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 공격을 자동화하나 (수동 반복의 한계, adversary emulation 의 정의) | 강의      |
| 0:25–0:55   | 이론 — MITRE ATT&CK 프레임워크 복습 + Caldera 4 구성요소                    | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — el34 의 Caldera 설치 해부(`--recursive`·sandcat agent) + ATT&CK=atomic | 강의/토론 |
| 1:35–2:00   | 실습 — 점검(Caldera·Wazuh) + discovery 에뮬레이션 (lab 1–2)                 | 실습      |
| 2:00–2:30   | 실습 — persistence·execution 에뮬레이션 (lab 3–4)                           | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 — 탐지 매핑 + 커버리지 갭 식별 (lab 5–6)                               | 실습      |
| 3:10–3:30   | 실습 — Caldera 자동화 구조 이해 + 정리 + 에뮬레이션 보고서 (lab 7–8)        | 실습      |
| 3:30–3:40   | 정리 + 다음 주차(W14 — purple team 커버리지 보강) 예고                      | 정리      |
...
- **server** — 웹 UI 와 REST API 를 제공하는 본부다. 운영자가 여기서 adversary 를 고르고,
  연결된 agent 에게 작전을 지시하며, 결과를 본다. el34 에서는 **호스트(192.168.0.151)에 직접
...
이번 주를 위해 Caldera 를 **el34 호스트(192.168.0.151)에 직접 설치**했다 — 컨테이너 안이
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 `1`)에서 실행한다.
