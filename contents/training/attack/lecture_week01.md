...
graph TD
    ...
    ATT -->|"nmap / nc<br/>대상 192.168.0.161"| FW
    FW -->|"포트 forward"| IPS
    IPS --> WEB
...
| **vhost(L7)** | 같은 포트 뒤 어떤 앱인가 | `whatweb -a1 http://dvwa.el34.lab/` / `whatweb` | web Apache 가 호스트명으로 dvwa/juice/… 분기 |
...
```bash
ssh ccc@10.20.31.2 'sudo tail -2000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")|.alert.signature" | sort | uniq -c'
```
