...
> `el34` 위에서 외부 공격자 VM(`192.168.0.202`)에서 대상의 포트·서비스·웹 기술 스택을
...
한 IP 주소(예: `192.168.0.161`)를 큰 건물 한 채로 생각하자. 이 건물에는 0번부터
...
```mermaid
graph TD
    IP["대상 IP<br/>192.168.0.161<br/>(건물 한 채)"]
...
> 위장)하지 않는다. 따라서 공격자 `외부 공격자 VM(192.168.0.202)`의 IP 가 Suricata·웹
...
```mermaid
graph TD
    ATT["공격자<br/>외부 공격자 VM<br/>192.168.0.202"]
...
본 주차의 공격 위치는 **외부 공격자 VM `192.168.0.202`(att/1)** — 네 자신의 공격 머신이다
(`ssh att@192.168.0.202`, 비번 `1`). 이 VM 은 외부에서 대상의 **공인 진입 `192.168.0.161`** 을
공격하며, fw 가 그 공인 포트를 DNAT 로 내부에 전달하므로 공격자는 내부 IP 를 몰라도 공개 포트만으로
내부 웹에 도달한다. 실제 외부 공격자와 똑같은 위치·경로이며, 내 출처 IP(192.168.0.202)는 방어
스택(Suricata/ModSec/Wazuh)에 그대로 보존된다.

```mermaid
graph TD
    ATT["외부 공격자 VM<br/>192.168.0.202<br/>(내 공격 머신·도구 설치됨)"]
    FW["공인 진입<br/>192.168.0.161<br/>el34-fw DNAT(nftables)"]
    ...
    ATT -->|"nmap / curl<br/>대상 192.168.0.161"| FW
    FW -->|"포트 forward"| IPS
    IPS --> WEB
...
> 게이트웨이(192.168.0.161)의 공개 포트만으로 내부 웹에 도달한다.
...
| **vhost(L7)** | 같은 포트 뒤 어떤 앱인가 | `curl -sI http://dvwa.el34.lab/` / `whatweb` | web Apache 가 호스트명으로 dvwa/juice/… 분기 |
...
### 3.3 실습 접근 모델 — 내 공격 VM(SSH) vs 방어 장비(SSH)

이 트랙은 **실제 외부 공격자 위치**에서 진행한다. 공격은 내 공격 VM
(`192.168.0.202`, att/1)에 SSH 로 로그인해 거기서 도구를 돌린다 — 남의 컨테이너를
`docker exec` 로 조작하는 게 아니라, 진짜 공격자처럼 내 박스에서 대상의 공인 진입
(`192.168.0.161`)을 친다. 방어 가시성(파랑 시선)을 볼 때는 해당 방어 장비
(예: IPS `10.20.31.2`)에 직접 SSH 한다.

```bash
# 공격자 시선 — 내 공격 VM 에서 도구 실행
ssh att@192.168.0.202                      # 비밀번호: 1
nmap --version

# 방어자 시선 — IPS 장비에 직접 접속해 Suricata 로그 확인
ssh ccc@10.20.31.2                         # 비밀번호: 1
tail /var/log/suricata/eve.json
```

> **자연 URL.** 내 공격 VM 의 `/etc/hosts` 에 `192.168.0.161 dvwa.el34.lab juice.el34.lab ...`
> 가 등록돼 있어, `http://dvwa.el34.lab/` 처럼 **호스트명으로 바로 접속**한다(브라우저에서 그
...
**el34 에서 어떻게**: 내 공격 VM에서 게이트웨이(192.168.0.161)의 공개 포트를 스캔한다.

```bash
nmap -sV -p 80,443,8001-8007,9100 192.168.0.161 -T4 --max-retries 1
```
...
**el34 에서 어떻게**: 내 공격 VM에서 vhost(호스트명)를 바꿔 가며 응답 헤더를 본다.

```bash
curl -sI http://dvwa.el34.lab/
```
...
**el34 에서 어떻게**: 내가 방금 보낸 정찰이 Suricata 에 내 IP(192.168.0.202)로
탐지됐는지 확인한다.

```bash
ssh ccc@10.20.31.2 'tail -2000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")|.alert.signature" | sort | uniq -c'
```
...
- `jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")"` — **jq** 는
...
- 출력이 보인다 = **내 정찰이 탐지됐다.** 출처가 내 IP(192.168.0.202)로 또렷이
...
`ssh att@192.168.0.202` 로 수행하며, **인가된 실습 환경에서만** 진행한다.
...
> 192.168.0.202 로 탐지됐는지, 무슨 시그니처로 분류됐는지.
>
