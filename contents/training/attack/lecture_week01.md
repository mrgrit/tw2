...
접근은 호스트에 SSH 로 들어간 뒤 `docker exec` 로 내 공격 VM에 명령을 보내는
것이다.

```bash
ssh ccc@192.168.0.80                       # 비밀번호: 1
nmap --version    # 내 공격 VM에서 도구 실행
```
...
**el34 에서 어떻게**: 내 공격 VM에서 Host 헤더를 바꿔 가며 응답 헤더를 본다.

```bash
curl -sI -H 'Host: dvwa.el34.lab' http://192.168.0.161/
```
...
- `-H 'Host: dvwa.el34.lab'` — **Host 헤더 지정**. 같은 IP(192.168.0.161)라도 이 헤더에
...
```bash
docker exec el34-ips sh -c 'tail -2000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")|.alert.signature" | sort | uniq -c'
```
