...
객체로 분석하는 라이브러리다. el34 의 공격자 컨테이너(`el34-attacker`)에 **scapy 2.7** 이
설치되어 있다.
...
요구한다. el34 의 attacker 컨테이너는 root 로 동작하므로 scapy 가 그대로 동작한다.

```bash
# scapy 가용 확인 — 버전이 찍히면 패킷 크래프팅 준비 완료
docker exec el34-attacker python3 -c "import scapy; print('scapy', scapy.__version__)"
```
...
> 정해진 대상(`el34-attacker` → el34 내부 fw 게이트웨이 `10.20.30.1`)에 한해서만 수행한다.
...
docker exec el34-attacker python3 -c "from scapy.all import *; \
  ans,u=sr(IP(dst='10.20.30.1')/TCP(dport=[22,80,443,8001,8002,3306],flags='S'),timeout=4,verbose=0); \
  print('open:', sorted(p[1].sport for p in ans if p[1][TCP].flags==0x12))"
```

이 한 줄을 뜯어보면 패킷 크래프팅의 문법이 보인다.

- **`IP(dst='10.20.30.1')`** — IP 계층. 목적지를 fw 게이트웨이로 지정한다.
...
docker exec el34-attacker python3 -c "from scapy.all import *; \
  a=sr1(IP(dst='10.20.30.1')/TCP(dport=80,flags='S'),timeout=3,verbose=0); \
...
docker exec el34-attacker python3 -c "from scapy.all import *; \
  sr(IP(dst='10.20.30.1')/TCP(dport=80,flags='F'),timeout=2,verbose=0); \
  sr(IP(dst='10.20.30.1')/TCP(dport=80,flags='FPU'),timeout=2,verbose=0); \
  sr(IP(dst='10.20.30.1')/TCP(dport=80,flags=0),timeout=2,verbose=0); \
...
docker exec el34-attacker python3 -c "from scapy.all import *; \
  sr(IP(dst='10.20.30.1')/TCP(dport=(1,200),flags='S'),timeout=6,verbose=0); print('mass scan sent')"
sleep 5
docker exec el34-ips sh -c 'tail -3000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"10.20.30.202\" and (.alert.signature|test(\"scan\";\"i\")))|.alert.signature" | sort | uniq -c | tail -2'
```
...
가 모든 탐지를 JSON 한 줄씩 적는 로그)에서, **출처 IP `10.20.30.202`**(el34-attacker 의 내부
...
# ③ 트리거 — NULL 패킷 전송
docker exec el34-attacker python3 -c "from scapy.all import *; sr(IP(dst='10.20.30.1')/TCP(dport=80,flags=0),timeout=2,verbose=0); print('null sent')"
sleep 4
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서
> `docker exec el34-attacker`(공격) 또는 `docker exec el34-ips`(방어)로 실행한다. **인가된
...
> 첫 단계는 도구 가용성 점검이다.
>
> **무엇을 알 수 있는가?** attacker 컨테이너에 scapy 가 설치되어 있고(버전 2.7), raw socket
> 을 쓸 root 권한이 있는지.
>
...
- **인가된 실습만.** 모든 패킷 크래프팅·스캔은 인가된 실습 환경(el34) 안에서, 정해진 대상
  (`el34-attacker` → fw 게이트웨이 `10.20.30.1`)에 한해서만. 임의 외부 IP 를 향한 포트
  스캔은 불법이며 RoE 위반이다.
