...
| 시간        | 내용                                                              | 유형     |
|-------------|------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — "입력으로 경계를 넘는다"는 한 줄 통찰 + 신뢰 경계 개념     | 강의     |
| 0:20–0:50   | 이론 — 경로순회(A03/A05): `../` 의 원리, 인코딩 우회, 정규화      | 강의     |
| 0:50–1:00   | 휴식                                                             | —        |
| 1:00–1:30   | 이론 — 명령주입·PHP 주입(A03): `;cmd`, `<?php`, webshell→RCE     | 강의     |
| 1:30–2:00   | 이론 — SSRF(A10): 169.254.169.254, 내부망 피벗, 위험 스킴        | 강의/토론 |
| 2:00–2:10   | 휴식                                                             | —        |
| 2:10–2:35   | 실습 1·2 — 대상 점검 + 경로순회 발사(평문/인코딩) → 403          | 실습     |
| 2:35–3:00   | 실습 3·4 — SSRF 시도 + PHP 코드 주입 → 탐지                       | 실습     |
| 3:00–3:25   | 실습 5·6·7 — ModSec 930/932/933 탐지 분석 + 영향·방어 정리       | 실습     |
| 3:25–3:40   | 실습 8 — 공격 보고서 작성 + 다음 주차(W08 중간고사 CTF) 예고      | 정리     |
...
진행한다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 `1`)에서
