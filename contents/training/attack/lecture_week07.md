...
1. 경로순회·명령주입·SSRF 세 공격이 공통적으로 "입력으로 경계를 넘는다"는 것을 한
   ...
   구분한다.
2. 외부 공격자 VM 192.168.0.202에서 dvwa 대상으로 `../../etc/passwd` 경로순회를 평문과
...
> attacker → `http://192.168.0.161/?q=<페이로드>` (Host 헤더 `dvwa.el34.lab`) 를 쓴다.

```mermaid
graph TD
    A["외부 공격자 VM 192.168.0.202<br/>curl ?q=../../etc/passwd<br/>(Host: dvwa.el34.lab)"]
...
`ssh att@192.168.0.202 ...` 로 발사하며, **인가된 학습 환경(el34) 안에서만** 시도한다.
...
> **무엇을 알 수 있는가?** attacker → `http://192.168.0.161/`(Host `dvwa.el34.lab`) 경로가
