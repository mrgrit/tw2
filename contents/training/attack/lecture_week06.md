...
| 시간        | 내용                                                                  | 유형      |
|-------------|-----------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 인증 vs 인가, A01 이 1위인 이유, 접근제어 결함의 3 가지 얼굴      | 강의      |
| 0:25–0:45   | 이론 — 강제 브라우징 / 무인증 관리 기능 / IDOR 원리                     | 강의      |
| 0:45–0:55   | 휴식                                                                   | —         |
| 0:55–1:20   | 실습 1, 2 — 대상 점검 + /ftp 강제 브라우징                              | 실습      |
| 1:20–1:50   | 실습 3, 4 — 무인증 관리 엔드포인트 + 보호 vs 노출 비교                  | 실습      |
| 1:50–2:00   | 휴식                                                                   | —         |
| 2:00–2:25   | 실습 5, 6 — IDOR 순차 자원 + access.log 탐지                            | 실습      |
| 2:25–2:45   | 실습 7, 8 — 방어 정리 + 접근제어 공격 보고서                            | 실습      |
| 2:45–2:50   | 정리 + 다음 주차 예고                                                   | 정리      |
...
```mermaid
graph TD
    ATK["공격자<br/>el34-attacker 10.20.30.202<br/>또는 외부 VM 192.168.0.202"]
    ATK -->|"curl /ftp · /rest/admin · 순차 ID"| FW["fw 10.20.30.1<br/>nftables DNAT<br/>L3/L4만 — 그대로 통과"]
...
- `docker exec el34-attacker sh -c "..."` — el34 호스트에서 공격자 컨테이너 안의 셸로
  명령을 실행한다. 모든 실습 명령은 `ssh ccc@192.168.0.151` 로 호스트에 접속한 뒤 이
  형태로 수행한다.
...
- `-w 'ftp=%{http_code}\n'` — `-w`(write-out)는 요청이 끝난 뒤 지정한 형식을 출력한다.
  ...
  줄이 찍힌다.
- `-H 'Host: juice.el34.lab'` — `Host:` 헤더를 juiceshop vhost 로 지정한다. el34 web 의
  Apache 는 이 헤더를 보고 어느 사이트로 보낼지 결정하므로(W01 의 vhost 라우팅), 반드시
  지정해야 juiceshop 에 도달한다.
- `http://10.20.30.1/ftp` — fw 의 ext 인터페이스(10.20.30.1)로 보낸다. 내부 공격자
  컨테이너는 이 게이트웨이를 거쳐 web 에 닿는다.
...
```bash
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'users=%{http_code} ' -H 'Host: juice.el34.lab' http://10.20.30.1/api/Users"
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'admincfg=%{http_code}\n' -H 'Host: juice.el34.lab' http://10.20.30.1/rest/admin/application-configuration"
```
...
```bash
for i in 1 2 3; do docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'product$i=%{http_code} ' -H 'Host: juice.el34.lab' http://10.20.30.1/rest/products/$i"; done; echo
```
...
```bash
docker exec el34-web sh -c 'sudo tail -50 /var/log/apache2/*access.log 2>/dev/null | grep -aE "/ftp|/rest/admin" | tail -5'
```

명령 풀이.

- `docker exec el34-web sh -c '...'` — 이번엔 공격자가 아니라 **방어 측 컨테이너(web)**
...
본 주차 lab(`lab_week06.yaml`)은 8개 미션으로 구성된다. 모든 명령은 el34 호스트
(`ssh ccc@192.168.0.151`, 비밀번호 1)에 접속한 뒤 `docker exec` 로 실행하며, **인가된 el34
실습 환경에서만** 수행한다. 각 미션을 4축(왜·무엇을·결과 해석·실전)으로 안내한다.
