...
- `http://192.168.0.161/ftp` — fw 의 ext 인터페이스(192.168.0.161)로 보낸다. 내부 공격자
...
```bash
curl -s -o /dev/null -w 'legal=%{http_code}\n' -H 'Host: juice.el34.lab' http://192.168.0.161/ftp/legal.md
```
...
```bash
curl -s -o /dev/null -w 'users=%{http_code} ' -H 'Host: juice.el34.lab' http://192.168.0.161/api/Users
curl -s -o /dev/null -w 'admincfg=%{http_code}\n' -H 'Host: juice.el34.lab' http://192.168.0.161/rest/admin/application-configuration
```
...
```bash
for i in 1 2 3; do docker exec 외부 공격자 VM 192.168.0.202 sh -c "curl -s -o /dev/null -w 'product$i=%{http_code} ' -H 'Host: juice.el34.lab' http://192.168.0.161/rest/products/$i"; done; echo
```
