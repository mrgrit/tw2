...
- `echo -en 'GET /ftp HTTP/1.0\r\nHost: juice.el34.lab\r\n...'` — 요청 라인과 헤더를
  손으로 작성해 raw HTTP 요청을 만든다. `Host:` 헤더로 juice vhost 를 지정한다.
- `| nc -w3 192.168.0.161 80` — 웹 진입점(공인 .161:80)으로 요청을 그대로 보낸다.
  `-w3` 은 3초 타임아웃이다.
- `| head -1 | grep -oE '[0-9]{3}'` — 응답 첫 줄(상태 라인)에서 세 자리 상태 코드만
  뽑는다. `ftp=$(...)` 로 감싸면 `ftp=200` 같은 한 줄이 찍힌다.
...
```bash
echo "legal=$(echo -en 'GET /ftp/legal.md HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
...
```bash
echo "admincfg=$(echo -en 'GET /rest/admin/application-configuration HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
...
```bash
echo -n "users=$(echo -en 'GET /api/Users HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}') "
echo "admincfg=$(echo -en 'GET /rest/admin/application-configuration HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
```
...
```bash
for i in 1 2 3; do echo -n "product$i=$(ssh att@192.168.0.202 "echo -en 'GET /rest/products/$i HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}'")" ; done; echo
```
