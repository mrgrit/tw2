...
graph TD
    ...
    ATK -->|"nc /ftp · /rest/admin · 순차 ID"| FW["fw 192.168.0.161<br/>nftables DNAT<br/>L3/L4만 — 그대로 통과"]
...
```bash
# ffuf로 UI에 링크 없는 숨은 경로 발견 — juice SPA 기본응답(9903B)은 -fs로 걸러냄
printf 'ftp\nadmin\nrobots.txt\nmetrics\nassets\n' > /tmp/w6.txt
ffuf -u http://juice.el34.lab/FUZZ -w /tmp/w6.txt -fs 9903 -mc all -s 2>/dev/null
rm -f /tmp/w6.txt
```

명령을 한 부분씩 해석하면 다음과 같다.

- `ffuf -u http://juice.el34.lab/FUZZ -w <wordlist>` — 진짜 웹 퍼저가 워드리스트의 각 단어를
  `FUZZ` 자리에 넣어 요청한다. 자연 URL 의 호스트명이 juiceshop vhost 를 지정한다(공격 VM
  `/etc/hosts` 로 fw 공인 진입 192.168.0.161 에 매핑).
- `-fs 9903` — juice SPA 는 없는 경로에도 index.html(9903B)을 200 으로 돌려주므로, 그 기본응답
  크기를 걸러 실재 경로만 남긴다. `-mc all` 은 상태 코드로 거르지 않는다는 뜻(크기로만 판별).
- el34 web 의 Apache 는 `Host:` 헤더를 보고 어느 사이트로 보낼지 결정하므로(W01 의 vhost 라우팅),
  호스트명이 juiceshop 도달을 결정한다. 외부 공격자 VM 은 이 게이트웨이를 거쳐 web 에 닿는다.

**결과 해석.** 출력에 `ftp`·`metrics`·`robots.txt` 가 뜨면 강제 브라우징 성공이다 — 링크가 없는데도
숨은 경로가 노출된다는 뜻이다. 이어서 그 안의 특정 파일을 직접 집어 올 수도 있다.

```bash
whatweb -a1 http://juice.el34.lab/ftp/legal.md
```

whatweb 이 `[200 OK]` 로 지문하면 디렉토리뿐 아니라 그 안의 파일까지 직접 접근된다는 것이 확인된다. 실제
...
- whatweb `[200 OK]` → **무인증 노출** 이다. 인증 헤더(토큰)를 전혀 보내지 않았는데도 관리
  설정이 돌아왔다(whatweb 은 유출되는 Email 까지 지문) = 인가 검사가 누락된 전형적 A01 결함이다.
- 만약 `[401 Unauthorized]` 또는 `[403 Forbidden]` 이 나왔다면 접근제어가 정상
...
```bash
# whatweb로 보호(401) vs 노출(200) 두 엔드포인트를 한 번에 지문 — 인가 비일관성
whatweb -a1 --color=never http://juice.el34.lab/api/Users http://juice.el34.lab/rest/admin/application-configuration 2>&1 | grep -oE 'http://[^ ]+ \[[0-9]{3}[^]]*\]'
```
...
의 제품 엔드포인트(`/api/Products/{id}`)에 순차 ID 로 접근해 본다.

```bash
# ffuf로 /api/Products/FUZZ 를 순차 ID(1..9)로 무차별 열거 — IDOR 방법론 자동화
seq 1 9 > /tmp/ids.txt
ffuf -u 'http://juice.el34.lab/api/Products/FUZZ' -w /tmp/ids.txt -mc 200 -s 2>/dev/null | sort -n | tr '\n' ' '; echo
rm -f /tmp/ids.txt
```

명령에서 새로 나온 부분만 풀이한다.

- `seq 1 9 > /tmp/ids.txt` — 순차 ID 1~9 를 워드리스트로 만든다. 진짜 웹 퍼저 ffuf 가 이를
  `FUZZ` 자리에 하나씩 넣어 무차별 대입(brute) 열거한다.
- `-u 'http://juice.el34.lab/api/Products/FUZZ'` — `FUZZ` 자리에 1..9 가 들어가 `.../Products/1`,
  `.../Products/2` … 를 차례로 요청한다. 이것이 바로 "ID 만 바꿔가며 접근" 하는 IDOR 의 핵심 동작이다.
- `-mc 200 -s` — 200 으로 응답한 ID 만 조용히(silent) 뽑아, 실재하는 자원 ID 를 한눈에 열거한다.

**결과 해석.** `1 2 3 4 5 6 7 8 9` 처럼 실재 ID 가 200 으로 쏟아지며 순차 열거가 동작하는 것을
확인한다. 단, 여기서 **반드시 구분해야 할 점** 이 있다.

- 제품 목록(`/api/Products/...`)은 본래 **누구나 볼 수 있는 공개 자원** 이다. 따라서
...
```bash
ssh ccc@10.20.32.80 'sudo tail -50 /var/log/apache2/*access.log | grep -aE "/ftp|/rest/admin" | tail -5'
```
