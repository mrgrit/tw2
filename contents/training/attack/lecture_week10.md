...
> `외부 공격자 VM 192.168.0.202` 에서 평문 SQLi(`UNION SELECT`)를 여러 방식으로 변형해 던지고, ModSec 의
...
1. WAF 우회의 근본 원리 — "WAF 가 보는 표현"과 "백엔드가 해석하는 의미"의 분리 — 를
   ...
   구분한다.
2. `외부 공격자 VM 192.168.0.202` 에서 평문 SQLi(`UNION SELECT`)를 baseline 으로 던져 dvwa(차단 모드)가
   `403` 으로 막는 것을 확인하고, 이 403 을 우회 성공 여부의 기준점으로 삼는다.
3. URL 인코딩 · 이중 인코딩 · 유니코드 인코딩 · 케이스 변조 · 주석 삽입 · 공백 대체 ·
   파라미터 오염(HPP) 의 각 우회 기법을 실제 nc(raw HTTP) 페이로드로 변형해 전송하고, **무엇이 막히고
...
```mermaid
graph TD
    A["el34 호스트<br/>ssh ccc@192.168.0.80"]
    B["내 공격 VM ssh att@192.168.0.202<br/>nc raw HTTP 로 변형 페이로드 전송"]
    C["fw 192.168.0.161<br/>nftables DNAT (L3/L4)<br/>HTTP 본문은 안 봄"]
...
본 주차의 실습은 `nc`(raw HTTP) 로 변형 페이로드를 보내 **응답 코드(403/200)와 ModSec 로그**로 우회
...
> 못하면 우회를 시험할 수 없다.
>
> **무엇을 알 수 있는가?** `외부 공격자 VM 192.168.0.202` 에서 fw 게이트웨이(192.168.0.161)에 Host 헤더로 dvwa
...
- **인가된 실습만.** 모든 우회 시도는 인가된 실습 환경(el34) 안에서, 정해진 대상
  (`외부 공격자 VM 192.168.0.202` → `dvwa.el34.lab`)에 한해서만 수행한다. 실제 외부 시스템을 대상으로 한
