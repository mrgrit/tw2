...
3. URL 인코딩 · 이중 인코딩 · 유니코드 인코딩 · 케이스 변조 · 주석 삽입 · 공백 대체 ·
   파라미터 오염(HPP) 의 각 우회 기법을 실제 curl 페이로드로 변형해 전송하고, 그 결과
   (대부분 여전히 403)를 해석한다.
4. ModSec CRS 의 **transformation(`t:`)** 4종(`urlDecodeUni` / `lowercase` /
   `removeComments` / `compressWhitespace`)이 각 우회 표현을 어떻게 원형으로 되돌리는지를,
   변형 → 정규화 → 매칭의 흐름으로 설명한다.
5. web 컨테이너의 `modsec_audit.log` 에서 본인의 우회 시도들이 정규화 후 **942 룰군(SQLi)**
   에 잡힌 흔적을 찾아, "표현은 달라도 정규화하면 같은 공격"임을 증거로 입증한다.
6. 단순 우회가 CRS 에 무력한 이유와, 우회가 여전히 통할 수 있는 고급 영역(정규화가 놓치는
   인코딩, WAF–백엔드 파서 차이, HPP)을 구분해 "WAF 는 완벽이 아니라 진입장벽"이라는
   결론을 정리한다.

> **본 주차의 시선** — W10 은 공격 트랙이지만, "우회가 통했다"를 자랑하는 주가 아니다.
> 오히려 **단순 우회가 왜 대부분 막히는지를 직접 겪어 정규화의 위력을 체감**하고, 그 위에서
> "진짜 우회는 어디서 시작되는가"를 가늠하는 주다. 채점은 우회 성공/실패의 결과가 아니라,
...
| 시간        | 내용                                                                   | 유형      |
|-------------|------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — WAF 우회의 근본 원리(표현 ≠ 의미) + W04–W05 복습 연결           | 강의      |
| 0:20–0:50   | 이론 — 인코딩 우회(URL·이중·유니코드·HTML 엔티티) 상세                 | 강의      |
| 0:50–1:00   | 휴식                                                                   | —         |
| 1:00–1:30   | 이론 — 분할·난독화(주석·공백·케이스·HPP) + 정규화 방어 원리            | 강의/토론 |
| 1:30–2:00   | 실습 — 점검 + 기준 공격(평문 SQLi 403) + 인코딩 우회 (lab 1–3)         | 실습      |
| 2:00–2:30   | 실습 — 분할·난독화 우회 + 정규화 확인 (lab 4–5)                        | 실습      |
| 2:30–2:40   | 휴식                                                                   | —         |
| 2:40–3:10   | 실습 — 탐지 분석(modsec 942) + 우회 한계·고급 (lab 6–7)                | 실습      |
| 3:10–3:30   | 실습 — 필터 우회 보고서 (lab 8) + 결과 종합 토론                       | 실습      |
| 3:30–3:40   | 정리 + 다음 주차(W11 — 권한 상승) 예고                                 | 정리      |
...
이 변형들 역시 **el34 에서는 대부분 여전히 403** 이다. 주석·탭·대소문자 같은 단순 난독화는
CRS 의 정규화(다음 절)가 정확히 겨냥하는 표적이기 때문이다.
...
| transformation | 하는 일 | 무력화되는 우회 |
|----------------|---------|-----------------|
| **`t:urlDecodeUni`** | URL/유니코드 인코딩을 디코드(`%55`→`U`, `%2527`→`'`) | URL 인코딩 · 이중 인코딩 · 유니코드 |
| **`t:removeComments`** | SQL 주석 `/**/` 을 제거(`UN/**/ION`→`UNION`) | 주석 삽입 |
| **`t:compressWhitespace`** | 연속·다양한 공백류를 단일 공백으로(`%09`→` `, 다중→1) | 공백 대체(탭·개행) |
| **`t:lowercase`** | 모든 문자를 소문자로(`UnIoN`→`union`) | 케이스 변조 |

이 4종을 거치면, §2–§3 의 모든 단순 우회 표현이 **하나의 원형**으로 수렴한다.
...
| URL 인코딩 | `%55NION %53ELECT` | `t:urlDecodeUni` | 대부분 403 |
| 이중 인코딩 | `1%2527%20UNION` | `t:urlDecodeUni` | 대부분 403 |
| 주석 삽입 | `UN/**/ION SE/**/LECT` | `t:removeComments` | 대부분 403 |
| 공백 대체(탭) | `UNION%09SELECT` | `t:compressWhitespace` | 대부분 403 |
| 케이스 변조 | `UnIoN sElEcT` | `t:lowercase` | 대부분 403 |
...
> **주목할 한 줄.** 표의 마지막 행 HPP 만 "환경 특이적"이다. 인코딩·주석·공백·대소문자는
> 정규화라는 단일 방어로 깔끔히 무력화되지만, HPP 는 **WAF 와 백엔드가 서로 다른 값을
> 본다**는 구조적 틈을 노리므로 정규화만으로는 닫히지 않는다. 그래서 §6 의 고급 우회는
...
§2–§3 의 인코딩·주석·공백·대소문자 우회가 el34 에서 대부분 403 으로 막히는 이유는 명확하다 —
이 변형들은 모두 **표현 계층**의 장난이고, CRS 의 정규화가 정확히 그 표현 계층을 원형으로
되돌리기 때문이다. 공격자가 표현을 바꾸는 속도보다, 방어자가 정규화로 표현을 통일하는 것이
훨씬 적은 비용이다(룰 N 개 대신 정규화 1번). 그래서 **잘 정규화하는 현대 WAF 앞에서 단순
우회는 거의 통하지 않는다**는 것이 본 주차의 1차 결론이다.
...
> 응답 코드. 그리고 CRS 의 `t:removeComments`·`t:compressWhitespace`·`t:lowercase` 가 이를
> 원형으로 되돌린다는 사실.
>
> **결과 해석.** 정상: 대부분 여전히 `403`. 핵심 깨달음 — 주석·공백·대소문자는 정규화가
> 정확히 겨냥하는 표적이라 무력하다. 비정상: 200 이면 어떤 난독화가 통했는지 기록한다.
>
> **실전 활용.** 분할·난독화가 정규식 기반 약한 룰은 속여도, transformation 을 갖춘 CRS 에는
> 무력함을 확인. "WAF 가 정규화하는가"가 우회 가능성의 분수령임을 체득한다.
...
>
> **무엇을 알 수 있는가?** 단순 우회(인코딩·주석·대소문자)는 CRS 정규화에 무력하다는 것, 그리고
> 고급 우회는 정규화가 놓치는 인코딩·WAF–백엔드 파서 차이·HPP(파라미터 오염) 같은 **구조적
> 틈**에서 출발한다는 것.
>
...
> 같은 공격을 다르게 표현해도 정규화가 통일하므로 단순 우회는 CRS 에 무력하고, 고급 우회는
> 파서 차이를 노린다. 비정상: 우회 결과만 나열하고 "왜 막혔는가(정규화)"가 빠지면 보강한다.
>
