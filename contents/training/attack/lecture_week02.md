...
2. el34-attacker 컨테이너에서 nmap NSE 스크립트(`http-headers`, `http-title`)를 실행해
...
```mermaid
graph TD
    ATT["el34-attacker<br/>10.20.30.202<br/>(내부 발판 공격자)"]
...
**왜 중요한가.** W01 에서는 `curl -I` 로 한 번에 한 vhost 씩 손으로 HTTP 헤더를 확인했다.
NSE 는 이 수작업을 스크립트로 자동화한다. 같은 정보를 더 빠르고 일관되게, 여러 대상에 대해
한 번에 수집할 수 있다.

**el34 에서 어떻게.** el34-attacker 컨테이너에 nmap 이 완비되어 있다. 본 주차에서 쓰는 두
스크립트는 다음과 같다.

```bash
docker exec el34-attacker nmap -sV --script=http-headers,http-title -p 80 10.20.30.1 -T4 --max-retries 1
```
...
- `10.20.30.1` — el34 의 fw 내부 게이트웨이 주소(여기로 보내면 fw → ips → web 으로 forward).
...
**el34 에서 어떻게.** el34-attacker 에 nikto 가 설치되어 있다.

```bash
docker exec el34-attacker sh -c 'nikto -h http://10.20.30.1 -maxtime 30s 2>&1 | head -15'
```

- `-h http://10.20.30.1` — 점검 대상 호스트(host). el34 의 fw 게이트웨이로 보내면 내부 web 까지 전달된다.
...
- **gobuster** — Go 로 작성된 디렉토리/DNS 브루트포서. 널리 쓰이지만, **el34-attacker 에
...
- **ffuf** (Fuzz Faster U Fool) — Go 로 작성된 초고속 웹 퍼저(fuzzer). 입력 위치를 `FUZZ`
  ...
  el34-attacker 에서 안정적으로 동작하므로 **본 주차의 표준 도구** 다.
...
```bash
ffuf -u http://10.20.30.1/FUZZ -w /tmp/aw2.txt -H 'Host: dvwa.el34.lab' -mc 200,301,302,403 -s
```

각 옵션의 의미는 다음과 같다.

- `-u http://10.20.30.1/FUZZ` — 대상 URL. 경로 자리에 `FUZZ` 키워드를 둔다 → 워드리스트 각
  단어가 이 자리에 대입된다.
- `-w /tmp/aw2.txt` — 워드리스트(wordlist) 파일. 시도할 경로 후보가 한 줄에 하나씩 들어 있다.
- `-H 'Host: dvwa.el34.lab'` — HTTP `Host` 헤더를 지정한다. el34 는 한 IP(fw 게이트웨이)에서
  `Host` 헤더로 vhost 를 구분하므로(W01 의 vhost 라우팅), 이 헤더가 있어야 web 의 Apache 가
  요청을 DVWA vhost 로 보낸다.
...
- **ips 의 Suricata eve.json** (`/var/log/suricata/eve.json`) — 출처 IP(공격자 10.20.30.202)
  에서 폭주한 요청이 alert 로 남는다. el34 는 출처 IP 를 보존하므로, alert 의 `src_ip` 가
  실제 공격자 주소 그대로다.

```mermaid
graph TD
    SCAN["열거 트래픽<br/>nikto UA / ffuf 폭주<br/>src 10.20.30.202"]
...
(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec el34-attacker ...` 형태로 실행하며,
**인가된 el34 환경에서만** 한다.

### 실습 1 — 심화 열거 도구 점검 (lab order 1)

> **왜 하는가?** 본격 열거에 앞서 nikto·gobuster·ffuf 가 el34-attacker 에 설치되어 있는지
...
§3 의 명령(`nikto -h http://10.20.30.1 -maxtime 30s`)을 사용한다.
...
§4.2 의 ffuf 명령(`ffuf -u .../FUZZ -w /tmp/aw2.txt -H 'Host: dvwa.el34.lab' -mc 200,301,302,403 -s`)을
사용한다. el34-attacker 의 gobuster 는 버그로 못 쓰므로 반드시 ffuf 를 쓴다.
...
> 건수, ips 의 Suricata eve.json 에서 출처 IP(10.20.30.202)별 alert 건수.
>
