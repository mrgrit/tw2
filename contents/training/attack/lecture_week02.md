...
```bash
nikto -h http://192.168.0.161 -maxtime 30s 2>&1 | head -15
```

- `-h http://192.168.0.161` — 점검 대상 호스트(host). el34 의 fw 게이트웨이로 보내면 내부 web 까지 전달된다.
...
```bash
ffuf -u http://192.168.0.161/FUZZ -w /tmp/aw2.txt -H 'Host: dvwa.el34.lab' -mc 200,301,302,403 -s
```

각 옵션의 의미는 다음과 같다.

- `-u http://192.168.0.161/FUZZ` — 대상 URL. 경로 자리에 `FUZZ` 키워드를 둔다 → 워드리스트 각
  단어가 이 자리에 대입된다.
...
§3 의 명령(`nikto -h http://192.168.0.161 -maxtime 30s`)을 사용한다.
