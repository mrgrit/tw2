...
- **A. 레거시 프로토콜 취약성 실증 (필수, 40점)** — `OT_PROTOCOL_WEAK` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 위험 명령 탐지 분석 (필수, 30점)** — `UNSAFE_COMMAND` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. OT 분리 강화 방어 설계 (필수, 30점)** — `OT_ISOLATED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(OT_PROTOCOL_WEAK) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(UNSAFE_COMMAND) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(OT_ISOLATED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **OT/SCADA 기초: 산업 제어 보안·안전 최우선·레거시·에어갭**.
- **레거시 프로토콜 취약성**(`OT_PROTOCOL_WEAK`): Modbus/DNP3의 무인증·무암호 상태를 평가한다.
- **위험 명령 탐지**(`UNSAFE_COMMAND`): 안전 관련 레지스터 쓰기·비정상 명령을 탐지한다.
- **OT 분리 강화**(`OT_ISOLATED`): Purdue 분리·읽기 전용·SIS를 적용한다.
