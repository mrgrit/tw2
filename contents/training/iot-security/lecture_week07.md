...
- **A. 페어링 취약성 실증 (필수, 40점)** — `PAIRING_WEAK` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 스니핑/MITM 분석 (필수, 30점)** — `BLE_INTERCEPTABLE` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. BLE 강화 방어 설계 (필수, 30점)** — `BLE_SECURED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(PAIRING_WEAK) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(BLE_INTERCEPTABLE) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(BLE_SECURED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **BLE 해킹: 약한 페어링·스니핑·MITM**.
- **페어링 취약성**(`PAIRING_WEAK`): Just Works·Legacy 등 약한 페어링 사용 여부를 평가한다.
- **스니핑/MITM**(`BLE_INTERCEPTABLE`): 스니핑 키 추출·MITM 세션 장악이 가능한지 판정한다.
- **BLE 강화**(`BLE_SECURED`): LE Secure Connections·MITM 보호·앱 암호화를 적용한다.
