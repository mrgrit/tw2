...
4. Suricata eve.json 에서 한 출발지(`192.168.0.202`)의 `event_type` 분포(alert / http / flow)와
...
공격자(`192.168.0.202`)든 외부 공격자 VM(`192.168.0.202`)이든, 그 실제 IP 가 Suricata · ModSecurity ·
Wazuh 의 전 계층에 그대로 보존된다. 이것이 본 주차의 교차 상관을 가능하게 하는 전제다.

```mermaid
graph TD
    ATK["공격자<br/>192.168.0.202<br/>(출처 IP 보존)"]
    L1["Apache access.log<br/>client IP = 192.168.0.202"]
    L2["ModSecurity audit<br/>remote_address = 192.168.0.202"]
    L3["Suricata eve.json<br/>src_ip = 192.168.0.202"]
...
본 주차의 세 로그는 두 컨테이너에 나뉘어 있다. 접근은 항상 el34 호스트(`ssh ccc@192.168.0.80`,
...
```
192.168.0.202 - - [20/Jun/2026:12:53:10 +0000] "GET /?q=<script>alert(1)</script>&id=1+UNION+SELECT HTTP/1.1" 403 438 "-" "sqlmap/1.5"
```

각 필드의 의미는 다음과 같다.

- `192.168.0.202` — **client IP**. el34 출처 보존 덕분에 실제 공격자 IP 가 그대로 보인다.
...
```mermaid
graph TD
    FLOW["공격자 한 연결<br/>src_ip 192.168.0.202"]
...
세 로그는 출발지를 각기 다른 필드 이름으로 부른다. 이 셋이 같은 값(`192.168.0.202`)이면 한
사건이다.

```mermaid
graph TD
    M["ModSecurity audit<br/>transaction.remote_address<br/>= 192.168.0.202"]
    A["Apache access.log<br/>client IP (첫 필드)<br/>= 192.168.0.202"]
    S["Suricata eve.json<br/>src_ip<br/>= 192.168.0.202"]
...
(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `docker exec` 로 실행한다. 공격은 `외부 공격자 VM 192.168.0.202`
...
> **이 실습을 왜 하는가?** 분석할 흔적을 직접 만든다. 외부 공격자 VM 192.168.0.202에서 차단형 vhost
> (`dvwa.el34.lab`)에 SQLi · XSS · 스캐너 UA 를 흘려 세 로그를 동시에 채운다.
>
> **무엇을 알 수 있나?** 한 공격자가 보낸 세 종류의 공격이 세 로그에 각기 다르게 남는다는
> 사실. 진짜 도구 sqlmap·dalfox 가 보낸 요청이 access(요청)·audit(룰 매치)·eve(네트워크)에 모두 흔적을 남긴다.
>
