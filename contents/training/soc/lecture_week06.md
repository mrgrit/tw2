...
```bash
ssh ccc@10.20.32.100 'tail -2000 /var/ossec/logs/alerts/alerts.json | jq -rc "select(.data.src_ip==\"192.168.0.202\")|[.timestamp,.rule.description]|@tsv" | tail -10'
```
...
```bash
ssh ccc@10.20.31.2 'sudo tail -3000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")|[.alert.signature_id,.alert.signature]|@tsv" | sort | uniq -c | tail -5'
```
