...
3. el34의 `alerts.json`에서 **같은 출발지 IP(192.168.0.202)** 의 경보를 **시간순**으로
...
## 0.5 핵심 개념 — "경보 한 줄"과 "공격 한 편"의 차이
...
SOC 분석가가 마주하는 경보는 대부분 **한 줄짜리 사실**이다. "192.168.0.202가 포트 스캔을
...
graph TD
    ...
    A --> B["스캔 경보<br/>192.168.0.202"]
    A --> C["SQLi 경보<br/>192.168.0.202"]
    A --> D["인증 실패 경보<br/>192.168.0.202"]
...
- **출발지 상관(같은 src_ip)** — 같은 공격자 IP에서 나온 경보끼리 묶는다. el34는 fw가
  ...
  `192.168.0.202`(외부 공격자 VM 공격자) 또는 `192.168.0.202`(외부 공격자 VM)로 정확히 묶을 수
...
graph TD
    ...
    POOL --> FILTER["1. 출발지 필터<br/>src_ip == 192.168.0.202"]
...
```bash
docker exec el34-ips sh -c 'tail -3000 /var/log/suricata/eve.json | jq -rc "select(.event_type==\"alert\" and .src_ip==\"192.168.0.202\")|[.alert.signature_id,.alert.signature]|@tsv" | sort | uniq -c | tail -5'
```

- 출발지가 공격자 `192.168.0.202`로 **보존**되어 정확히 묶인다.
...
모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `docker exec`로 실행하며,
...
> **이 실습을 왜 하는가?**
> 분석할 캠페인이 있어야 분석을 한다. 외부 공격자 VM 192.168.0.202에서 **정찰(포트 스캔) → 초기침투
...
>
> **이걸 하면 무엇을 알 수 있는가?**
> - `alerts.json`에서 src_ip 192.168.0.202의 경보를 시간순으로 추출하는 법
> - 추출된 순서가 ATT&CK 전술 순서(정찰→침투)와 맞아떨어지는지 = 캠페인 신호
>
> **결과 해석**
> 출력에 출발지 192.168.0.202의 경보들이 시간순으로 나열되면 합격. 위→아래로 읽어 단계
...
>
> **이걸 하면 무엇을 알 수 있는가?**
> - eve.json에서 출발지 192.168.0.202의 alert signature_id/signature 집계
