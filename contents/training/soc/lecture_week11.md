...
| el34 위치 | `ssh ccc@10.20.32.80 sudo osqueryi` (컨테이너 안) | 호스트(192.168.0.80)에 설치, 컨테이너 내부까지 포착 |

> **el34 사실 — Sysmon 은 호스트에 산다.** el34-web 컨테이너는 비특권이라 eBPF 센서를 직접
> 올릴 권한(`CAP_BPF`/`SYS_ADMIN`)이 없다. 그래서 **Sysmon for Linux 는 el34 호스트
> (192.168.0.80)에 systemd 데몬으로 설치** 돼 있다. 컨테이너 프로세스도 결국 **호스트 커널이
...
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT pid,name,cmdline FROM processes WHERE cmdline LIKE "%c2_beacon%";'

# 네트워크 IOC — 비표준(높은) 포트로 나가는 외부 연결
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT pid,remote_address,remote_port FROM process_open_sockets WHERE remote_port>40000;'
```
...
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT pid,port,address FROM listening_ports WHERE port=45511;'
```
...
```bash
# 악성 프로세스 + C2 리스너 종료
ssh ccc@10.20.32.80 'pkill -f "[c]2_beacon"; pkill -f "[h]ttp.server 45511"; true'
# 제거 확인 — 결과가 비어야 정상
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT pid FROM processes WHERE cmdline LIKE "%c2_beacon%";'
```
...
ssh ccc@10.20.32.80 'pgrep -f "[c]2_beacon" && echo "잔재!" || echo clean'
```
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`)에서 실행한다. 감염
> 재현·호스트 분석은 `ssh ccc@10.20.32.80`, C2 차단 룰은 `ssh ccc@10.20.31.2`, 점검의
