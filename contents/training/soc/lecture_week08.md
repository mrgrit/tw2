...
3. el34 가 **출처 IP 를 보존**한다는 사실(fw 가 SNAT 를 하지 않음)을 이용해, 네 소스에
   흩어진 흔적을 **같은 출발지 IP(`192.168.0.202`) + 시각**으로 묶어 한 공격자의 행위로
   상관(correlation)한다.
...
> 공격자의 진짜 출발지 `192.168.0.202` 를 본다 — 이것이 본 중간고사에서 흩어진 흔적을 **한
> 서사로 엮는 키**다.
...
```mermaid
graph TD
    ATK["한 공격자<br/>출발지 192.168.0.202"]
...
**무엇을 하나.** 네트워크는 Suricata `eve.json` 에서 출발지 `192.168.0.202` 의 alert 를, 웹은
...
`remote_address` 가 같은 `192.168.0.202` 임을 확인하면, 두 흔적이 같은 공격자의 것임이
증명된다.
...
el34 의 외부 공격자 VM 공격자(`외부 공격자 VM 192.168.0.202`, 출발지 `192.168.0.202`)가 fw 의 게이트웨이
(`192.168.0.161`)를 거쳐 web 으로 들어가는 경로는 다음과 같다.

```mermaid
graph TD
    ATK["공격자 진짜 출발지<br/>192.168.0.202"]
    FW["el34-fw (nftables)<br/>DNAT 만 함 · SNAT 안 함<br/>→ 출발지 IP 그대로 통과"]
    NET["네트워크: Suricata eve.json<br/>src_ip = 192.168.0.202"]
    WEB["웹: modsec_audit + access.log<br/>remote_address = 192.168.0.202"]
    AUTH["인증: auth.log<br/>from 192.168.0.202 (SSH 시도)"]
...
그래서 안쪽의 모든 소스가 공격자의 진짜 출발지 `192.168.0.202` 를 본다. Suricata 의 `src_ip`,
ModSec 의 `remote_address`, `auth.log` 의 `from <IP>` 가 모두 같은 IP 를 가리킨다.

### 3.2 출처 보존이 곧 상관의 키

이 한 IP 가, 정찰·웹 침투·인증 공격이라는 **흩어진 흔적을 한 공격자의 한 사슬로 엮는
키**다. 만약 el34 가 SNAT 를 했다면 안쪽 소스는 fw 의 IP(`192.168.0.161`)만 보게 되어, "이
스캔과 저 SQLi 가 같은 사람인가?"라는 질문에 영영 답할 수 없었을 것이다. 출처 보존 덕에
중간고사의 핵심 미션(미션 7 의 교차 상관)이 성립한다.

> **참고 — 외부 공격자도 마찬가지.** 본 시험의 주 시나리오는 외부 공격자 VM 공격자
> `192.168.0.202` 이지만, 별도 외부 공격자 VM(`192.168.0.202`)이 공인 IP `.161` 을 공격하는
> 경우에도 출처는 `192.168.0.202` 로 보존된다. 그래서 미션 9 의 침투 판정은 두 출처
> (`192.168.0.202`, `192.168.0.202`)의 `Accepted` 를 함께 확인한다. el34 의 4-tier
...
한 다단계 공격이 남기는 네 소스의 흔적을 **출발지 IP(192.168.0.202) + 시각**으로 엮으면 하나의
타임라인이 된다. 같은 사건이 소스마다 다른 단서로 보인다는 것을 이해하는 것이 종합 분석의
정점이다.

```mermaid
graph TD
    EVENT["한 공격자의 다단계 공격<br/>출발지 192.168.0.202"]
    NET["네트워크 eve.json<br/>src_ip + 스캔/sqlmap 시그니처"]
    WEB["웹 modsec_audit + access<br/>remote_address + 942/949110 403"]
    AUTH["인증 auth.log<br/>from 192.168.0.202 · Invalid user 다발"]
...
와 웹의 `remote_address` 와 인증의 `from <IP>` 가 모두 같은 `192.168.0.202`** 라는 것 — 이것이
...
> **시험 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`)에서 실행한다. 공격
> 재현은 `ssh att@192.168.0.202`, 네 소스 분석은 각 컨테이너(`el34-ips`/`el34-web`/
...
> 어떻게 남는지 출발점을 만든다.
>
> **무엇을 알 수 있는가?** 한 공격자(`192.168.0.202`)의 세 단계 — 포트 스캔(정찰) → SQLi(웹
...
> 탐지됐는지 본다.
>
> **무엇을 알 수 있는가?** `eve.json` 에서 출발지 `192.168.0.202` 의 alert 시그니처 분포 —
> 스캔 탐지와 sqlmap UA 시그니처가 다수임을 식별하는 법. `event_type=="alert"` + `src_ip` 로
> 한 공격자만 골라내는 필터링.
>
> **결과 해석.** 정상: 출발지 `192.168.0.202` 의 alert 가 다수 잡히고 스캔/SQLi 시그니처가
...
> 통합 관제의 전제다.
>
> **무엇을 알 수 있는가?** 출발지 `192.168.0.202` 의 경보가 Wazuh `alerts.json` 에 다소스
> (네트워크/웹 등) 그룹으로 모여 있는지. 네 소스가 한 manager 로 수렴했다는 증거.
>
> **결과 해석.** 정상: 출처 `192.168.0.202` 의 경보 그룹 분포에 `ids` 등 다소스가 보임. 비정상:
...
> 공격자의 단계적 체인"임을 입증한다.
>
> **무엇을 알 수 있는가?** 같은 출발지 `192.168.0.202` 의 경보를 `alerts.json` 에서 **시간순
> 체인**으로 늘어놓는 법. 출처가 보존되므로 네트워크·웹의 흔적이 같은 IP 로 묶인다는 것 —
> 이것이 "한 공격자"임을 증명하는 키.
>
> **결과 해석.** 정상: 출처 `192.168.0.202` 의 경보가 시각순으로 나열되어 정찰→침투의 진행이
...
> 판정한다 — 분석의 결론.
>
> **무엇을 알 수 있는가?** 공격 출처(`192.168.0.202` / `192.168.0.202`)의 `Accepted` 유무로
...
- **출처 IP 로 묶는다.** 흩어진 흔적은 같은 출발지 IP(`192.168.0.202`, 외부의 경우 `192.168.0.202`)
