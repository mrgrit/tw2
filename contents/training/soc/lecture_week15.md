...
2. 한 APT 행위자(출처 IP `192.168.0.202`)의 4 단계 캠페인을 재현하고, 그 흩어진 흔적을
...
이 시험의 APT 는 한 공격자(`외부 공격자 VM 192.168.0.202`, 출처 IP `192.168.0.202`)가 **일관된 출처**로
4 단계를 진행하는 시나리오다.
...
(osquery 의 백도어 계정), 네트워크(Suricata eve)의 흔적을 모두 같은 출처 `192.168.0.202` 로
...
**무엇을 하나.** **격리(Contain)** — APT 출처(`192.168.0.202`)를 IDS 룰(sid `9515001`)로
...
|------|------|-----------|------|-----------|
| ① 정찰 | 스캐너 UA(sqlmap) | 네트워크 eve / 웹 audit | 트리아지(P 결정) | eve 시그니처 / src 192.168.0.202 |
| ② 침투 | SQLi(UNION SELECT) | 웹 ModSec(942 → 949110) | 격리(sid 9515001) | audit 403 + remote 192.168.0.202 |
...
```mermaid
graph TD
    P1["① 캠페인 개요<br/>출처 192.168.0.202, 4 단계 킬체인"]
...
> 정반대다. **DNAT** 는 **목적지**를 바꾼다 — el34 의 fw 가 공개 주소 `192.168.0.161` 로 온
...
graph TD
    ...
    A["네트워크 eve.json<br/>src_ip = 192.168.0.202"]
    B["웹 modsec_audit<br/>remote_address = 192.168.0.202"]
    C["인증 auth.log<br/>from 192.168.0.202"]
...
fw 가 SNAT 를 하지 않으므로, 안쪽의 모든 소스가 APT 의 **진짜 출처 IP `192.168.0.202`** 를
...
10.20.40` 이며, 공격은 ext 의 공격자(`192.168.0.202`)에서 시작해 fw(`192.168.0.161`)를 거쳐
...
echo '{"apt_ioc":"aptsoc","src_ip":"192.168.0.202"}' | docker exec -i el34-siem sudo /var/ossec/bin/wazuh-logtest
...
IP(`192.168.0.202`)인지. 여러 소스 그룹(`ids` 등)이 한 출처로 모이는 것이 4 단계를 한 캠페인으로
...
> 어떻게 남는지 출발점을 만든다.
>
> **무엇을 알 수 있는가?** 한 공격자(`192.168.0.202`)의 네 단계 — 포트 스캔(정찰) → SQLi(웹
...
>
> **무엇을 알 수 있는가?** `alerts.json` 에서 출처 `192.168.0.202` 의 경보를 **4요소**(출발지·
> 시그니처·시각·심각도)로 보는 법, 그리고 발판(침투 성공)이 있으면 우선순위를 **P1**로
> 끌어올리는 판단.
>
> **결과 해석.** 정상: 출처 `192.168.0.202` 의 다단계 경보가 보이고 P1 으로 판정됨. 비정상:
...
> 한 캠페인에 엮이는지 확인한다(§4).
>
> **무엇을 알 수 있는가?** Wazuh `alerts.json` 의 그룹에 출처 `192.168.0.202` 가 (다단계)
> 수렴함. el34 가 SNAT 를 안 하므로 출처가 전 계층에 보존된다는 사실.
>
> **결과 해석.** 정상: alerts.json 에 출처 `192.168.0.202` 가 `ids` 등 다그룹으로 수렴. 비정상:
