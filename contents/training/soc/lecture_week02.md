...
| 시간        | 내용                                                                  | 유형      |
|-------------|-----------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 왜 "스캔 → 대입"을 한 흐름으로 보는가 (공격 단계 모델)          | 강의      |
| 0:20–0:50   | 이론 — 포트 스캔 유형(SYN/connect) + Suricata eve.json 구조           | 강의      |
| 0:50–1:00   | 휴식                                                                  | —         |
| 1:00–1:35   | 이론 — sshd 인증 로그 메시지 + 무차별 대입 vs 정상 구분(임계 기반)     | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — auth.log 접근 점검 + 스캔/대입 재현                        | 실습      |
| 2:00–2:30   | 실습 3, 4 — 실패 빈도 분석 + 표적 계정 분포 분석                       | 실습      |
| 2:30–2:40   | 휴식                                                                  | —         |
| 2:40–3:10   | 실습 5, 6 — Accepted(침투) 추적 + 스캔↔인증 출발지 상관               | 실습      |
| 3:10–3:30   | 실습 7, 8 — 사건 타임라인 재구성 + 분석 보고서 종합                    | 실습      |
...
```mermaid
graph TD
    ATK["공격자<br/>el34-attacker 10.20.30.202"]
    SCAN["① 포트 스캔(SYN)"]
    BRUTE["② SSH 무차별 대입"]
...
분석가가 이 한 줄에서 읽는 것은 세 가지다 — **누가**(`src_ip` = 10.20.30.202, 공격자),
...
**해석**: 출력에 `10.20.30.202` 가 여러 번 찍히면, 그 IP 가 스캔의 출발지다. 이 출발지를
...
el34 에서 인증 분석의 원천은 **호스트(`ssh ccc@192.168.0.151`)의 `/var/log/auth.log`** 다.
...
> **이 데모의 출발지에 대한 정직한 설명.** 본 실습에서 스캔은 attacker 컨테이너
> (10.20.30.202)가 일으키고, 무차별 대입은 호스트 자기 자신(127.0.0.1 localhost)에서
> 재현한다. 그래서 데모 화면에서는 두 출발지가 다르게 보인다. **실전에서는 스캔과 대입이
> 같은 외부 IP 에서 오므로 출발지가 일치하며**, 분석 기법(같은 IP 로 묶기)은 데모든 실전이든
> 동일하다. el34 가 외부 출처 IP 를 보존하기 때문에 실전 상관이 성립한다.
...
명령은 모두 el34 호스트(`ssh ccc@192.168.0.151`)에서 실행한다. 인증 분석은 호스트
