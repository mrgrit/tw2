...
> 출발지 주소를 바꾸지 않는다(SNAT 안 함). 그래서 외부 공격자(192.168.0.202)나 내부 발판
> (10.20.30.202)의 출발지 IP 가 로그·경보에 **그대로 보존**되고, "같은 출발지의 반복" 을
...
> **결과 해석.** `alerts.json` 의 ids 경보에 출발지 **10.20.30.202** 가 보이면 정상(출발지 보존
