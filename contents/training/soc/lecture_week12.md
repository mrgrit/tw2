...
UEBA 의 발상을 실제 명령으로 옮긴다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호
1)에서 실행한다. 인증 로그는 호스트의 `/var/log/auth.log` 를 직접 읽고, 파일/프로세스 행위는
`ssh ccc@10.20.32.80` 의 osquery 로 본다.
...
```bash
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT path,size FROM file WHERE path="/tmp/socw12_exfil.tar.gz";'
```
...
```bash
ssh ccc@10.20.32.80 'sha256sum /tmp/socw12_exfil.tar.gz; stat -c "%y %s" /tmp/socw12_exfil.tar.gz'
```
...
> el34-web` osquery, 유출 채널 격리는 `ssh ccc@10.20.31.2` 의 Suricata 로컬 룰이다. 만든
