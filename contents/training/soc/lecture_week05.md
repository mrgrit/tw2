...
> **el34 환경 주의.** 본 주차의 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)
...
top 출발지에서 `10.20.30.202`(el34 의 내부 공격자 컨테이너) 가 압도적이라면:

- 한 룰이 폭주의 대부분 → 그 룰을 의심한다. **헬스체크 봇의 반복 404** 라면 오탐, **공격자의
  스캔** 이라면 진짜.
- 한 출발지가 폭주 → `10.20.30.202` 한 곳에서 집중적으로 들어온다. el34 는 fw 가 SNAT 하지
...
- **왜 하는가?** 분석할 폭주를 직접 만든다. el34 의 attacker 컨테이너에서 포트 스캔 + 웹
...
- **결과 해석.** 한 룰/출발지가 폭주의 대부분이면 → 오탐 후보이거나 집중 공격. el34 는 출처
  보존이라 `10.20.30.202` 같은 공격자 IP 가 그대로 top 에 뜬다.
...
> **공통 수칙.** 명령은 모두 el34 호스트(`ssh ccc@192.168.0.151`)에서. 억제 룰은 `wazuh-logtest`
