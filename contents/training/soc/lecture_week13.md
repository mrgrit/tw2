...
```mermaid
graph TD
    RAW["원시 경보<br/>SQLi from 192.168.0.202<br/>(내가 본 것)"]
    INTEL["인텔 대조<br/>OpenCTI · MISP · CDB list<br/>(세상이 아는 것)"]
    ENR["enrichment 된 경보<br/>SQLi from 192.168.0.202<br/>+ tool=sqlmap=known-bad<br/>+ 평판 불량"]
...
echo '{"tool":"sqlmap","src_ip":"192.168.0.202"}' | sudo /var/ossec/bin/wazuh-logtest
...
```bash
# el34 호스트(ssh ccc@192.168.0.80)에서 — 인텔 소스가 살아 있는지
...
비정상) / 실전 활용. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서
...
>
> **이걸 하면 무엇을 알 수 있는가?**
> - 외부/외부 공격자 VM attacker 의 sqlmap UA + SQLi 가 경보로 남는다는 것
> - "도구 시그니처 = known-bad IOC" 라는 개념(이후 매칭 룰의 매칭 대상)
>
> **결과 해석**
> 정상: 공격이 재현되고 `attack done` 이 출력됨(이 sqlmap 흔적이 이후 미션의 원천 데이터다).
> 비정상: 재현이 안 되면 Host 헤더(`dvwa.el34.lab`)나 외부 공격자 VM 192.168.0.202 경로를 점검.
>
> **실전 활용**
