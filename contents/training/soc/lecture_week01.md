...
```mermaid
graph TD
    A1["스캔 경보<br/>출발지 10.20.30.202"]
    A2["인증 실패 경보<br/>출발지 10.20.30.202"]
    A3["웹 SQLi 경보<br/>출발지 10.20.30.202"]
...
연속으로 시도하는 것이다. 본 주차의 외부 공격자(insider 발판 `10.20.30.202`)가 그 예다.
...
> **참고 — 실습의 인증 벡터 생성 방식.** el34 실습에서 ②SSH 무차별 대입은 데모 편의상 호스트
> 자신(`127.0.0.1`)에 대해 실패를 발생시켜 `auth.log` 에 인증 실패 다발을 만든다. 실전에서는
> 외부 IP 에서 들어오지만, **분석 기법(빈도+계정 다양성으로 무차별 대입 판정)은 완전히 동일**
> 하다. 스캔(①)과 웹(③)은 실제 외부 공격자 출발지 `10.20.30.202` 로 기록된다.
...
두 소스 모두에서 `10.20.30.202` 가 출발지로 나오면, 스캔과 웹 공격이 **한 공격자**임이 확인된다.
...
> `10.20.30.202` 로 보존됨(el34 의 출처 보존). `403` 은 ModSec 이 차단했다는 뜻.
>
...
> (`10.20.30.202`)로 수렴함을 확인하고, "한 공격자의 단계적 캠페인"으로 종합하는 법.
>
> **결과 해석.** 정상: 두 소스 모두에서 `10.20.30.202` 가 출발지로 집계됨 → 스캔 + 웹(+ 인증)이
