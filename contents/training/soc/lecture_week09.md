...
> **임의로 보이는 번호 — 커스텀 탐지 룰 ID 규칙.** 본 주차부터 직접 만드는 탐지 룰엔 고유
> 번호가 붙는다(예: Suricata `sid:9509001`, Wazuh 룰 `id 100909`). 이 번호는 아무렇게나 정한
> 게 아니라 **내장 룰과 충돌하지 않도록 예약 대역**을 쓴 것이다 — Suricata 커스텀 sid 는
> `9000000+`, Wazuh 커스텀 룰 id 는 `100000+` 대역에서, 주차·용도별로 자리를 고정해 추적성을
> 준다. 그래서 로그에 이 번호가 보이면 "내장이 아니라 우리가 만든 룰"임을 바로 안다(soc 트랙
> 전 주차 동일 규칙이며, W15 에서 전체 네임스페이스를 종합한다).
...
ssh ccc@10.20.32.100 'tail -1000 /var/ossec/logs/alerts/alerts.json | jq -rc "select(.data.src_ip==\"192.168.0.202\")|[.timestamp,.rule.description]|@tsv" | tail -5'

# (식별-2) osquery 로 호스트에 남은 발판 — 어디까지 뚫렸나
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT username,uid FROM users WHERE username="socw9bd";'
```
...
ssh ccc@10.20.31.2 'sudo sed -i "/sid:9509001/d" /etc/suricata/rules/local.rules; sudo suricatasc -c reload-rules >/dev/null'
```
...
ssh ccc@10.20.32.80 sudo osqueryi --json 'SELECT username FROM users WHERE username="socw9bd";'
ssh ccc@10.20.32.80 'ls -la /tmp/socw9_shell.php'

# (제거-실행) 계정 삭제(userdel) + 웹쉘 파일 삭제(rm)
ssh ccc@10.20.32.80 'sudo id socw9bd >/dev/null 2>&1 && userdel -r socw9bd; rm -f /tmp/socw9_shell.php; echo eradicated'
```
...
호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `docker exec` 로 해당 컨테이너에 들어가
수행한다.
