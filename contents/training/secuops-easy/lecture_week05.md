...
> **이번 주 한 줄 요약**
>
> WAF 장비(`ssh ccc@10.20.32.80`)을 열어, ModSecurity의 탐지룰인 **SecRule**이 어떻게 생겼는지
> (변수 + 연산자 + 액션) 분해해 보고, 직접 **직접 SecRule을 만들어 적용**한다. 적용 전
...
4. SecRule을 직접 작성해, 그 **SecRule을 확인하고** 적용한다.
...
el34 호스트 브라우저에서 `ssh ccc@10.20.32.80`에 접속하면 7개 메뉴가 있습니다.
...
| 내 룰 파일 | `/etc/modsecurity/edu_rules.conf` | 장비가 만드는 룰이 쌓이는 파일 |
...
> **장비가 만드는 룰의 약속:** 직접 작성한 학생 룰은 `edu_rules.conf`에 **id 9000000 이상**으로
...
**② 적용(configtest+reload)**을 누르면: ① 장비가 이 룰을 `edu_rules.conf`에 추가하고 ②
...
> 잘못된 룰을 그냥 reload하면 **웹서버 전체가 죽습니다.** 그래서 장비은 적용 전 반드시 configtest를
...
`pass`(탐지/로그만)로 먼저 넣어 오탐을 관찰**한 뒤, 안전이 확인되면 `deny`로 바꿉니다. 장비의
...
보면, WAF는 **요청을 보낸 모든 출발지**를 기록합니다. 공격자(192.168.0.202)뿐 아니라, 어떤 내부
...
- 적용 전 **configtest**(apache2ctl configtest)로 문법을 검사해 웹서버를 보호한다(실패 시 자동 복원).
...
1. CRS 942(SQLi) 룰 하나를 **SecRule 분석기**에 넣고(또는 CRS 예시 보기에서 꺼내), 변수·연산자·
   액션을 표로 정리하세요.
2. 장비에서 **sqlmap 차단 SecRule**을 만들어 적용하고, 미리보기에 나온 SecRule을 그대로 적으세요.
