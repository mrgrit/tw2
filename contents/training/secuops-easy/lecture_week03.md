...
> **이번 주 한 줄 요약**
>
> IPS 콘솔(`192.168.136.145:8082`)을 열어, Suricata 탐지룰이 **어떻게 생겼는지**(헤더 + 옵션) 분해해
...
4. 콘솔의 폼으로 탐지룰을 만들어, 생성되는 **Suricata rule을 확인하고** 적용·삭제한다.
...
el34 호스트 브라우저에서 `http://192.168.136.145:8082/`에 접속하면 7개 메뉴가 있습니다.
...
(10.20.30.202)도 10.0.0.0/8 안**에 있습니다. 즉 공격자도 HOME_NET입니다. 그래서 우리 룰은
...
## 5. 탐지룰 만들기 — 콘솔이 Suricata rule을 만들어 주는 과정
...
**콘솔 폼에서:** action `alert`, 프로토콜 `http`, 출발지/목적지 `any`, 탐지 버퍼 `http.uri`,
...
**② 적용 + reload**를 누르면 — 콘솔이 이 룰을 `local.rules`에 추가하고 `suricatasc -c reload-rules`로
Suricata에 즉시 반영합니다. 그리고 **로딩 결과**를 보여 줍니다.

### 5.2 로딩 성공/실패를 즉시 확인 (초보자 필수)

적용 직후 콘솔은 "룰이 정상 로딩되었습니다. loaded 6 / failed 0"처럼 알려 줍니다. 만약 룰 문법이
...
- 콘솔이 폼을 **Suricata rule**로 변환해 local.rules에 적용하고 reload한다.
