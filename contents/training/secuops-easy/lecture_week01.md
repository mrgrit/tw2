...
**가장 중요한 약속이 하나 있습니다.** 이 특강에서는 실제 운영자처럼 각 보안장비에 **직접 SSH 로
접속해서, 진짜 명령을 손으로 칩니다.** GUI 대시보드를 클릭하는 대신, 방화벽 장비에서는 `nft` 를,
IPS 장비에서는 Suricata 룰을, WAF 장비에서는 ModSecurity 설정을 그 장비 위에서 직접 다룹니다.

> 즉 우리는 "화면의 버튼"이 아니라 **"장비에서 실제로 쓰는 명령"**을 배웁니다. 그래야 어떤 현장,
> 어떤 장비를 만나도 똑같이 일할 수 있습니다. (각 장비에 웹 대시보드도 있지만, 그건 첫 화면을
> 눈으로 익히는 참고용일 뿐 — 실제 작업과 확인은 장비에 접속해 명령으로 합니다.)

각 보안장비는 el34 안에서 실제로 돌아가는 컴퓨터이고, el34 호스트에서 SSH 로 바로 들어갈 수 있습니다.

| 장비 | 접속 (el34 호스트에서, 비번 `1`) | 다루는 것 |
|------|----------------------------------|-----------|
| 방화벽 | `ssh ccc@10.20.30.1` | nftables |
| IPS | `ssh ccc@10.20.31.2` | Suricata |
| WAF/웹서버 | `ssh ccc@10.20.32.80` | ModSecurity (Apache) |
| SIEM | `ssh ccc@10.20.32.100` | Wazuh 관제 |
...
5. 세 보안장비(방화벽/IPS/WAF)에 SSH로 접속해 실제 명령으로 상태를 확인한다.
...
이번 특강의 주인공인 세 보안장비는 el34 안에서 실제로 돌아가는 컴퓨터입니다. **el34 호스트에 먼저
접속(`ssh ccc@{{TARGET_IP}}`, 비번 `1`)한 뒤**, 거기서 각 장비로 SSH 로 바로 들어갑니다.

| 장비 | 접속 명령 | 다루는 것 | 배우는 주차 |
|------|-----------|-----------|------------|
| 방화벽 | `ssh ccc@10.20.30.1` | nftables | W2 |
| IPS | `ssh ccc@10.20.31.2` | Suricata | W3~W4 |
| WAF/웹서버 | `ssh ccc@10.20.32.80` | ModSecurity (Apache) | W5~W6 |
| SIEM | `ssh ccc@10.20.32.100` | Wazuh 관제 | 전 주차 |

접속 비밀번호는 모두 `1` 이고, 장비 관리 명령(예: `nft`, `apache2ctl`)은 `sudo` 를 붙입니다. 장비에
들어가면 그 장비의 상태를 진짜 명령으로 확인합니다 — 예를 들어 방화벽에서는 `ip -br addr`(인터페이스)와
`sudo nft list ruleset`(룰), IPS 에서는 `pgrep -af suricata`(가동)와 `tail /var/log/suricata/eve.json`
(이벤트), WAF 에서는 `sudo apache2ctl -M`(모듈)과 `grep SecRuleEngine`(모드)을 봅니다.

> **철학.** 우리는 화면의 버튼이 아니라 **장비에서 실제로 쓰는 명령**을 배웁니다. 그래야 대시보드가
> 없는 어떤 현장에서도 똑같이 일할 수 있습니다. (각 장비에 웹 대시보드도 있지만 참고용일 뿐입니다.)
...
- 우리는 세 장비에 각각 SSH(ssh ccc@10.20.30.1 / .31.2 / .32.80)로 접속해 실제 명령으로 다룬다.

## 다음 주 예고 (W2 — 방화벽)

다음 주에는 방화벽 장비(`ssh ccc@10.20.30.1`)에 접속해, 실제로 nft 룰을 만들어 봅니다. 악성 IP를 차단하고,
