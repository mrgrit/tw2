...
| 시간        | 내용                                                                          | 유형      |
|-------------|-------------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 왜 컨테이너 네트워크를 따로 배우는가 (한 호스트의 수십 컨테이너 · 측면 이동) | 강의      |
| 0:20–0:55   | 이론 — docker 네트워크와 분리(segmentation): 같은 네트워크만 직접 통신          | 강의      |
| 0:55–1:05   | 휴식                                                                          | —         |
| 1:05–1:35   | 이론 — el34 4-tier(ext/pipe/dmz/int) + 다리 컨테이너(web) + 노출 포트          | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 대상 확인 + docker 네트워크 목록(분리 구조)                        | 실습      |
| 2:00–2:30   | 실습 3, 4 — el34-web 네트워크 소속(dmz/int 다리) + 노출 포트 점검              | 실습      |
| 2:30–2:40   | 휴식                                                                          | —         |
| 2:40–3:10   | 실습 5, 6 — 분리(segmentation)의 의미 + 네트워크 위협(측면 이동)               | 실습      |
| 3:10–3:30   | 실습 7, 8 — 네트워크 방어(분리/최소노출) + 네트워크 보안 보고서                | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W08 — 중간 점검) 예고                        | 정리      |
...
수집기 등 여러 컨테이너가 **서로 통신**하며 이뤄진다. el34 만 해도 단일 호스트(192.168.0.151) 위에서
