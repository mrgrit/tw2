...
1. el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker ps` 로 **실행 중인 컨테이너를 식별·집계**하고,
...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 왜 컨테이너 보안을 별도로 배우는가 (VM 과의 격리 차이, 실 사례 3건)  | 강의      |
| 0:20–0:55   | 이론 — 컨테이너 vs VM 의 내부 구조 + 공유 커널의 의미                       | 강의      |
| 0:55–1:05   | 휴식                                                                       | —         |
| 1:05–1:35   | 이론 — 컨테이너 보안 4 계층(NIST 800-190) + CIS Docker Benchmark 소개       | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — el34 호스트 접속 + docker 가용 점검 + 컨테이너 식별             | 실습      |
| 2:00–2:30   | 실습 3, 4 — 컨테이너 vs VM 정리 + 보안 4 계층 정리                          | 실습      |
| 2:30–2:40   | 휴식                                                                       | —         |
| 2:40–3:10   | 실습 5, 6 — 공유 커널 위험 정리 + **docker inspect 기본 점검(root 갭 발견)** | 실습      |
| 3:10–3:30   | 실습 7, 8 — 방어 개요(최소권한) + 개론 보고서 작성                          | 실습      |
| 3:30–3:40   | 정리 + 과제 안내 + 다음 주차(W02 — 이미지 보안) 예고                        | 정리      |
...
el34 는 단일 VM(192.168.0.151) 위에서 docker 로 **41 개 컨테이너**가 돈다(web/ips/siem/fw/attacker/
...
```mermaid
graph TD
    SSH["el34 호스트 접속<br/>ssh ccc@192.168.0.151"]
...
el34 의 보안 컨테이너(el34-web 등)는 모두 단일 호스트 VM(192.168.0.151)의 **하나의 커널**을 공유한다.
...
본 주차의 핵심 점검 명령을 한 번에 정리한다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호
