...
본 주차의 모든 스캔은 **el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에 설치된 Trivy** 로 한다.
...
```bash
/usr/local/bin/trivy --version | head -1
```
...
> **점검 경로 정리.** el34 의 모든 이미지·컨테이너는 타깃 VM(192.168.0.80) 한 대 위에 있다. 점검자는
...
본 주차의 스캔은 모두 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 **`/usr/local/bin/trivy`**
...
```bash
/usr/local/bin/trivy image --scanners vuln --severity HIGH,CRITICAL --no-progress el34-web:latest | grep -iE 'CVE-' | head -3
```
...
```bash
for img in el34-web el34-ips; do echo -n "$img "; /usr/local/bin/trivy image --scanners vuln --severity HIGH,CRITICAL --no-progress "$img:latest" | grep -iE 'Total:'; done
```
