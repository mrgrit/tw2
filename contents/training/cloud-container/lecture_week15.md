...
이번 시험의 대상은 **el34-web 한 컨테이너** 다. el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)
...
기말의 점검은 모두 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker`/`trivy` CLI 로
...
> **시험 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker`/`trivy`
