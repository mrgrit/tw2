...
| 시간        | 내용                                                                          | 유형      |
|-------------|-------------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 오케스트레이션이란 무엇인가 (4계층의 마지막 · 개별 vs 스택 전체 강제)   | 강의      |
| 0:20–0:45   | 이론 — 운영 일관성(restart)과 복원력 · Docker Compose = IaC                    | 강의      |
| 0:45–0:55   | 휴식                                                                          | —         |
| 0:55–1:25   | 이론 — Kubernetes Pod 보안 5통제 + 오케스트레이션 위협(컨트롤플레인)           | 강의/토론 |
| 1:25–1:50   | 실습 — 스택 확인 + 운영 일관성(restart) 점검 (미션 1–2)                        | 실습      |
| 1:50–2:15   | 실습 — Compose 보안 + Kubernetes Pod 보안 정리 (미션 3–4)                      | 실습      |
| 2:15–2:25   | 휴식                                                                          | —         |
| 2:25–2:45   | 실습 — 오케스트레이션 위협 + 방어 (미션 5–6)                                   | 실습      |
| 2:45–2:55   | 실습 — 오케스트레이션 보안 보고서 (미션 7)                                     | 실습      |
| 2:55–3:00   | 정리 + 채점 기준 안내 + 다음 주차 예고                                         | 정리      |
...
el34 는 단일 호스트(192.168.0.151)에서 41개 컨테이너가 도는 **Docker Compose 스택**이다.
...
el34 스택의 재시작 정책 일관성은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 다음과 같이 훑는다
(미션 2).
...
본 주차의 직접 점검은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker` CLI 로 수행하며,
...
> **실습 진행 원칙.** 직접 점검(미션 1·2)은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker` CLI 로
