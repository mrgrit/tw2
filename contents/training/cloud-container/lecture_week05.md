...
| 시간        | 내용                                                                          | 유형      |
|-------------|-------------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 격리는 어떻게 "만들어지는가" (권한 점검 W04 → 격리 메커니즘 W05)        | 강의      |
| 0:20–0:55   | 이론 — namespace 6 종(pid/net/mnt/ipc/uts/user) 각각이 무엇을 분리하나         | 강의      |
| 0:55–1:05   | 휴식                                                                          | —         |
| 1:05–1:35   | 이론 — cgroups(자원 제한) + 격리의 근본 한계(공유 커널 → escape)              | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 대상 확인 + `/proc/1/ns/` namespace 목록 읽기                      | 실습      |
| 2:00–2:30   | 실습 3, 4 — pid namespace 격리(`PidMode`) + net namespace(`el34-dmz`)          | 실습      |
| 2:30–2:40   | 휴식                                                                          | —         |
| 2:40–3:10   | 실습 5, 6 — cgroups 자원 한도(`Memory`/`NanoCpus`) + 격리의 한계 정리          | 실습      |
| 3:10–3:30   | 실습 7, 8 — 격리 강화 방어(user ns/seccomp/AppArmor) + 격리 보고서             | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W06 — 시크릿 관리) 예고                      | 정리      |
