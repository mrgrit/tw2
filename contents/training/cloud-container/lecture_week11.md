...
> **el34 에서의 레지스트리 관점.** el34 의 모든 이미지·컨테이너는 타깃 VM(192.168.0.80) 한 대 위에
...
el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 이미지 목록은 다음과 같이 본다.
...
> **점검 경로.** el34 의 모든 이미지는 타깃 VM(192.168.0.80, target_vm: web) 한 대 위에 보관돼 있다.
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `docker` CLI 로
