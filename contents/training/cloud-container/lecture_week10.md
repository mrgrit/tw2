...
3. el34 의 **Wazuh**(siem 컨테이너)가 컨테이너 호스트의 로그·FIM·행위를 수집해 비정상을 탐지함을
   `docker exec el34-siem` 로 `/var/ossec/logs/alerts/alerts.json` 적재로 확인한다.
...
| 시간        | 내용                                                                            | 유형      |
|-------------|---------------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 정적 점검만으로는 부족한가 (배포 시점 vs 실행 중 행위)                 | 강의      |
| 0:25–0:55   | 이론 — 런타임 위협 탐지가 보는 세 신호 (프로세스·파일·아웃바운드)                | 강의      |
| 0:55–1:05   | 휴식                                                                            | —         |
| 1:05–1:35   | 이론 — el34 의 탐지 소스: Wazuh + 호스트 Sysmon(eBPF) + Falco                    | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 대상(siem) 확인 + 정적 점검의 한계 정리                              | 실습      |
| 2:00–2:30   | 실습 3, 4 — Wazuh 모니터링 확인 + 호스트 Sysmon 의 컨테이너 가시성              | 실습      |
| 2:30–2:40   | 휴식                                                                            | —         |
| 2:40–3:10   | 실습 5, 6 — Falco(컨테이너 런타임 탐지) + 탐지 vs 예방                           | 실습      |
| 3:10–3:30   | 실습 7, 8 — 런타임 탐지 운영(방어) + 런타임 위협 탐지 보고서                     | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W11 — 레지스트리·이미지 신뢰) 예고            | 정리      |
...
```mermaid
graph TD
    HK["el34 호스트 커널 (192.168.0.151)<br/>모든 컨테이너가 공유하는 단일 커널"]
...
```bash
docker exec el34-siem sh -c 'tail -1 /var/ossec/logs/alerts/alerts.json 2>/dev/null | head -c 60; echo; echo wazuh_active'
```

- `docker exec el34-siem` — el34 의 siem(Wazuh) 컨테이너 안에서 명령을 실행한다. el34 의 모든 점검은
  호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec` 로 인가된 컨테이너에만 들어간다.
...
```mermaid
graph TD
    HOST["el34 호스트 커널 (192.168.0.151)<br/>root · 단일 공유 커널"]
...
- `docker exec el34-web sh -c "..."` — el34-web 컨테이너 안에서 한 줄 명령을 실행한다(=컨테이너 내부
...
> (192.168.0.151)** 에 설치돼 있다. 그 이유와 설치 과정, EventID(ProcessCreate 1 / NetworkConnect 3
...
| 무엇을 | 명령 | 무엇을 보나 |
|--------|------|-------------|
| 대상(SIEM) 확인 | `docker exec el34-siem sh -c "hostname; echo target_ok"` | 런타임 탐지 원천(Wazuh)이 가동 중인가(`target_ok`) |
| 정적 점검의 한계 | (개념 정리) | 정적=배포 시점 설정 vs 런타임=실행 중 행위(`static_vs_runtime`) |
| Wazuh 모니터링 | `docker exec el34-siem sh -c 'tail -1 /var/ossec/logs/alerts/alerts.json ...'` | Wazuh alert 적재(로그·FIM·행위 탐지, `wazuh_active`) |
| 호스트 Sysmon 가시성 | `docker exec el34-web sh -c "...; echo behavior_generated"` | 컨테이너 내 행위 발생(호스트 Sysmon 이 관측 가능, `behavior_generated`) |
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker exec` 로 인가된
...
> 확인한다.
>
> **무엇을 알 수 있는가?** `docker exec el34-siem` 으로 siem 컨테이너의 가동 상태. 정적 점검이
