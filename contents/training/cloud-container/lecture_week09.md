...
> 통제하는 "관제탑"이고, 그 데몬을 떠받치는 **호스트(el34, 192.168.0.151)** 가 가장 밑바닥의 토대다.
...
| 시간        | 내용                                                                            | 유형      |
|-------------|---------------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 왜 데몬·호스트를 따로 점검하는가 (컨테이너 한 칸 vs 데몬·호스트 토대)      | 강의      |
| 0:20–0:55   | 이론 — CIS Docker Benchmark 6 절 구조 + 본 주차의 점검 범위(1·2·3·5)             | 강의      |
| 0:55–1:05   | 휴식                                                                            | —         |
| 1:05–1:35   | 이론 — docker.sock(CIS 3.15) + daemon.json(icc·userns-remap·live-restore)        | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 점검 환경 확인 + docker.sock 권한 점검 (CIS 3.15)                    | 실습      |
| 2:00–2:30   | 실습 3, 4 — daemon.json 점검 + healthcheck 점검 (CIS 5.26 갭 발견)               | 실습      |
| 2:30–2:40   | 휴식                                                                            | —         |
| 2:40–3:10   | 실습 5, 6 — restart policy 점검 + CIS 자동화(docker-bench)                       | 실습      |
| 3:10–3:30   | 실습 7, 8 — CIS Docker 방어 종합 + CIS Docker 보고서                             | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W10 — 런타임 위협 탐지) 예고                   | 정리      |
...
그리고 그 관제탑이 서 있는 **땅 자체가 호스트(el34, 192.168.0.151)** 다. 호스트의 커널·파일시스템·감사
...
(el34, 192.168.0.151)** 위에 올라 있다(W01 §2.2 의 41개 컨테이너가 모두 이 한 데몬·한 호스트 위에서
돈다). 따라서 데몬이나 호스트에 갭이 있으면, 그 위에 아무리 잘 강화한 컨테이너 41개를 올려도 **한꺼번에**
위험해진다.

```mermaid
graph TD
    HOST["호스트 (el34, 192.168.0.151)<br/>커널 · 파일시스템 · 감사 — CIS 1번 절"]
...
el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 docker.sock 의 소유·권한을 다음과 같이 점검한다
(미션 2).
...
본 주차의 점검은 모두 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker` CLI 와 파일 점검
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker` CLI 와 파일
