...
> 호스트(`ssh ccc@192.168.0.80`)에서 `docker inspect` 로 `el34-web` 의 환경변수를 직접 덤프해, 거기
...
# → 비밀은 '구워 넣으면' 회수 불가. 런타임에 외부에서 주입해야
```
...
# → 핵심: 이미지/env 가 아니라 런타임 외부 주입 + 메모리에서만 사용
```
...
# → 비밀은 '안 두기'가 최선, 그래도 새는지 자동 탐지로 이중 안전망
```
...
echo "5) 감사: 비밀 접근 로깅"
# → 노출된 SSH_PASS 같은 기존 비밀은 즉시 rotation
```
