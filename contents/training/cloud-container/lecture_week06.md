...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 비밀이란 무엇이며 왜 따로 관리하나 (노출 = 즉시 침해, 실 사례)       | 강의      |
| 0:20–0:55   | 이론 — env 가 새는 세 경로(inspect·/proc·자식 상속) + 이미지 레이어 잔존    | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — 올바른 시크릿 주입(매니저·tmpfs·런타임 주입) + rotation              | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 대상 확인(el34-web) + 환경변수 덤프(docker inspect)             | 실습      |
| 2:00–2:30   | 실습 3, 4 — **비밀 탐지(SSH_PASS, gap=secret_in_env)** + 노출 경로 정리     | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 5, 6 — 올바른 시크릿 주입(Vault 등) + 비밀 탐지 자동화(trivy/gitleaks) | 실습      |
| 3:10–3:30   | 실습 7, 8 — 방어 종합(rotation) + 시크릿 관리 보고서 작성                   | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W07 — 컨테이너 네트워크 보안) 예고        | 정리      |
