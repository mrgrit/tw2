...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 런타임 보안이란 무엇인가 (이미지 vs 런타임, 같은 이미지 다른 위험)   | 강의      |
| 0:20–0:55   | 이론 — 특권 컨테이너와 격리 무력화 (CIS Docker 5.4) + 실 사례               | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — Linux capability(CapAdd/CapDrop)와 root 실행 (CIS 5.3 · 4.1)         | 강의/토론 |
| 1:35–2:00   | 실습 — 대상 확인 + 특권 점검 (미션 1–2)                                     | 실습      |
| 2:00–2:30   | 실습 — capability 점검 + 실행 사용자 점검 (미션 3–4, **두 갭 발견**)        | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 — 읽기전용 루트FS·no-new-privileges + 영향 정리 (미션 5–6)             | 실습      |
| 3:10–3:30   | 실습 — 최소권한 방어 + 런타임 보안 보고서 (미션 7–8)                        | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W05 — namespace·cgroups 격리) 예고        | 정리      |
...
el34 는 한 호스트(192.168.0.151)에 **41개 컨테이너**가 같은 커널을 공유하며 돈다(W01 §3.3). 그래서 한
...
(`ssh ccc@192.168.0.151`)에서 다음과 같이 점검한다(미션 2).
...
- 셋째 줄은 `docker exec el34-web id -u` 로 컨테이너 안의 실제 uid 를 찍어, 그것이 `0`(root)이면
...
| 실행 사용자 (CIS 4.1) | `... '{{.Config.User}}'` + `docker exec el34-web id -u` | User 빈값 + uid 0 = root = **갭** |
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker` CLI 로 수행한다.
