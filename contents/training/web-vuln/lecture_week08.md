...
방식으로 한다. 본 시험의 명령들이 화면이 아니라 `nc` 로 API 를 직접 두드리는 이유다.
...
호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `ssh att@192.168.0.202`(점검) 또는 `docker exec
el34-web`(방어 측 WAF 확인)으로 실행한다. 점검의 주력 도구는 **`nc`**(raw HTTP) 다.

> **용어 — nc.** `echo -en` 으로 HTTP 요청 라인·헤더를 손수 써서 `nc` 로 웹에 보내는 방식이다. 요청 라인
> `<메서드> /경로 HTTP/1.0` 으로 메서드(예: `OPTIONS`)를, `헤더: 값` 으로 헤더(예: `Host`)를, 빈 줄 뒤에
> 본문(POST 데이터)을 붙인다. `| head -1 | grep -oE '[0-9]{3}'` 로 **응답 코드만** 뽑는 관용구는 도달성·차단 여부를 빠르게 볼 때 쓴다.

### 4.1 정찰 (HTTP 메서드 · 정보 노출)

```bash
# 허용 메서드 열거 + /ftp 정보 노출
echo -en 'OPTIONS / HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | grep -iE '^Allow|^HTTP' | head -2
echo -en "GET /ftp HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n" | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}'
```
...
```bash
echo "sqli=$(echo -en 'GET /?id=1%27%20UNION%20SELECT%201-- HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}')"
echo -en 'GET /?q=<script>alert(1)</script> HTTP/1.0\r\nHost: dvwa.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}'
```
