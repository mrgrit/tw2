...
2. el34 의 `외부 공격자 VM 192.168.0.202` 컨테이너에서 `whatweb`·`nikto`·`ffuf`(W02 도구) 로 표적 vhost
   (`dvwa.el34.lab` / `juice.el34.lab`) 의 **robots.txt · 정보 노출 · 백업 파일 · VCS 디렉토리 · 에러 메시지**를 빠짐없이 수집한다.
...
| **ffuf** | Fuzz Faster U Fool | 워드리스트로 경로·파일명을 대량 대입해 열거하는 웹 퍼저 | 이름 목록으로 문을 빠르게 두드려 봄 |
...
> 배웠다**. W03 은 그 도구들을 **정보 노출원별로 겨냥해** 다시 쓰는 단계다 — 도달·robots 는 `whatweb`,
> 정보 노출은 `nikto`, 백업·VCS 열거는 소규모 커스텀 워드리스트로 `ffuf` 를 돌려 항목별로 정밀하게 점검한다.
...
### 1.3 el34 에서의 정보 수집 — 외부 공격자 VM 192.168.0.202에서 whatweb·nikto·ffuf 로
...
```mermaid
graph TD
    ATK["외부 공격자 VM<br/>192.168.0.202<br/>whatweb · nikto · ffuf"]
...
`ssh att@192.168.0.202 ...` 형태로 실행한다. 점검 도구는 노출원별로 겨냥해 쓴다 — 도달·robots 확인은
`whatweb`(핑거프린팅), 정보 노출 점검은 `nikto`(취약점 스캐너), 백업·VCS 파일 열거는 `ffuf`(웹 퍼저) 로 한다(모두 W02 에서 배운 도구).
...
- 표적 서버에 부하를 주는 대규모 디렉토리 무차별 탐색(ffuf 대량 fuzzing) 은 W02 에서 다뤘고, 여기서는
  알려진 경로(robots/백업/`.git`) 를 겨냥한 **소규모 커스텀 워드리스트** 열거에 집중한다.
...
**el34 에서 어떻게 확인.** attacker 에서 `whatweb` 으로 표적 vhost 의 `/robots.txt` 를 가져온다. whatweb 의
`robots_txt` 플러그인이 `Disallow:` 경로를 그대로 파싱해 준다.

```bash
whatweb -a1 http://juice.el34.lab/robots.txt
```

**해석.** juice 는 `[200 OK]` 와 함께 `robots_txt[/ftp]` 를 노출한다 — 이 `/ftp` 가 다음 점검 후보(숨긴 경로) 다.
`[403 Forbidden]`(dvwa) 이면 WAF 가 robots 수집 자체를 막은 것이다.
...
화면엔 안 보이니까"라고 생각하지만, 페이지 소스(또는 `nikto` 같은 스캐너) 로는 누구나 읽을 수 있다.

**el34 에서 어떻게 확인.** 취약점 스캐너 `nikto` 로 서버 배너·비표준 헤더·robots 항목·주석/메타 등
정보 노출을 한 번에 점검한다.

```bash
nikto -h http://juice.el34.lab -maxtime 30s 2>&1 | head -20
```

**해석.** nikto 가 `Server: Apache/2.4.52 (Ubuntu)`·`Uncommon header 'x-recruiting'`·robots 항목 등을
보고하면 정보 노출이 식별된 것이다. 스캐너 결과는 오탐이 섞이므로 사람이 재현·검증해 실마리로 기록한다.
...
**el34 에서 어떻게 확인.** 흔한 백업 이름으로 커스텀 워드리스트를 만들어 `ffuf` 로 열거한다.

```bash
printf 'index.php.bak\nindex.php~\nconfig.php.old\nbackup.zip\n.env\n' > /tmp/wvbak.txt
ffuf -u http://dvwa.el34.lab/FUZZ -w /tmp/wvbak.txt -mc 200 -s   # 노출(200): dvwa 는 WAF 로 없음
ffuf -u http://dvwa.el34.lab/FUZZ -w /tmp/wvbak.txt -mc 403 -s   # 차단(403): 전부 WAF 차단
rm -f /tmp/wvbak.txt
```

**해석.** `-mc 200` 목록에 뜬 파일이 곧 노출이다(소스/설정/자격증명 유출). dvwa 는 WAF(ModSecurity) 가
이 요청들을 전부 403 으로 막아 `-mc 200` 은 비고 `-mc 403` 에 전부 뜬다. 실환경에서 하나라도 200 이 보이면 즉시 내용을 확인한다.
...
**el34 에서 어떻게 확인.** VCS·메타 파일명으로 워드리스트를 만들어 `ffuf` 로 열거한다.

```bash
printf '.git/config\n.git/HEAD\n.svn/entries\n.DS_Store\n' > /tmp/wvvcs.txt
ffuf -u http://dvwa.el34.lab/FUZZ -w /tmp/wvvcs.txt -mc 200 -s   # 노출(200)
ffuf -u http://dvwa.el34.lab/FUZZ -w /tmp/wvvcs.txt -mc 403 -s   # 차단(403)
rm -f /tmp/wvvcs.txt
```

**해석.** `-mc 200` 에 `.git/config` 이 뜨면 사실상 **소스 전체 유출**로 간주하는 최고 등급 발견이다.
dvwa 는 WAF 가 전부 403 으로 막아 `-mc 200` 은 비고 `-mc 403` 에 전부 뜬다. `.DS_Store` 가 200 이면
...
**el34 에서 어떻게 확인.** W02 의 `whatweb -a1` 으로 응답 헤더의 서버·버전 단서와 기술 스택을 함께 본다.

```bash
whatweb -a1 http://dvwa.el34.lab/
```

**해석.** `HTTPServer[Ubuntu Linux][Apache/2.4.52 (Ubuntu)]` 처럼 버전이 그대로 노출되면 그 버전의 알려진
취약점을 다음 단계에서 조사한다. 에러로 내부 경로가 새면 파일 구조 파악의 단서가 된다(에러 기반 정보 노출의 심화는 W11 에서 다룬다).
...
비정상) / 실전 활용 — 으로 설명한다. 미션은 점검 → robots → 정보노출 → 백업 → VCS → 종합 → 방어 → 보고서 순서로,
...
### 미션 3 — 정보 노출 점검(nikto) (12점, recon)

> **왜 하는가?** 개발자가 흘리는 정보(HTML 주석·서버 배너·비표준 헤더·robots 항목) 가 노출되는지 스캐너로 확인한다.
>
> **무엇을 알 수 있는가?** `nikto` 가 `Server:` 버전 배너·`Uncommon header`·robots 항목 등 알려진 노출 패턴을
> 한 번에 훑어 준다는 사실. 화면엔 안 보여도 스캐너에는 잡힌다.
>
> **결과 해석.** 정상: nikto 가 `Uncommon header 'x-recruiting'`·robots `/ftp` 항목 등을 보고하면 정보 노출이
> 식별된 것. 핵심 깨달음 — "화면에 안 보임"은 "비공개"가 아니다(스캐너 결과는 사람이 재현·검증).
>
> **실전 활용.** 소스 리뷰·점검에서 자주 쓰는 항목. 서버 배너·주석에 남은 임시 자격증명이 그대로 침해로
> 이어진 실제 사례가 많다.
