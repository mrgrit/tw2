...
| **curl** | — | 명령줄에서 HTTP 요청을 보내는 기본 도구 | 손으로 직접 문을 두드려 응답을 듣는 일 |
...
> 자동 스캐너로 배웠다**. W03 은 그 자동 스캐너가 "시끄럽게" 한 번에 훑던 일을, 점검자가 `curl` 로 **항목별로
> 정밀하게·조용히** 다시 확인하는 단계다. 두 방식은 보완 관계다 — 스캐너로 빠르게 넓게, curl 로 정확하게 깊게.
...
화면엔 안 보이니까"라고 생각하지만, 페이지 소스(또는 `curl`) 로는 누구나 읽을 수 있다.
