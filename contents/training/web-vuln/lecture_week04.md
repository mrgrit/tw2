...
2. `el34-attacker` 컨테이너에서 juiceshop 로그인 API 에 **무차별 대입**(여러 약한 비밀번호를
...
> 환경(el34)** 안에서, 정해진 대상(`el34-attacker` → el34 내부 vhost `juice.el34.lab` /
...
| 시간        | 내용                                                                  | 유형      |
|-------------|-----------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 인증/세션이 가장 자주 뚫리는가 (OWASP A07, 실 사례 3건)       | 강의      |
| 0:25–0:55   | 이론 — 인증 4공격(무차별·스터핑·SQLi 우회·기본계정) + 세션 점검 시야     | 강의      |
| 0:55–1:05   | 휴식                                                                   | —         |
| 1:05–1:35   | 이론 — JWT 구조·약점 + 세션 쿠키 플래그 + el34 진입 경로                | 강의/토론 |
| 1:35–2:00   | 실습 1–2 — 로그인 API 점검 + 무차별 대입                                | 실습      |
| 2:00–2:30   | 실습 3–4 — SQLi 인증 우회 → JWT 획득 + JWT payload 분석                 | 실습      |
| 2:30–2:40   | 휴식                                                                   | —         |
| 2:40–3:10   | 실습 5–6 — 세션 쿠키 플래그 점검 + 로그인 남용 탐지(방어자 시선)         | 실습      |
| 3:10–3:30   | 실습 7–8 — 인증/세션 방어 정리 + 점검 보고서 작성                       | 실습      |
| 3:30–3:40   | 정리 + 과제 안내 + 다음 주차(W05 — SQL Injection 심화) 예고             | 정리      |
...
**el34 에서 어떻게.** 실습 5 에서 학생은 `curl -sI`(헤더만 요청)로 `dvwa.el34.lab` 의 응답
...
본 주차의 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서
`docker exec el34-attacker ...` 로 실행한다. attacker 컨테이너가 보내는 로그인 요청은 다음 경로를
탄다. 이 경로를 알아야 "내 요청이 어느 장비에 흔적을 남기는가"를 추적할 수 있다.

```mermaid
graph TD
    ATK["el34-attacker (내부 발판)<br/>출처 10.20.30.202<br/>curl → http://10.20.30.1 (Host: juice/dvwa)"]
...
로그인 폭주·SQLi 우회가 모두 출처 `10.20.30.202` 로 묶여 접근로그·WAF 로그에 남는다. 둘째, **vhost 별
...
> 공격자 `el34-attacker`(ext .202) → fw 게이트웨이(ext .1) → web(dmz .80) → 내부 앱(int .81 juice /
...
```mermaid
graph TD
    EVENT["한 점검자의 인증 공격<br/>출처 10.20.30.202"]
...
두 흔적 모두 같은 출처 IP `10.20.30.202` 를 가리키므로, 방어자는 이를 한 점검자의 한 인증 공격
...
> 판정한다.
>
> **무엇을 알 수 있는가?** `curl -sI`(헤더만)로 `dvwa.el34.lab` 의 `Set-Cookie` 를 보고 세션
