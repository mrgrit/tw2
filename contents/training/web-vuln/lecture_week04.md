...
**el34 에서 어떻게.** 실습 5 에서 학생은 `nc` raw HEAD 요청으로 `dvwa.el34.lab` 의 응답
...
```mermaid
graph TD
    ATK["외부 공격자 VM 192.168.0.202 (외부 공격자 VM)<br/>출처 192.168.0.202<br/>nc raw → 192.168.0.161:80 (Host: juice/dvwa)"]
...
> 판정한다.
>
> **무엇을 알 수 있는가?** `nc` raw HEAD 요청으로 `dvwa.el34.lab` 의 `Set-Cookie` 를 보고 세션
