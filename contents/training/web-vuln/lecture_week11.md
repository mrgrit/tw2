...
2. `curl` 로 JuiceShop 의 **응답 헤더**를 떠서 `Server:` 배너(웹서버 버전)와 `X-Recruiting` 같은
...
| 시간        | 내용                                                                  | 유형      |
|-------------|-----------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 정보 노출이란 무엇인가, 왜 "직접 침해 아님"인데도 위험한가      | 강의      |
| 0:20–0:50   | 이론 — 네 가지 노출 채널(배너·상세 에러·DB 에러·헤더)과 A05 자리매김   | 강의      |
| 0:50–1:00   | 휴식                                                                   | —         |
| 1:00–1:30   | 이론 — 정찰 가속 모델: 노출 한 조각이 다음 공격에 어떻게 쓰이나        | 강의/토론 |
| 1:30–2:00   | 실습 — 점검(도달성) + 배너 노출(Server 버전) (lab 1–2)                | 실습      |
| 2:00–2:30   | 실습 — 상세 에러(내부 경로) + DB 에러(엔진·주입 표면) (lab 3–4)        | 실습      |
| 2:30–2:40   | 휴식                                                                   | —         |
| 2:40–3:10   | 실습 — 헤더 누출 + 정찰 가속 영향 종합 (lab 5–6)                       | 실습      |
| 3:10–3:30   | 실습 — 정보 최소화 방어 + 정보 노출 보고서 (lab 7–8)                   | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W12 — API 보안) 예고                 | 정리      |
...
> 모두 헤더다. `curl -D -`(또는 `-I`) 로 헤더만 따로 떠서 볼 수 있다. 점검자는 본문보다 헤더에 노출된
...
점검자는 el34 의 점검 컨테이너 `el34-attacker`(출처 IP `10.20.30.202`)에서 fw 게이트웨이
(`10.20.30.1`)를 통해, HTTP `Host` 헤더로 표적 vhost(`juice.el34.lab`)를 지정해 점검한다.
...
> 이면 JuiceShop 으로 라우팅된다. 그래서 모든 점검 명령은 `curl -H 'Host: juice.el34.lab'` 형태로 표적을
> 명시한다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec el34-attacker`
> 로 실행한다.
...
```bash
docker exec el34-attacker sh -c "curl -sk -D - -o /dev/null -H 'Host: juice.el34.lab' http://10.20.30.1/ | grep -i 'server:'"
```
...
```bash
docker exec el34-attacker sh -c "curl -sk -H 'Host: juice.el34.lab' http://10.20.30.1/api/Nonexistent123 | grep -oE 'Unexpected path[^<]*' | head -1"
```
...
```bash
docker exec el34-attacker sh -c "curl -sk -H 'Host: juice.el34.lab' \"http://10.20.30.1/rest/products/search?q=test'\" | grep -oE 'SQLITE_ERROR[^<]*' | head -1"
```
...
```bash
docker exec el34-attacker sh -c "curl -sk -D - -o /dev/null -H 'Host: juice.el34.lab' http://10.20.30.1/ | grep -iE 'x-recruiting|x-powered|feature-policy'"
```
...
> **진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec
> el34-attacker` 로 실행한다. 각 미션은 독립적이며, **인가된 표적(`juice.el34.lab`) vhost 만** 점검한다.
> 신규 도구 설치는 없다 — `curl` 은 기본 탑재되어 있다. 합격 임계값은 0.7 이다.
...
> 경로(Host 헤더·게이트웨이 10.20.30.1)부터 점검한다.
>
...
> 나오는지 먼저 확인한다.
>
> **무엇을 알 수 있는가?** `curl -D -` 로 헤더를 떠서 `Server:` 값을 본다. `Apache/2.4.x` 가 보이면
