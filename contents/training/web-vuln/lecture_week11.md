...
2. `whatweb` 로 JuiceShop 을 핑거프린팅해 `Server:` 배너(웹서버 버전)와 `X-Recruiting` 같은
...
> 모두 헤더다. `whatweb` 같은 핑거프린팅 도구가 이 헤더를 파싱해 보여준다. 점검자는 본문보다 헤더에 노출된
...
```mermaid
graph TD
    ATK["점검자 외부 공격자 VM<br/>192.168.0.202<br/>whatweb / sqlmap (juice.el34.lab)"]
...
**el34 에서 어떻게.** whatweb 으로 JuiceShop 을 핑거프린팅하면 `Server:` 배너가 그대로 파싱된다.

```bash
whatweb -a1 --color=never http://juice.el34.lab/ | tr ',' '\n' | grep -i apache
```

- `whatweb` 는 응답을 분석해 상태코드·서버 배너·헤더를 요약하는 웹 핑거프린팅 도구다(raw 소켓 nc 와 달리
  노출을 곧바로 읽어준다). `tr ',' '\n' | grep -i apache` 로 서버 배너 필드(`Apache[...]`)만 추린다.
- **실측 노출**: `Apache[2.4.52]` · `HTTPServer[Ubuntu Linux][Apache/2.4.52 (Ubuntu)]` — 웹서버가 Apache 이고
  그 버전·OS 까지 드러난다. 이 한 줄이 "Apache 이 버전의 CVE 를 찾아라" 라는 공격자의 다음 행동을 부른다.
...
**el34 에서 어떻게.** whatweb 으로 존재하지 않는 API 경로를 호출해 에러 페이지 타이틀을 본다.

```bash
whatweb -a1 --color=never http://juice.el34.lab/api/Nonexistent123 | tr ',' '\n' | grep -i unexpected
```

- `/api/Nonexistent123` 은 일부러 없는 경로다. JuiceShop 은 이를 일반 404 로 처리하지 않고 500 과 함께
  `Error: Unexpected path: /api/Nonexistent123` 같은 상세 에러를 내주고, whatweb 은 이를 `Title[...]` 로 잡는다.
- `tr ',' '\n' | grep -i unexpected` 으로 그 에러 타이틀만 추출한다. 이 노출은 **라우팅 구조/내부 경로**가
  외부로 새어나가는 것을 보여준다 — 커스텀 에러 페이지로 일반 메시지만 보여줬어야 한다.
...
**el34 에서 어떻게.** sqlmap 으로 검색 파라미터(`q`)에 주입을 시도한다 — 앱이 원본 DB 에러를 노출해 엔진이 드러난다.

```bash
sqlmap -u "http://juice.el34.lab/rest/products/search?q=apple" --batch --level=2 --risk=2 --disable-coloring
```

- sqlmap 이 `q` 에 여러 페이로드를 던지면 JuiceShop 은 잘못된 입력에 `SQLITE_ERROR: near "'%'": syntax error`
  같은 원본 에러를 그대로 내주고, sqlmap 은 이를 근거로 `back-end DBMS: SQLite` 를 확정한다.
- `SQLITE_ERROR` 라는 문자열은 **엔진이 SQLite 임을 확정**하고, 페이로드가 질의에 섞여 오류를 냈다는 것은
  **이 검색 파라미터가 SQLi 표면**임을 입증한다. 사용자에게는 일반 메시지만, 원본 에러는 서버 로그로만
  남겼어야 한다(추출 `--tables`/`--dump` 은 W05 의 인가 범위).
...
**el34 에서 어떻게.** whatweb 의 UncommonHeaders 에서 비표준 헤더만 골라 본다.

```bash
whatweb -a1 --color=never http://juice.el34.lab/ | tr ',' '\n' | grep -iE 'recruiting|feature-policy|powered'
```

- **실측 노출**: `x-recruiting`(→ `/#/jobs`) · `feature-policy` — whatweb 이 `UncommonHeaders[...]` 로 모아
  보여주는 비표준 헤더다. JuiceShop 이 채용 페이지를 광고하려고 붙인 `X-Recruiting: /#/jobs` 는 숨은 경로를
  알려준다. 운영에 불필요한 헤더는 제거해 정보 노출을 줄여야 한다.
...
> **진행 원칙.** 모든 명령은 외부 공격자 VM(`ssh att@192.168.0.202`, 비밀번호 1)에서 실행한다.
> 각 미션은 독립적이며, **인가된 표적(`juice.el34.lab`) vhost 만** 점검한다.
> 도구는 whatweb(배너·에러·헤더 노출)·sqlmap(DB 에러 기반 노출) — att 에 탑재. 합격 임계값은 0.7 이다.
...
> 나오는지 먼저 확인한다.
>
> **무엇을 알 수 있는가?** `whatweb` 핑거프린팅으로 `Server:` 배너를 본다. `Apache[2.4.52]` 가 보이면
> 웹서버 종류·버전이 노출된 것 — 공격자가 해당 버전의 CVE 를 곧장 매핑할 수 있다.
>
> **결과 해석.** 정상(노출 확인): `Apache[2.4.52]`/`Apache/2.4.52 (Ubuntu)` 가 출력. 비정상: 배너가 안
> 보이면 whatweb 명령과 표적 URL 을 재확인한다. (방어가 잘 된 서버라면 `Server: Apache` 한 단어만
> 나오는데, el34 JuiceShop 은 학습용으로 버전을 노출한다.)
>
...
>
> **무엇을 알 수 있는가?** sqlmap 이 검색 파라미터(`q`)에 주입을 시도할 때 앱이 `SQLITE_ERROR ... syntax
> error` 원본 에러를 노출해, sqlmap 이 `back-end DBMS: SQLite` 를 확정하는지. `SQLITE_ERROR` 는 DB 엔진이
> SQLite 임을 확정하고, 페이로드가 질의에 섞여 오류를 냈다는 것은 이 지점이 SQLi 표면임을 입증한다.
>
> **결과 해석.** 정상(노출 확인): sqlmap 출력에 `back-end DBMS: SQLite`(및 q 주입 가능). 비정상: 판정이
> 안 나오면 검색 엔드포인트(`/rest/products/search?q=`)를 재확인한다.
>
...
>
> **결과 해석.** 정상(노출 확인): `x-recruiting`(또는 다른 비표준 헤더)이 UncommonHeaders 로 출력. 비정상:
> 빈 출력이면 whatweb 명령과 grep 패턴을 재확인한다.
>
