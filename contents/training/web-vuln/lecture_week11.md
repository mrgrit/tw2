...
```bash
curl -sk -D - -o /dev/null -H 'Host: juice.el34.lab' http://192.168.0.161/ | grep -i 'server:'
```
...
```bash
curl -sk -H 'Host: juice.el34.lab' http://192.168.0.161/api/Nonexistent123 | grep -oE 'Unexpected path[^<]*' | head -1
```
...
```bash
curl -sk -H 'Host: juice.el34.lab' \"http://192.168.0.161/rest/products/search?q=test'\" | grep -oE 'SQLITE_ERROR[^<]*' | head -1
```
...
```bash
curl -sk -D - -o /dev/null -H 'Host: juice.el34.lab' http://192.168.0.161/ | grep -iE 'x-recruiting|x-powered|feature-policy'
```
