...
| **curl** | — | 명령줄에서 HTTP 요청을 보내는 도구(브라우저의 수동 버전) | 손으로 주문서를 직접 쓰는 펜 |
...
### 0.5.5 curl — 손으로 쓰는 HTTP 주문서

브라우저는 주문서(요청)를 자동으로 만들어 보내준다. 그런데 점검자는 메서드를 PUT 으로 바꾸거나
헤더를 위조하는 등 **주문서를 마음대로 손으로 써야** 한다. 이때 쓰는 도구가 **curl** 이다.

**curl** 은 명령줄에서 HTTP 요청을 직접 보내고 응답을 그대로 보여주는 도구다. 브라우저가 가려주는
요청·응답의 원본을 날것 그대로 다룰 수 있어 점검의 기본기다. 이번 주에 반복해서 쓰는 옵션은 다음과
같다.

| curl 옵션 | 의미 | 점검에서의 쓰임 |
|-----------|------|----------------|
| `-X <메서드>` | 요청 메서드 지정 | `-X OPTIONS` / `-X PUT` / `-X TRACE` 로 위험 메서드 시도 |
| `-H '키: 값'` | 헤더 추가 | `-H 'Host: dvwa.el34.lab'` 로 vhost 지정, `-H 'X-Forwarded-For: 127.0.0.1'` 로 위조 |
| `-i` | 응답 헤더까지 출력 | 상태 라인과 `Allow` 헤더를 보려고 |
| `-I` | 헤더만(HEAD 요청) | 보안 헤더 존재/누락만 빠르게 확인 |
| `-s` | 진행 표시 숨김 | 출력을 깔끔하게 |
| `-o /dev/null` | 본문 버림 | HTML 본문이 필요 없고 코드만 볼 때 |
| `-w 'PUT=%{http_code}\n'` | 출력 포맷 지정 | 응답 코드만 깔끔히 뽑아 보기 |

> 이번 주의 모든 점검 명령은 이 curl 옵션들의 조합이다. 옵션의 의미만 알면 명령이 길어도 "메서드를
> 바꿔 보낸다 / 헤더를 위조해 보낸다 / 코드만 본다"의 단순한 행위임을 알 수 있다.
...
이번 주(W01)는 `curl` 로 HTTP 요청·응답을 **한 줄씩 손으로** 점검하는 기초를 익혔다. 메서드·상태
