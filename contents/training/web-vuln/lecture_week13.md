...
1. **nuclei** 가 무엇이며 "템플릿(YAML 서명)"으로 어떻게 동작하는지 설명하고, 외부 공격자 VM 192.168.0.202 에 템플릿이
...
**el34 사실.** 외부 공격자 VM 192.168.0.202 에 설치된 nuclei 는 **v3.3.5** 이며, 템플릿은 `http/` 하위에 약
...
```bash
nuclei -u http://juice.el34.lab \
  -t ~/nuclei-templates/http/technologies/tech-detect.yaml \
  -t ~/nuclei-templates/http/misconfiguration/http-missing-security-headers.yaml \
  -silent -nc
```

각 옵션의 뜻은 다음과 같다.

- `-u http://192.168.0.161` — 점검할 기본 URL(여기서는 fw 게이트웨이). 실제 어느 vhost 로 라우팅될지는
  `-H` 의 Host 헤더가 정한다.
...
## 2.5 템플릿 설치 — nuclei 는 엔진과 서명(템플릿)이 분리돼 있다

nuclei 의 **바이너리는 외부 공격자 VM 192.168.0.202 에 이미 탑재**(`/usr/local/bin/nuclei`)돼 있지만,
점검에 쓸 **템플릿(YAML 서명) 데이터는 따로 내려받아야** 한다. 안티바이러스로 치면 **엔진과 시그니처가
분리**돼 있어, 엔진만으로는 무엇을 찾을지 모르는 것과 같다.

### 2.5.1 202 에서는 직접 내려받는다

외부 공격자 VM 192.168.0.202 는 인터넷이 되므로, 표준 명령 한 줄로 GitHub 에서 최신 템플릿을 직접
내려받는다(첫 실행 시 자동으로도 받는다).

```bash
# 공격 VM(ssh att@192.168.0.202)에서 — 템플릿을 ~/nuclei-templates 로 내려받음
nuclei -update-templates -silent

# 확인 — http 템플릿 개수
find ~/nuclei-templates/http -name "*.yaml" | wc -l   # → 약 10,964
```

템플릿은 `~/nuclei-templates/`(즉 `/home/att/nuclei-templates/`) 아래에 받아지며, 이후 `-t ~/nuclei-templates/...`
로 특정 템플릿을 지목해 쓴다. 시간이 지나면 새 CVE 가 계속 추가되므로, 운영에서는 주기적으로
`nuclei -update-templates` 를 다시 돌려 **갱신**한다.

### 2.5.2 격리망이라면 — 오프라인 주입 패턴 (참고)

> **용어 — egress(이그레스) 차단.** 외부에서 내부로 **들어오는** 것이 ingress, 내부에서 외부로 **나가는**
> 것이 egress 다. **egress 차단**은 내부 호스트가 인터넷으로 나가는 통신을 막는 보안 통제로, 침해된 발판이
> 데이터를 빼돌리거나 추가 도구를 받는 것을 막으려 현실에서 흔히 쓴다.

현실의 침투 발판(insider foothold)은 종종 egress 가 막혀 있어 `nuclei -update-templates` 가 실패한다
(GitHub 도달 실패, 외부 `curl` 이 응답 코드 `000`). 그럴 때는 **nuclei 엔진은 오프라인에서도 동작**하므로,
인터넷이 되는 신뢰 경로(예: 관리 호스트)에서 `git clone https://github.com/projectdiscovery/nuclei-templates`
로 템플릿만 받아 발판으로 복사해 넣는다 — 안티바이러스에 시그니처만 따로 넣어 주는 것과 같은 표준
패턴이다. 본 실습의 202 는 인터넷이 되므로 이 우회는 필요 없지만, 격리망 현장을 만나면 이 패턴을 떠올린다.
...
```bash
dalfox url "http://dvwa.el34.lab/?q=test" --silence
```
...
> **실습 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`)에서 `ssh att@192.168.0.202`
...
>
> **무엇을 알 수 있는가?** `whatweb` 로 보낸 `Host: juice.el34.lab` 요청이 fw → web 경로를 거쳐 JuiceShop 에
> 닿아 응답 코드·기술 스택을 돌려주는지. 표적이 스캔 가능한 상태인지를 본다.
>
...
> 템플릿 경로(`~/nuclei-templates/...`)와 Host 헤더, 도달성(미션 1)을 재확인한다.
>
