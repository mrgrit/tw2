...
```bash
whatweb -H 'Host: dvwa.el34.lab' http://192.168.0.161/ 2>&1 | head -5
```
...
```bash
printf "admin\nlogin\nconfig\n" > /tmp/wv2.txt; ffuf -u http://192.168.0.161/FUZZ -w /tmp/wv2.txt -H "Host: dvwa.el34.lab" -mc 200,301,403 -s 2>&1 | head; rm -f /tmp/wv2.txt
```
...
```bash
curl -sI -H 'Host: dvwa.el34.lab' http://192.168.0.161/ | grep -iE 'Server|X-Powered'
```
...
> `ssh att@192.168.0.202`(스캔) 또는 `docker exec el34-web`(방어자 로그 확인)로
