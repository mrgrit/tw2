...
> **본 주차의 한 줄 요약**
>
> 지난 주(W01)에 학생은 `curl` 로 HTTP 메서드·헤더를 **한 줄씩 손으로** 점검했다. 이번
> 주에는 그 수작업을 자동화하는 **취약점 스캐너**(nikto)와 **핑거프린팅**(whatweb),
...
2. el34 의 `el34-attacker` 컨테이너에서 **nikto**(취약점 스캐너)를 `-maxtime` 으로 시간을
...
| 시간        | 내용                                                                   | 유형     |
|-------------|------------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — 수동 점검(W01)의 한계와 자동 스캐너의 등장 (능동 vs 수동)        | 강의     |
| 0:20–0:55   | 이론 — 3종 도구(nikto / whatweb / ffuf)의 역할·입출력·차이             | 강의     |
| 0:55–1:05   | 휴식                                                                   | —        |
| 1:05–1:35   | 이론 — 스캐너의 두 그림자: 오탐(false positive)과 시끄러움(탐지)        | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 도구 점검 + nikto 취약점 스캔                              | 실습     |
| 2:00–2:30   | 실습 3, 4 — whatweb 핑거프린팅 + ffuf 디렉터리 탐색                    | 실습     |
| 2:30–2:40   | 휴식                                                                   | —        |
| 2:40–3:10   | 실습 5, 6 — 스캐너 WAF 탐지(913) + 핑거프린팅 노출 정보 점검           | 실습     |
| 3:10–3:30   | 실습 7, 8 — 핑거프린팅 축소 방어 + 스캐너 점검 보고서                  | 실습     |
| 3:30–3:40   | 정리 + 다음 주차(W03 — 정보수집 WSTG-INFO) 예고                        | 정리     |
...
W01 에서 학생은 `curl -X OPTIONS`, `curl -sI` 처럼 **한 번에 한 가지**를 손으로 점검했다.
이 방식은 정확하고 학습에 좋지만, 실전의 웹 서비스는 점검할 표면이 매우 넓다 — 수십 개의
경로, 수백 가지의 알려진 취약 파일, 수십 개의 보안 헤더 조합. 이걸 사람이 일일이 `curl`
로 확인하려면 며칠이 걸린다. **취약점 스캐너**는 바로 이 반복을 자동화한다.

```mermaid
graph TD
    W01["W01 수동 점검<br/>curl 한 줄씩<br/>정확하지만 느림"]
...
> 대상(`el34-attacker` → el34 내부 vhost `dvwa.el34.lab` 등)에 한해서만 수행한다. 취약점
...
이번 주에 쓰는 세 도구는 모두 el34 의 **`el34-attacker` 컨테이너**(내부 발판, 출처 IP
`10.20.30.202`)에 설치되어 있다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호
1)에서 `docker exec el34-attacker ...` 형태로 실행한다. 세 도구는 **무엇을 입력으로 받아
무엇을 산출하는가**가 서로 다르다.

```mermaid
graph TD
    TARGET["대상 웹 서비스<br/>dvwa.el34.lab (10.20.30.1 경유)"]
...
**el34 에서 어떻게.** `el34-attacker` 에서 fw 게이트웨이(`10.20.30.1`)를 대상으로 돌린다.
...
```bash
docker exec el34-attacker sh -c 'nikto -h http://10.20.30.1 -maxtime 25s 2>&1 | head -12'
```
...
**el34 에서 어떻게.** el34 는 같은 IP(`10.20.30.1`)에서 `Host:` 헤더로 여러 vhost 를
구분하므로(W01 복습), 어떤 사이트를 핑거프린팅할지 `-H 'Host: ...'` 로 지정한다.

```bash
docker exec el34-attacker sh -c "whatweb -H 'Host: dvwa.el34.lab' http://10.20.30.1/ 2>&1 | head -5"
```

> **용어 — `-H 'Host: ...'`.** HTTP 요청의 `Host` 헤더를 강제로 지정하는 옵션이다. el34
> 의 web Apache 는 이 헤더를 보고 어느 vhost(여기선 dvwa)를 응답할지 결정한다. 같은
> `10.20.30.1` 이라도 `Host` 를 바꾸면 전혀 다른 사이트가 핑거프린팅된다.
...
```bash
docker exec el34-attacker sh -c 'printf "admin\nlogin\nconfig\n" > /tmp/wv2.txt; ffuf -u http://10.20.30.1/FUZZ -w /tmp/wv2.txt -H "Host: dvwa.el34.lab" -mc 200,301,403 -s 2>&1 | head; rm -f /tmp/wv2.txt'
```
...
```bash
docker exec el34-web sh -c 'sudo tail -200 /var/log/apache2/modsec_audit.log | grep -oiE "nikto|913[0-9]{3}" | sort | uniq -c'
```
...
```bash
docker exec el34-attacker sh -c "curl -sI -H 'Host: dvwa.el34.lab' http://10.20.30.1/ | grep -iE 'Server|X-Powered'"
```

`-sI` 는 본문 없이 응답 헤더만(`I`=head) 조용히(`s`=silent) 가져온다. 출력에 `Server:
Apache/2.4.52 ...` 처럼 **버전**이 보이면 핑거프린팅 단서가 노출된 것이다.
...
> **진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서
> `docker exec el34-attacker`(스캔) 또는 `docker exec el34-web`(방어자 로그 확인)로
...
>
> **무엇을 알 수 있는가?** `el34-attacker` 에 nikto·whatweb·ffuf 가 설치돼 실행 가능한지.
...
>
> **결과 해석.** 정상: 출력에 대상(`10.20.30.1`)과 식별된 기술 항목이 보임. 핵심 깨달음 —
...
>
> **무엇을 알 수 있는가?** `curl -sI` 로 `Server`·`X-Powered-By` 헤더를 점검해, 버전·기술
> 노출 여부를 판단하는 법. 노출된 버전이 곧 CVE 매핑의 입구라는 것.
>
> **결과 해석.** 정상: `Server` 헤더가 보이고, 거기에 버전(또는 OS) 정보가 노출됨 = 축소가
> 필요한 개선 지점. 비정상: 헤더 자체가 안 보이면 요청 대상·Host 헤더를 점검한다.
>
