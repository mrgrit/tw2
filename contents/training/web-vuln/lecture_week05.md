...
방화벽(fw)은 L3/L4 만 보므로 HTTP 안의 SQLi 문자열을 보지 못하고, **SQLi 를 실제로 잡는 곳은 web 의 ModSecurity(L7)** 다. 또한 el34 의 fw 는 DNAT 만 하고 SNAT 를 하지 않으므로, web·ips·siem 이 모두 공격자의 **진짜 출처 IP `10.20.30.202`** 를 본다 — 이것이 흩어진 흔적을 한 공격자로 묶는 키다(W01 §3.2 복습). 실습 명령은 모두 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서 `docker exec el34-<comp>` 로 실행한다.
