...
2. el34 의 `외부 공격자 VM 192.168.0.202` 컨테이너에서 각 유형의 SQLi 페이로드를 **URL 인코딩하여**
...
> 대상(`외부 공격자 VM 192.168.0.202` → el34 내부 vhost `dvwa`/`juice`)에 한해서만 수행한다. RoE(범위·시간·
...
**무엇을 보나.** `SLEEP(3)` 은 조건이 실행될 때 DB 가 3초 동안 멈추게 한다. 공격자는 "비밀번호 첫 글자가 'm' 이면 5초 자라" 처럼 조건부 지연을 걸어, **응답이 늦으면 참 / 빠르면 거짓**으로 판단한다. 화면에 아무 차이가 없어도 시간만으로 데이터를 빼낼 수 있다 — 그만큼 **가장 느리다**. 실습에서는 `sqlmap` 이 시간 기반 블라인드(time-based blind) 페이로드로 응답 지연을 자동 측정한다(juice 에서는 `SQLite > 2.0 AND time-based blind (heavy query)` 로 탐지). el34 dvwa 에서는 `SLEEP` 패턴이 WAF 942 룰에 잡혀 **403** 으로 막히고, `sqlmap` 이 "protection mechanism (e.g. WAF)" 로 이를 보고하는 것까지 함께 본다.
...
```mermaid
graph TD
    ATK["외부 공격자 VM 192.168.0.202 (외부 공격자 VM)<br/>192.168.0.202<br/>sqlmap (SQLi 자동화)"]
...
방화벽(fw)은 L3/L4 만 보므로 HTTP 안의 SQLi 문자열을 보지 못하고, **SQLi 를 실제로 잡는 곳은 web 의 ModSecurity(L7)** 다. 또한 el34 의 fw 는 DNAT 만 하고 SNAT 를 하지 않으므로, web·ips·siem 이 모두 공격자의 **진짜 출처 IP `192.168.0.202`** 를 본다 — 이것이 흩어진 흔적을 한 공격자로 묶는 키다(W01 §3.2 복습). 공격 요청은 공격 VM(`ssh att@192.168.0.202`, 비밀번호 1)에서, 탐지 로그 확인은 방어 측 장비(`ssh ccc@10.20.32.80` 등)에서 실행한다.
...
여기서 `%27` 은 `'`, `%20` 은 공백이다. URL 에서 공백을 그대로 두면 페이로드가 공백에서 잘리므로 인코딩이 필요하다는 원리를 이해해 두자. (실습에서 SQLi 자동화 도구인 `sqlmap` 은 이런 페이로드 인코딩을 자동으로 처리하므로, 학생이 직접 `%20`·`%27` 로 바꿀 필요는 없다 — 원리만 알면 된다.)
...
> 공격 요청은 `외부 공격자 VM 192.168.0.202`(ssh att@192.168.0.202) 에서, 탐지 로그 확인은
> web 장비(`ssh ccc@10.20.32.80`)에서 한다. **인가된 실습 환경(el34)에서만** 수행하고, 합격 임계값은 0.7 이다.
...
>
> **무엇을 알 수 있는가?** `외부 공격자 VM 192.168.0.202` 에서 `Host: dvwa.el34.lab` 헤더로 fw 게이트웨이
> (192.168.0.161)에 요청해 dvwa 가 HTTP 코드로 응답하는지. SQLi 점검 대상이 dvwa(차단)와
...
>
> **무엇을 알 수 있는가?** `sqlmap` 이 시간 기반 블라인드 페이로드를 dvwa 에 보내고, 응답의
> 지연 시간(총 응답 시간)으로 지연을 관찰하는 법. `SLEEP`/`heavy query` 패턴이 WAF 942(blind 룰)에
> 어떻게 탐지되는지.
>
