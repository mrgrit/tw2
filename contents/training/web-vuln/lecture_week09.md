...
1. **인증과 인가의 차이**를 한 문장으로 설명하고, 접근제어(인가) 실패가 왜 OWASP Top 10 의 1위인지
   근거를 댄다.
2. el34 의 점검자(`el34-attacker`, 출처 IP `10.20.30.202`)에서 JuiceShop(`juice.el34.lab`)에 요청을
...
| 시간        | 내용                                                                          | 유형      |
|-------------|-------------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 인증 vs 인가, 왜 접근제어가 OWASP A01(1위)인가                          | 강의      |
| 0:25–0:55   | 이론 — WSTG-ATHZ 점검 4축: 무인증 노출 · 강제 브라우징 · 인가 일관성 · IDOR    | 강의      |
| 0:55–1:05   | 휴식                                                                          | —         |
| 1:05–1:30   | 이론 — 수직/수평 권한상승과 IDOR, 서버측 인가 설계(소유권·역할·기본 deny)      | 강의/토론 |
| 1:30–2:00   | 실습 — 점검(도달성) + 무인증 관리 엔드포인트(A01) (lab 1–2)                    | 실습      |
| 2:00–2:30   | 실습 — 강제 브라우징(/ftp) + 인가 일관성(401 vs 200) (lab 3–4)                 | 실습      |
| 2:30–2:40   | 휴식                                                                          | —         |
| 2:40–3:10   | 실습 — IDOR 시도(순차 ID) + 탐지(접근 로그) (lab 5–6)                          | 실습      |
| 3:10–3:30   | 실습 — 서버측 인가 방어 정리 + 접근제어 종합 보고서 (lab 7–8)                  | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W10 — 전송 보안 TLS) 예고                    | 정리      |
...
점검자 컨테이너(`el34-attacker`, 출처 IP `10.20.30.202`)가 fw 의 게이트웨이(`10.20.30.1`)를 통해, HTTP
...
> juice.el34.lab` 이면 JuiceShop 으로 라우팅된다. 그래서 모든 점검 명령은 `curl -H 'Host:
> juice.el34.lab'` 형태로 표적을 명시한다.
...
점검자의 **진짜 출처 IP `10.20.30.202`** 가 그대로 남는다. 그래서 미션 6 에서 web 의 `access.log` 를
...
```mermaid
graph TD
    REQ["한 번의 점검 요청<br/>출처 10.20.30.202<br/>GET /api/Users/2 (IDOR 시도)"]
...
| 방어자(방어 측) | web access.log | 출처 IP(10.20.30.202) + 민감 경로(admin/ftp/Users) 반복 |

이 양면을 함께 볼 수 있으면, 점검 보고서에 "이 취약점은 이렇게 발견했고, 방어 측 로그에는 이렇게
남더라"까지 적을 수 있다 — 단순 공격자를 넘어 **방어를 개선하는 점검자**의 시야다. 같은 출처
IP(`10.20.30.202`)가 두 관점을 한 사건으로 묶는다는 점은 secuops/soc 트랙의 상관 분석과도 이어진다.
...
> el34-attacker`(점검) 또는 `docker exec el34-web`(방어 측 로그 확인)로 실행한다. 점검의 주력 도구는
...
> 실패면 경로(Host 헤더·게이트웨이 `10.20.30.1`)부터 점검한다.
>
