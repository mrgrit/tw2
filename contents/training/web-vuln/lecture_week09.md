...
> 자동화한 형태로, el34 의 attacker 에 설치되어 있다. 본 주차 실습은 `nc` 로 단일 경로(`/ftp`)를
...
한다. 본 주차의 모든 명령이 `nc` 로 API 를 직접 두드리는 이유다. 또한 el34 에서 JuiceShop 의 WAF 는
...
- **클라이언트측 검사** — 브라우저(JavaScript)에서 "너는 관리자가 아니니 막는다"고 판단하는 것.
  공격자는 브라우저를 거치지 않고 `nc` 로 서버에 직접 요청한다 — 클라이언트 검사는 보이지도 않는다.
...
> **`nc`**(raw HTTP) 이며, `head -1 | grep -oE '[0-9]{3}'` 로 응답 첫 줄에서 **응답 코드만** 빠르게 본다.
...
> 직접 입력으로 접근되는지 확인한다.
>
> **무엇을 알 수 있는가?** 화면 메뉴에 링크가 없는 `/ftp` 디렉터리를 `nc` 로 직접 호출했을 때 접근이
