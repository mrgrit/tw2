...
> 자동화한 형태로, el34 의 attacker 에 설치되어 있다. 본 주차 실습에서 단일 경로(`/ftp`) 존재·상태
> 확인은 `whatweb`(핑거프린팅)로, 여러 경로를 한꺼번에 훑는 열거는 `ffuf` 로 수행한다.
...
한다. 본 주차의 점검이 `whatweb`·`ffuf` 같은 도구로 REST API 엔드포인트를 직접 호출하는 이유다. 또한 el34 에서 JuiceShop 의 WAF 는
...
```mermaid
graph TD
    ATK["점검자 외부 공격자 VM<br/>192.168.0.202<br/>whatweb/ffuf (Host: juice.el34.lab)"]
...
- **클라이언트측 검사** — 브라우저(JavaScript)에서 "너는 관리자가 아니니 막는다"고 판단하는 것.
  공격자는 브라우저를 거치지 않고 `whatweb`·`ffuf` 같은 도구로 서버에 직접 요청한다 — 클라이언트 검사는 보이지도 않는다.
...
> **`whatweb`**(도달성·핑거프린팅·헤더)와 **`ffuf`**(경로 열거)이며, 응답 코드·기술 스택·노출 경로를 한 번에 본다.
...
> 직접 입력으로 접근되는지 확인한다.
>
> **무엇을 알 수 있는가?** 화면 메뉴에 링크가 없는 `/ftp` 디렉터리를 `whatweb`(또는 `ffuf`)로 직접 호출했을 때 접근이
