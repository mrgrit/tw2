...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 데이터는 선(線) 위에서 샌다: A02 와 전송 보안의 위치                 | 강의      |
| 0:20–0:50   | 이론 — TLS 한눈에: 핸드셰이크·프로토콜 버전·cipher 묶음·인증서 신뢰        | 강의      |
| 0:50–1:00   | 휴식                                                                        | —         |
| 1:00–1:30   | 이론 — MITM(중간자)은 어떻게 전송을 노리나 + 자체서명 인증서의 함정        | 강의/토론 |
| 1:30–2:00   | 실습 — 점검(도달성) + 프로토콜/cipher 확인(openssl s_client) (lab 1–2)     | 실습      |
| 2:00–2:30   | 실습 — cipher 강도 등급(nmap ssl-enum) + 자체서명 인증서 탐지 (lab 3–4)    | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 — 폐기 프로토콜(TLS1.1) 거부 확인 + 영향 정리 (lab 5–6)               | 실습      |
| 3:10–3:30   | 실습 — 전송 강화 방어 + 전송 보안 보고서 (lab 7–8)                         | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W11 — 에러/정보 노출) 예고               | 정리      |
...
일괄 열거)이다. 두 도구 모두 el34 의 점검자 컨테이너(`el34-attacker`)에 **이미 설치돼 있어 신규
설치가 필요 없다**. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec
el34-attacker` 로 실행한다.
...
```bash
docker exec el34-attacker sh -c "timeout 50 nmap --script ssl-enum-ciphers -p 443 10.20.30.1 2>/dev/null | grep -iE 'TLSv|least strength'"
```
...
> el34-attacker` 로 실행한다. 신규 도구 설치는 없다 — `openssl`·`nmap` 모두 el34-attacker 에 기본
...
> 코드를 돌려주는지. `curl -k` 는 자체서명 인증서 경고를 무시하고 연결 자체의 도달성만 본다.
>
> **결과 해석.** 정상: `https=<코드>`(예: 200/302)가 출력 → 443 도달. 비정상: 응답이 없으면
> 경로(Host 헤더·게이트웨이 10.20.30.1)부터 재확인한다.
>
