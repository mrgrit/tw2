...
2. el34 의 외부 공격자 VM(`ssh att@192.168.0.202`)에서 `whatweb` 로 JuiceShop REST API 를 핑거프린팅해,
   **공개 자원(`/api/Products` 200)과 보호 자원(`/api/Users` 401)의 경계**를 상태코드로 매핑한다.
...
| **whatweb** | — | 요청을 보내 상태·서버·헤더·본문 단서를 요약하는 웹 핑거프린팅 도구 | 창구에 물어보고 간판·재고표를 통째로 받아오기 |
| **ffuf** | — | URL 의 FUZZ 자리를 워드리스트로 치환해 반복 요청하는 퍼징 도구 | 접수번호를 1·2·3… 바꿔가며 창구를 두드려 보기 |
...
공격자는 화면을 쓰지 않고 `whatweb`·`ffuf` 같은 도구로 API 에 직접 요청을 보낼 수 있기 때문이다. 한 장의 비유로 정리하면
끝까지 헷갈리지 않는다.
...
그러나 공격자는 화면을 전혀 쓰지 않는다. 도구 한 줄로 API 에 직접 요청을 보내면 화면의 통제는
...
> 표현한다. 본 주차의 점검 명령이 화면이 아니라 도구로 `/api/...` 를 직접 두드리는 이유가 여기에 있다.
...
```mermaid
graph TD
    REQ["점검자 도구 요청<br/>화면 우회 → /api/... 직접 호출"]
...
추가하는 것은 도구 한 줄이면 충분하므로 화면 통제는 무의미하다.
...
```mermaid
graph TD
    ATK["점검자 외부 공격자 VM<br/>192.168.0.202<br/>whatweb / ffuf (juice.el34.lab)"]
...
본 주차의 점검 도구는 **whatweb**(핑거프린팅·경계/노출 확인)과 **ffuf**(객체 id 퍼징)다. 모든 명령은
외부 공격자 VM(`ssh att@192.168.0.202`, 비밀번호 1)에서 실행하며, 표적은 URL(`http://juice.el34.lab/...`)로 지정한다.

> **용어 — whatweb / ffuf.** `whatweb -a1 --color=never http://juice.el34.lab/경로` 는 요청을 보내
> 상태코드(`[200 OK]`/`[401 Unauthorized]`)·서버·헤더·본문 단서(이메일 등)를 한 줄로 요약한다 — 경계
> 매핑과 과다 노출(Email 추출) 확인에 쓴다. `ffuf -u .../FUZZ -w <목록>` 는 URL 의 `FUZZ` 자리를
> 워드리스트로 치환해 반복 요청하는 퍼징 도구로, 객체 id 열거(BOLA 점검)에 쓴다.

### 4.1 공개/보호 경계 매핑

```bash
# whatweb으로 공개 자원(200)과 보호 자원(401)을 상태코드로 구분
whatweb -a1 --color=never http://juice.el34.lab/api/Products | grep -oE '[0-9]{3} (OK|Unauthorized|Forbidden)'
whatweb -a1 --color=never http://juice.el34.lab/api/Users | grep -oE '[0-9]{3} (OK|Unauthorized|Forbidden)'
```

무엇을 보나 — `/api/Products` 는 `200 OK`(공개), `/api/Users` 는 `401 Unauthorized`(보호). 이 두 코드로 API 의
공개/보호 경계를 긋는다. 200 은 "의도된 공개일 수 있음", 401 은 "인증으로 보호됨(양호)"으로 해석한다.

### 4.2 과다 데이터 노출 (API3)

```bash
# whatweb으로 무인증 엔드포인트 응답을 분석 → Email 플러그인이 본문에서 이메일 추출
whatweb -a1 --color=never http://juice.el34.lab/api/Feedbacks | grep -oE 'Email\[[^]]*\]'
```

무엇을 보나 — `/api/Feedbacks` 가 무인증 200 으로 응답하면서 본문에 작성자 이메일(PII)을 담아 whatweb 이
`Email[...]` 로 추출하는지. 화면에는 안 보이지만 응답에는 들어 있다는 것이 과다 노출(API3)의 증거다.

### 4.3 BOLA (API1) — 순차 객체 id

```bash
# ffuf로 /api/Users/{id} 를 순차 퍼징해 객체 수준 인가 상태 확인
seq 1 6 > /tmp/ids.txt
ffuf -u http://juice.el34.lab/api/Users/FUZZ -w /tmp/ids.txt -mc all 2>/dev/null | grep -oE 'Status: [0-9]+'
```

무엇을 보나 — 각 id 의 응답 코드. el34 JuiceShop 은 전부 `Status: 401`(보호) 을 주어 객체 수준 인가가
작동함을 보인다(양호). 만약 200 이 나오면 남의 객체가 노출되는 치명적 BOLA 다.
...
> **실습 진행 원칙.** 모든 명령은 외부 공격자 VM(`ssh att@192.168.0.202`, 비밀번호 1)에서 실행한다.
> 표적은 **인가된 JuiceShop(`juice.el34.lab`) vhost 만** 점검한다. 도구는 whatweb·ffuf — att 에
> 탑재. 합격 임계값은 0.7 이다.
...
본 주차(W12)에서 학생은 API 취약점을 whatweb·ffuf 로 **하나씩 손으로** 점검했다. 수동 점검은 정밀하지만
...
```mermaid
graph TD
    W12["W12 API 보안<br/>BOLA·과다노출·Mass Assignment<br/>(수동 whatweb/ffuf 점검 — 정밀)"]
