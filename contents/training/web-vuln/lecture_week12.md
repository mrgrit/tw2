...
| **nc** | — | raw HTTP 요청을 손수 보내는 표준 도구 | 사람 손으로 직접 창구에 주문서를 밀어 넣는 도구 |
...
공격자는 화면을 쓰지 않고 `nc`(raw HTTP) 로 API 에 직접 요청을 보낼 수 있기 때문이다. 한 장의 비유로 정리하면
끝까지 헷갈리지 않는다.
...
그러나 공격자는 화면을 전혀 쓰지 않는다. `nc` 한 줄로 API 에 직접 요청을 보내면 화면의 통제는
...
> 표현한다. 본 주차의 점검 명령이 화면이 아니라 `nc` 로 `/api/...` 를 직접 두드리는 이유가 여기에 있다.
...
```mermaid
graph TD
    REQ["점검자 nc 요청<br/>화면 우회 → /api/... 직접 호출"]
...
추가하는 것은 `nc` 한 줄이면 충분하므로 화면 통제는 무의미하다.
...
본 주차의 점검 도구는 **`nc`**(raw HTTP) 하나다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에서
`ssh att@192.168.0.202` 로 실행하며, 표적은 `Host: juice.el34.lab` 헤더로 지정한다.

> **용어 — nc raw HTTP 관용구.** 요청 줄(`GET /경로 HTTP/1.0`)과 `Host: juice.el34.lab` 헤더(어느
> vhost 를 점검할지 지정)를 손으로 써서 `nc` 로 보낸다. `head -1 | grep -oE '[0-9]{3}'`(응답 첫 줄에서
> **응답 코드만** 뽑기 — 도달성·보호 여부를 빠르게 볼 때 쓰는 관용구). 응답 **본문**을 봐야 할 때(과다
> 노출 확인)는 `head -1` 대신 본문을 받아 `grep` 으로 원하는 필드를 추출한다.
...
> 설치는 없다 — `nc` 는 기본 탑재되어 있다. 합격 임계값은 0.7 이다.
...
본 주차(W12)에서 학생은 API 취약점을 `nc` 로 **하나씩 손으로** 점검했다. 수동 점검은 정밀하지만
...
```mermaid
graph TD
    W12["W12 API 보안<br/>BOLA·과다노출·Mass Assignment<br/>(수동 nc 점검 — 정밀)"]
