...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — API 란 무엇인가, 왜 화면 없는 공격 표면이 위험한가 (UI 우회)         | 강의      |
| 0:25–1:00   | 이론 — OWASP API Top 10 핵심 4종: BOLA(API1)/인증(API2)/과다노출(API3)/Mass Assignment(API6) | 강의 |
| 1:00–1:10   | 휴식                                                                       | —         |
| 1:10–1:40   | 이론 — JuiceShop REST 구조 + el34 진입 경로(Host 헤더 vhost) + 출처 IP 보존 | 강의/토론 |
| 1:40–2:05   | 실습 — 점검(도달성) + 공개/보호 경계 매핑 (lab 1–3)                         | 실습      |
| 2:05–2:35   | 실습 — 과다 데이터 노출(API3) + BOLA(API1) (lab 4–5)                        | 실습      |
| 2:35–2:45   | 휴식                                                                       | —         |
| 2:45–3:10   | 실습 — Mass Assignment 원리 + API 취약점 영향 정리 (lab 6)                  | 실습      |
| 3:10–3:30   | 실습 — API 인가 방어 정리 + 종합 보고서 (lab 7–8)                           | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W13 — 자동화 스캐닝 nuclei) 예고          | 정리      |
