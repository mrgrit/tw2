...
```bash
# 평문 경로순회 — 930 → 403
curl -s -o /dev/null -w 'pt=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://192.168.0.161/?q=../../etc/passwd'
# URL 인코딩 우회 시도 — 정규화 후 동일하게 930 → 403
curl -s -o /dev/null -w 'enc=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://192.168.0.161/?q=%2e%2e%2f%2e%2e%2fetc%2fpasswd'
```
...
```bash
# 세미콜론으로 명령 연결
curl -s -o /dev/null -w 'cmdi=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://192.168.0.161/?q=1;cat%20/etc/passwd'
# 파이프로 명령 연결
curl -s -o /dev/null -w 'pipe=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://192.168.0.161/?q=1|id'
```
...
curl -s -o /dev/null -w 'php=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://192.168.0.161/?q=%3C%3Fphp%20system(%27id%27)%3B%20%3F%3E'
```
...
`ssh att@192.168.0.202`(공격 발생)와 `docker exec el34-web`(탐지 확인)로 수행한다. 각 미션을
