...
```bash
# nuclei LFI 템플릿군으로 경로순회·LFI 자동 점검 (../ · %2e%2e%2f 변형 포함) — 930 차단으로 finding 0
nuclei -u http://dvwa.el34.lab -tags lfi 2>&1 | grep -aE 'Templates loaded|signed templates'
```

nuclei 가 LFI 템플릿군(수백 개)을 실행하고도 매치(finding)를 하나도 못 내면 정상이다 — dvwa 의
ModSec 930 이 평문 `../` 와 URL 인코딩 `%2e%2e%2f` 변형을 **정규화**해 모두 403 차단했기 때문이다.
여기서 학생이 얻어야 할 통찰은 두 가지다. (1) **WAF 는 인코딩만 다른 같은 공격을 한 패턴으로 묶어
잡는다**(정규화의 힘). (2) **그러나 이 차단은 "WAF 가 막아 준" 것이지 "앱이 안전한" 것이 아니다** —
WAF 를 우회하는 인코딩이 발견되면 뚫린다. 그래서 근본 방어는 앱 자신의 경로 검증이다(§5).
...
```bash
# nuclei RCE/injection 템플릿군으로 명령주입 자동 점검 (;cat · |id · $( ) 등) — 932 차단으로 finding 0
# (명령주입 자동화 전용 도구 commix -u "URL" --batch 는 이 att 미설치 → nuclei 로 대체)
nuclei -u http://dvwa.el34.lab -tags rce,injection 2>&1 | grep -aE 'Templates loaded|signed templates'
```

> **도구 노트 — commix vs nuclei.** 명령주입을 자동화하는 전용 도구는 `commix`(`commix -u "URL"
> --batch`)지만 이 실습 att VM 에는 미설치라, 범용 취약점 스캐너 `nuclei` 의 RCE/injection 템플릿군으로
> 대체한다. 두 도구 모두 `;` `|` `$( )` 같은 셸 구분자 페이로드와 URL 인코딩(공백=`%20`)을 내부에서
> 자동 생성해 보낸다.

**결과 해석 — 명령주입은 다르게 본다.** 경로순회·PHP 주입은 스캐너 매치 0(=WAF 차단)으로 결론지었지만,
명령주입은 스캐너 결과만으로 성패를 단정하기 어렵다(블라인드 명령주입 포함 — 앱이 명령 결과를 화면에
안 보여주면 매치가 안 잡혀도 명령은 실행됐을 수 있다). 그래서 점검자는 **"이 입력점이 OS 명령에 닿는가,
그리고 그 시도가 방어 스택에 흔적을 남기는가"** 를 먼저 확인한 뒤(다음 단계에서) ModSec audit 로그의
932 매치로 탐지를 입증한다. 점검의 무게중심을 "스캐너 매치 하나"가 아니라 **"방어 스택이 이 공격을
보았는가(932 흔적)"** 에 두는 것이다.
...
```bash
# nikto 주입 계열(Tuning 4) 점검으로 dvwa에 위험 콘텐츠(<?php 등) 탐침 — 933 차단
nikto -h http://dvwa.el34.lab -Tuning 4 -maxtime 30s
```

nikto 의 주입 계열(`-Tuning 4`) 점검은 `<?php ... ?>` 같은 위험 콘텐츠를 포함한 탐침을 보내지만,
ModSec 933 이 PHP 태그·위험 함수를 webshell 시그니처로 인지해 403 차단하므로 nikto 는 주입 취약점을
하나도 보고하지 못한다(서버 배너·헤더 관찰만 남는다). 단, 이 점검은 **"위험한 콘텐츠가 WAF 에
걸리는가"** 를 보는 것이지, 실제 파일을 디스크에 올려 실행까지 한 것은 아니다 — 실제 업로드 우회·셸
핸들링은 고급 트랙의 영역이다(§1.3).
...
|--------|-----------|-----------|-------------------|-----------|-----------|
| 경로순회 | `../` 경로 조각 | 임의 파일 읽기 | **930**(LFI) | nuclei 0건(WAF 403) | 경로 정규화 + 허용 폴더 검증 |
| 명령주입 | `; \| & $( )` 셸 구분자 | **RCE**(완전 장악) | **932**(RCE) | nuclei 0건·932 흔적 | OS 호출 회피 + 인자 allowlist |
| 파일 업로드 | webshell(`<?php`) | **RCE**(완전 장악) | **933**(PHP) | nikto 0건(WAF 403) | 확장자·MIME allowlist + 실행권한 제거 |
...
> 이후 어떤 페이로드도 의미가 없다.
>
> **무엇을 알 수 있는가?** whatweb 핑거프린팅으로 `dvwa.el34.lab` 이 응답하는지·WAF 가 걸려 있는지,
> 즉 RCE 취약점 점검을 시작할 준비가 됐는지.
>
> **결과 해석.** 정상: whatweb 이 `[403 Forbidden]` + `Apache/2.4.52 (Ubuntu)` 배너로 응답 = 도달·WAF
> 확인. 비정상: 무응답이면 호스트 도달성·vhost 헤더·컨테이너 상태를 먼저 점검한다.
>
> **실전 활용.** WSTG 점검 착수 시 첫 확인. 대상 도달성을 검증해야 본격 점검으로 넘어간다.

### 미션 2 — 경로순회: nuclei LFI 스캔(WAF 930 차단) (manipulation)

> **왜 하는가?** 세 취약점 중 가장 직관적인 경로순회를 취약점 스캐너(nuclei LFI 템플릿군)로 자동
> 시도해, `../` 가 어떻게 폴더를 빠져나가려 하는지와 WAF 가 그것을 어떻게 잡는지 본다.
>
> **무엇을 알 수 있는가?** nuclei 의 LFI 템플릿군(평문 `../`·URL 인코딩 `%2e%2e%2f` 변형 포함)이
> 모두 ModSec 930 에 막혀 finding 0 이 된다는 것. 즉 WAF 가 정규화로 인코딩 우회를 함께 막는다는 것.
>
> **결과 해석.** 정상: nuclei LFI 템플릿 매치 0건(930 차단). 핵심 깨달음 — 이 차단은 "WAF 가 막아 준"
> 것이지 "앱이 안전한" 것이 아니다. 근본 방어는 앱의 경로 검증이다.
>
> **실전 활용.** 파일 경로를 입력으로 받는 모든 엔드포인트(다운로드·미리보기·템플릿 로드)의 표준
> 점검 항목.

### 미션 3 — 명령주입: nuclei RCE 스캔(commix 미설치 대체) (manipulation)

> **왜 하는가?** RCE 로 직결되는 명령주입을, 명령주입 자동화 전용 도구 commix 미설치 환경에서
> nuclei RCE/injection 템플릿군으로 자동 시도해, 셸 구분자(`;` `|`)가 OS 명령을 어떻게 잇는지 본다.
>
> **무엇을 알 수 있는가?** nuclei RCE/injection 템플릿군이 셸 구분자 페이로드를 던져도 ModSec 932 에
> 막혀 finding 0 이 된다는 것. 명령주입은 스캐너 매치만으로 성패를 단정하기 어려워(블라인드 포함),
> 탐지의 입증은 다음 미션(ModSec 932 로그)에서 한다(§3.3).
>
> **결과 해석.** 정상: nuclei RCE/injection 매치 0건(932 차단). 핵심 깨달음 — 명령주입은 "이 입력점이
...
### 미션 4 — PHP 주입: nikto 주입 점검(WAF 933 차단) (manipulation)

> **왜 하는가?** 파일 업로드 RCE 의 핵심인 webshell 콘텐츠가 WAF 에 어떻게 잡히는지, 업로드 폼
> 없이 nikto 주입 계열(Tuning 4) 점검으로 확인한다.
>
> **무엇을 알 수 있는가?** nikto 주입 계열 탐침(`<?php ... ?>` 위험 콘텐츠 포함)이 ModSec 933 에 막혀
> 주입 취약점 0건이 된다는 것.
>
> **결과 해석.** 정상: nikto 배너·서버 관찰만 남고 주입 취약점 0건(933 차단). 핵심 깨달음 — 이는
> "위험 콘텐츠가 WAF 에 걸린" 것이지 실제 파일을 올려 실행한 것은 아니다. 실제 업로드 우회·셸 핸들링은
> 고급 트랙의 영역이다.
>
