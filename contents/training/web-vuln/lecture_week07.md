...
| 시간        | 내용                                                                    | 유형     |
|-------------|-------------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — 왜 이 세 취약점이 가장 위험한가 (입력 신뢰 → RCE)                  | 강의     |
| 0:20–0:55   | 이론 — 경로순회·명령주입·파일 업로드 메커니즘 상세                        | 강의     |
| 0:55–1:05   | 휴식                                                                    | —        |
| 1:05–1:35   | 이론 — ModSec 930/932/933 룰군 + dvwa(차단) vs juice(탐지만) 재확인       | 강의/토론 |
| 1:35–2:00   | 실습 — 점검(대상) + 경로순회(`../../etc/passwd`) (lab 1–2)               | 실습     |
| 2:00–2:30   | 실습 — 명령주입(`;cat`) + PHP 주입(`<?php`) (lab 3–4)                    | 실습     |
| 2:30–2:40   | 휴식                                                                    | —        |
| 2:40–3:10   | 실습 — ModSec 93x 탐지 확인 + 영향 정리 (lab 5–6)                        | 실습     |
| 3:10–3:30   | 실습 — 입력검증 방어 + WSTG 점검 보고서 (lab 7–8)                        | 실습     |
| 3:30–3:40   | 정리 + 다음 주차(W08 — 중간고사) 예고                                    | 정리     |
...
docker exec el34-attacker sh -c "curl -s -o /dev/null -w 'php=%{http_code}\n' -H 'Host: dvwa.el34.lab' 'http://10.20.30.1/?q=%3C%3Fphp%20system(%27id%27)%3B%20%3F%3E'"
```
...
실습은 8 미션으로 구성되며, 모두 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서
`docker exec el34-attacker`(공격 발생)와 `docker exec el34-web`(탐지 확인)로 수행한다. 각 미션을
**4축**(왜 하는가 / 무엇을 알 수 있는가 / 결과 해석 / 실전 활용)으로 설명한다.

> ⚠️ **인가된 실습만.** 본 트랙의 모든 공격은 **인가된 실습 환경(el34)** 안에서, 정해진 대상
> (`el34-attacker` → `dvwa.el34.lab`)에 한해서만 수행한다. 실제 외부 시스템을 대상으로 한 시도는
