...
실제 아티팩트 분석으로 익힌다(실제 ROS2 공격은 로봇·네트워크 환경 필요).
...
- **A. 토픽 스니핑 실증 (필수, 40점)** — `TOPIC_SNIFFED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 명령 인젝션 분석 (필수, 30점)** — `COMMAND_INJECTED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. SROS2 강화 방어 설계 (필수, 30점)** — `SROS2_ENFORCED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(TOPIC_SNIFFED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(COMMAND_INJECTED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(SROS2_ENFORCED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **ROS2 보안: DDS·토픽 스니핑·명령 인젝션·SROS2**.
- **토픽 스니핑**(`TOPIC_SNIFFED`): 무암호 DDS에서 토픽을 감청한다.
- **명령 인젝션**(`COMMAND_INJECTED`): 제어 토픽에 위조 명령을 발행해 로봇을 움직인다.
- **SROS2 강화**(`SROS2_ENFORCED`): 인증·암호화·접근 제어를 활성해 스니핑·인젝션을 막는다.
