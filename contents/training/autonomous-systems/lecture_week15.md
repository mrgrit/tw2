...
- **A. 전체 CPS 종합 평가 실증 (필수, 40점)** — `FULL_CPS_PENTEST` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 안전 우선 다층 방어 분석 (필수, 30점)** — `DEFENSE_SYNTHESIZED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
...
| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(FULL_CPS_PENTEST) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(DEFENSE_SYNTHESIZED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(SYNTHESIS) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **종합 평가: 전체 CPS 침투+방어**.
- **전체 CPS 종합 평가**(`FULL_CPS_PENTEST`): 대상 CPS를 3계층에서 점검하고 사이버→물리 경로를 구성한다.
- **안전 우선 다층 방어**(`DEFENSE_SYNTHESIZED`): 통신·센서·AI·OT·V2X 방어와 독립 안전 계층을 겹층으로 종합한다.
- **핵심 원칙 종합**(`SYNTHESIS`): 5대 원칙(3계층·경로·심층 방어·안전 최우선·CPS IR)을 정리한다.
