...
6. 알려진 악성 도구(sqlmap)로 공격을 재현해 IOC 가 Wazuh `alerts.json`(ids 그룹)으로
   수렴하고 **출처 IP(10.20.30.202)가 보존** 됨을 데이터로 확인하며, 그 IOC 를 **MITRE
...
| 시간      | 내용                                                                       | 유형      |
|-----------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25 | 이론 — "내가 본 것" vs "세상이 아는 것": CTI·IOC 의 동기                    | 강의      |
| 0:25–0:55 | 이론 — STIX 2.1 객체 + TAXII 2.1 (위협의 표준 언어)                         | 강의      |
| 0:55–1:05 | 휴식                                                                       | —         |
| 1:05–1:35 | 이론 — el34 인텔 스택(OpenCTI/MISP) + CDB list + 매칭/격상 룰             | 강의/토론 |
| 1:35–2:00 | 실습 1, 2 — 인텔 스택 점검 + IOC 공격 재현(텔레메트리 출현)                 | 실습      |
| 2:00–2:30 | 실습 3, 4 — CDB list 작성 + IOC 매칭/격상 룰 logtest 검증                   | 실습      |
| 2:30–2:40 | 휴식                                                                       | —         |
| 2:40–3:10 | 실습 5, 6, 7 — ATT&CK 매핑 + 인텔 플랫폼 + IOC 수렴(출처 보존)              | 실습      |
| 3:10–3:30 | 실습 8, 9 — 종합 보고서 + 정리(베이스 보존) 확인                            | 실습      |
| 3:30–3:40 | 정리 + 과제 안내 + 다음 주차(W13) 예고                                      | 정리      |
...
## 0.5 신입생 친화 핵심 개념 — "내가 본 것"에 "세상이 아는 것"을 더한다
...
```bash
# el34 호스트(ssh ccc@192.168.0.151)에서
...
10.20.30.202 / 외부 공격자 192.168.0.202** 의 출처 IP 가 ips·web·siem 전 계층에 그대로
...
docker exec el34-attacker sh -c "curl -s -o /dev/null -A sqlmap/1.7 -H 'Host: dvwa.el34.lab' \
  \"http://10.20.30.1/?id=ct12-7%27%20UNION%20SELECT%201--%20-\""
sleep 8
...
#   → {"r":"86601","s":"10.20.30.202"}   (ids 그룹 경보, 출처 보존)
```
...
`s:"10.20.30.202"` 가 출처가 보존됐다는 증거다. `sleep 8` 은 공격이 eve.json 을 거쳐
...
graph TD
    ...
    ATTACK["④ 실제 공격 (텔레메트리)<br/>sqlmap UA → Suricata eve → alerts.json<br/>출처 10.20.30.202 보존"]
...
`docker exec el34-<comp>` 로 실행한다. 매칭 룰은 **logtest 로만 검증**(라이브 restart
...
> - 출처 IP(10.20.30.202)가 전 계층에 보존됨 → 인텔 매칭의 전제
>
> **결과 해석**
...
6. **출처 보존 + ATT&CK 매핑** — el34 는 SNAT 안 해 IOC 공격의 출처(10.20.30.202)가
