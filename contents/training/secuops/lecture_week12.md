...
```bash
# 외부 공격자 sqlmap SQLi(자체 UA sqlmap/1.x + UNION SELECT) → Suricata eve → ips agent → manager → alerts.json
ssh att@192.168.0.202 "sqlmap -u 'http://dvwa.el34.lab/?id=ct12-7' --batch --level=2 --risk=2 --disable-coloring 2>&1 | grep -iE 'sqlmap|403|WAF/IPS' | head -3"
...
`s:"192.168.0.202"` 가 출처가 보존됐다는 증거다. 조건 폴링(로그 흔적 대기) 은 공격이 eve.json 을 거쳐
