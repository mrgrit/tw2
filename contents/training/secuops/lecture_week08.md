...
6. el34 가 **SNAT 를 하지 않아 출처 IP(10.20.30.202)가 끝까지 보존된다**는 사실을 이용해, IDS 의
...
중간고사는 시험이지만, 본 강의 시간은 시험 직전의 **종합 복습 + 판단 프레임워크 정립 + 실기 평가**로
구성한다.

| 시간        | 내용                                                                  | 유형     |
|-------------|-----------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — 왜 단일 장비로는 사슬을 못 막는가 (다층 방어 재정리)             | 강의     |
| 0:20–0:50   | 이론 — 공격 사슬(kill chain) 3 단계 + 각 단계의 장비 매핑              | 강의     |
| 0:50–1:00   | 휴식                                                                   | —        |
| 1:00–1:30   | 이론 — el34 진입 경로(fw nftables + web Apache, HAProxy 없음) 재확인   | 강의/토론 |
| 1:30–2:00   | 실기 — 점검(5종 가용성) + ① 정찰 재현 + IDS 룰 작성 (lab 1–3)          | 시험     |
| 2:00–2:30   | 실기 — ② 웹 침투(SQLi) + WAF anomaly 누적 확인 + 방화벽 점검 (lab 4–6) | 시험     |
| 2:30–2:40   | 휴식                                                                   | —        |
| 2:40–3:10   | 실기 — ③ 호스트 발판(리스너/계정/cron) + osquery 사냥 (lab 7–8)        | 시험     |
| 3:10–3:30   | 실기 — SIEM 수렴 + 출처 IP 상관 + 종합 보고 + 정리 확인 (lab 9–12)     | 시험     |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W09 — Wazuh manager) 예고            | 정리     |
...
> 진짜 출처 IP(`10.20.30.202`)를 본다 — 이것이 본 중간고사에서 **5계층을 한 사건으로 엮는 키**다.
...
공격자**(`el34-attacker`, 출처 IP `10.20.30.202`)가 fw 의 게이트웨이(`10.20.30.1`)를 통해
`dvwa.el34.lab` vhost 를 노린다. el34 는 SNAT 를 하지 않으므로 출처 IP `10.20.30.202` 가 모든
계층에 그대로 보존된다.
...
```mermaid
graph TD
    ATK["외부 침입자<br/>10.20.30.202<br/>http://10.20.30.1 (Host: dvwa.el34.lab)"]
...
graph TD
    ...
    C["web access.log<br/>10.20.30.202"]
    ATK --> FW
    FW --> A
    FW --> B
    FW --> C
...
fw 가 SNAT 를 하지 않으므로, 안쪽의 모든 장비가 공격자의 **진짜 출처 IP `10.20.30.202`** 를 본다.
...
```bash
docker exec el34-fw nft list ruleset | head -40        # 정책 + NAT 테이블 전체
docker exec el34-fw nft list table ip six_nat          # DNAT 규칙만
```
...
한 SQLi 요청이 남기는 5개의 흔적을 **출처 IP(10.20.30.202) + 시각**으로 엮으면 하나의 타임라인이
된다. 같은 사건이 장비마다 다른 단서로 보인다는 것을 이해하는 것이 종합 사고의 정점이다.

```mermaid
graph TD
    EVENT["한 SQLi 요청<br/>출처 10.20.30.202"]
...
의 `remote_address` 가 같은 `10.20.30.202`** 라는 것 — 이것이 출처 보존(SNAT 없음) 덕에 가능하며,
...
> **시험 진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker exec el34-<comp>`
> 로. 각 미션은 **독립적**이며, 각 미션에서 심은 것은 그 미션에서 정리한다(self-clean). 합격
> 임계값은 0.7 이다.
...
> 본다.
>
> **무엇을 알 수 있는가?** 외부 공격자(10.20.30.202)의 `sqlmap` UA 요청이 WAF 의 CRS **913 룰군(스캐너
...
> IP(10.20.30.202)로 보존됨.
>
> **결과 해석.** 정상: audit 에 `942100` 과 `949110` 이 함께 보이고 `status`=403, `remote_address`
> =10.20.30.202. 비정상: 949110 없이 942 만 보이면 anomaly 임계 설정을 확인.
>
...
> 같은 `10.20.30.202` 임을 확인. 이것이 정찰·침투·발판이 한 공격자의 한 사슬임을 입증하는 키.
>
> **결과 해석.** 정상: eve.json `src_ip` = modsec `remote_address` = 10.20.30.202. 비정상: 두 IP 가
