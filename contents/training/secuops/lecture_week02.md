...
1. `docker exec el34-fw nft list ruleset` 로 방화벽의 인터페이스, 테이블(`inet six_filter`/`ip six_nat`), 룰셋, 객체(set), conntrack 을 읽는다.
...
> 우리가 만지는 것은 **`inet six_filter` / `ip six_nat`** 두 테이블뿐이다. Docker 가 자동 생성한
> `ip nat` 는 건드리지 않는다 — 무심코 바꾸면 컨테이너 망이 깨진다.
...
예: 체인 `forward`, 출발지 `10.20.30.202`, 동작 `drop`, 위치 `top` → nft 명령:
...
nft **set**(객체)으로 IP 그룹 `blocklist` 생성 → 룰에서 `@blocklist` 참조:
