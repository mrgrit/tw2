...
> (실측 — src=10.20.30.202 그대로 보존), ⑤ 새 alert 룰 작성·트리거 → R/B/P 까지를
...
3. el34-ips 의 두 NIC (eth0=pipe / eth1=dmz) 가 같은 transaction 을 어떻게 2 event 로
   기록하는지 + 실측 src IP (el34 출처 보존, 10.20.30.202)
...
| 0:55–1:05 | 휴식                                                                 | —        |
| 1:05–1:30 | el34-ips 실측 — af-packet/cluster/runmode/출처 IP 보존                | 강의/토론|
...
| 3:30–3:40 | 정리 + W04 (pcre / flowbits / threshold) 예고                        | 정리     |
...
- `src_ip: 10.20.30.202` — **실제 공격자 IP** — el34 는 fw 가 SNAT 하지 않아 출처가 보존된다
...
```bash
# 1. 룰 추가
docker exec el34-ips sh -c 'echo "alert http any any -> any any (msg:\"el34 sqlmap UA\"; http.user_agent; content:\"sqlmap\"; nocase; classtype:web-application-attack; priority:2; sid:9004001; rev:1;)" | sudo tee -a /etc/suricata/rules/local.rules'
...
docker exec el34-ips sh -c 'grep "9004001" /var/log/suricata/eve.json | tail -1 | jq .alert'
```
...
```bash
docker exec el34-ips sh -c 'sudo grep -c "9004001" /var/log/suricata/eve.json'
```
...
같은 src_ip (`10.20.30.202`) 에서 여러 URL 에 시도가 burst 로 일어나는 패턴이 핵심이다.
...
4. Search bar 에 `event_type:http AND src_ip:"10.20.30.202"` 입력 후 Enter. el34 는 출처 IP 가 보존되므로 src_ip 로 바로 attacker 를 식별한다.
...
- src_ip 가 `10.20.30.202` 하나로 일관된다. 학습 환경의 el34-attacker(외부 .202) 다.
...
학습 환경에서는 attacker 의 ext IP (`10.20.30.202`) 를 el34-fw 의 nftables set 에 timeout 5분으로 추가하는 것으로 충분하다(출처 IP 가 보존되므로 src 기반 차단이 바로 동작). L7 기반 차단은 W05 ModSec 에서 별도 다룬다.

```bash
docker exec el34-fw sh -c 'sudo nft add element ip six_filter blacklist_v4 { 10.20.30.202 timeout 5m }'
```
...
4. Search bar 에 `event_type:tls AND src_ip:"10.20.30.202"` 를 입력하고 Enter. TLS 평문 메타는 fw 의 TCP termination 전에 ips 가 보므로 src_ip 가 그대로 el34-attacker 의 ext IP 다.
...
```bash
docker exec -it el34-ips bash    # ips 컨테이너 진입
...
- src_ip 가 `10.20.30.202` 하나로 일관된다. el34-attacker 의 ext IP 다.
...
```bash
docker exec el34-ips sh -c 'pgrep -a Suricata'
docker exec el34-ips sh -c 'sudo suricatasc -c version'
docker exec el34-ips sh -c 'sudo suricatasc -c uptime'
```

### 실습 2 — suricata.yaml 핵심 키 (15분)

```bash
docker exec el34-ips sh -c 'sudo grep -A3 "^af-packet:" /etc/suricata/suricata.yaml | head -15'
docker exec el34-ips sh -c 'sudo grep -E "^rule-files:|^default-rule-path" /etc/suricata/suricata.yaml'
docker exec el34-ips sh -c 'sudo grep -A8 "^outputs:" /etc/suricata/suricata.yaml | head -15'
```
...
docker exec el34-ips sh -c 'sudo grep "\"event_type\":\"http\"" /var/log/suricata/eve.json | tail -1 | jq -c .http'
docker exec el34-ips sh -c 'sudo grep "\"event_type\":\"tls\"" /var/log/suricata/eve.json | tail -1 | jq -c {sni:.tls.sni, ja3:.tls.ja3}'
```
...
4. **el34 출처 보존** — ips 의 src_ip = 10.20.30.202 (실제 attacker) 그대로. 룰은 src_ip 로 바로 매칭
