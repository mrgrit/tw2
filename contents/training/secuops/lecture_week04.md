...
[Step 2] nc 로 GET /admin → step1 의 flag 가 켜진 conn 이라면 alert
```
...
트리거: `nc` 로 POST(바디 `rrn=901231-1234567`) 형식.
...
# admin 기본인증 실패 시도 → 401/403
echo -en 'GET /admin/ HTTP/1.0\r\nHost: juice.el34.lab\r\nAuthorization: Basic YWRtaW46d3Jvbmc=\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}'

# 추측 쿠키로 보호 리소스 접근 시도
echo -en 'GET /admin/users.json HTTP/1.0\r\nHost: juice.el34.lab\r\nCookie: session=guess\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | head -1 | grep -oE '[0-9]{3}'
```
...
|---------|---------|--------------|--------------|-----------|------------|
| **① 다단계 admin** | `nc GET /admin/login` → fail → `nc /admin/login (admin:admin)` (15초 내) | flowbits ADM_VISIT setbit → ADM_BRUTE isset → alert | jq 로 src IP / flow id / timestamp 분석 | flowbits race (동시 connection 의 bit 충돌) | flow:to_server,established + flowbits:noalert 의 조합 |
