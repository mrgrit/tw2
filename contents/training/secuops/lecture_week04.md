...
curl -s -o /dev/null -w "%{http_code}\n" \
    ...
    -u admin:wrong http://10.20.30.1/admin/

curl -s -o /dev/null -w "%{http_code}\n" \
    -H "Host: juice.el34.lab" \
    -H "Cookie: session=guess" \
    http://10.20.30.1/admin/users.json
```
...
# attacker VM 내부 (학습 환경 한정)
curl -s -H "Host: juice.el34.lab" http://10.20.30.1/demo/leak.html | head -10
```
...
T+30s  운영자 의 분석 (Blue 2차)
       └→ jq 로 alert 분석:
          $ docker exec el34-siem sh -c "tail -100 /var/ossec/logs/alerts/alerts.json | \
