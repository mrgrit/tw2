...
```
[Step 1] ffuf 가 UA=FlowbitTest 로 GET / → 매치 (state set, noalert)
[Step 2] ffuf 가 같은 UA 로 GET /admin → step1 의 flag 가 켜진 conn 이라면 alert
```
...
해결: HTTP/1.1 keep-alive 보장 (browser·ffuf 는 connection 재사용이 기본) 또는 flowbits 대신
...
트리거: `ffuf -X POST -d 'rrn=901231-1234567' ...` 로 POST body 를 실제 전송하는 형식(lab §2 참조).
...
- **ffuf** — 웹 퍼저. wordlist 항목을 URL 의 FUZZ 자리에 대입해 `/admin/`·`/admin/users.json` 같은 admin endpoint 를 실제로 열거(디렉터리/경로 탐색)하고 401/403 응답을 확인한다. Burp Intruder 의 CLI 등가물.
...
attacker VM 내부에서 ffuf 로 admin 경로 2개를 같은 connection 에서 짧은 간격으로 열거한다.

```bash
# attacker VM 내부 (학습 환경 한정)
# ffuf 가 admin endpoint 2개(/admin/, /admin/users.json)를 FUZZ 에 대입해 실제 GET 요청 → 401/403 관찰
ffuf -u 'http://juice.el34.lab/FUZZ' -w <(printf 'admin/\nadmin/users.json\n') -mc all -s
```

각 옵션의 의미는 다음과 같다.

- `-u '.../FUZZ'` — FUZZ 자리에 wordlist 항목을 대입해 요청 경로를 만든다.
- `-w <(printf 'admin/\nadmin/users.json\n')` — 열거할 두 admin 경로 목록.
- `-mc all` — 모든 status code 를 결과로 표시(401/403 도 놓치지 않는다).
- `-s` — silent. 배너 없이 결과만 출력.

첫 번째 경로는 401, 두 번째도 정상이면 401 또는 403 을 받는다. ffuf 는 connection 을 재사용하므로 두 요청이 짧은 시간 안에 같은 src·같은 flow 에서 일어난다.
...
# attacker VM 내부 (학습 환경 한정) — ffuf 로 데모 endpoint 를 실제 요청해 응답 body 를 생성(Suricata 가 sniff)
ffuf -u 'http://juice.el34.lab/demo/leak.html?x=FUZZ' -w <(echo 1) -mc all -s
```

응답 body 에 `900101-1234567` 같은 형식 문자열이 포함되면(브라우저로 열어 확인) leak 시뮬이 성공이다. ffuf 요청이 만든 응답 body 를 ips 의 Suricata 가 sniff 한다.
...
attacker VM 에서 위 ffuf 요청을 다시 보낸 뒤 ips 에서 alert 발생 여부를 확인한다.
...
|---------|---------|--------------|--------------|-----------|------------|
| **① 다단계 admin** | `ffuf /admin/login` → 401 → 같은 flow 로 `/admin/login` 재접근 (15초 내) | flowbits ADM_VISIT setbit → ADM_BRUTE isset → alert | jq 로 src IP / flow id / timestamp 분석 | flowbits race (동시 connection 의 bit 충돌) | flow:to_server,established + flowbits:noalert 의 조합 |
