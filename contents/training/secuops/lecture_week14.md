...
각 장비에서 실행하며, osquery 는 `ssh ccc@10.20.32.80`(web),
...
포함한다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`)에서 실행하며, osquery·계정·cron
작업은 `ssh ccc@10.20.32.80`(web), sysmon 관측은 호스트의 `/var/log/syslog`, Wazuh 룰은
`ssh ccc@10.20.32.100`(siem) 로 한다.
