...
#### 시나리오: "Red 가 attacker 컨테이너에서 sqlmap UA + XSS payload 로 dvwa.6v6.lab 공격"

**Red — 공격 발생 (1초)**:

```bash
docker exec el34-attacker sh -c 'curl -s -o /dev/null -w "%{http_code}\n" -A "sqlmap/1.5" -H "Host: dvwa.6v6.lab" "http://10.20.30.1/?q=<script>alert(1)</script>&id=1+UNION+SELECT"'
```
...
```bash
docker exec el34-ips sh -c 'tail -5000 /var/log/suricata/eve.json | jq -c "select(.event_type==\"http\" and .http.hostname==\"dvwa.6v6.lab\") | {ts: .timestamp, src: .src_ip, url: .http.url, status: .http.status}" | tail -2'
```
...
```bash
docker exec el34-web sh -c 'grep " 403 " /var/log/apache2/dvwa_access.log | tail -5'
```
...
docker exec el34-web sh -c 'tail -500 /var/log/apache2/modsec_audit.log | jq -c "select(.response.status==403 and .request.headers.Host==\"dvwa.6v6.lab\") | {client:.transaction.remote_address, status:.response.status, ua:.request.headers.\"User-Agent\"}" | tail -1'
# 매치 룰 ID (error.log)
docker exec el34-web sh -c 'grep -oE "id \"[0-9]{6}\"" /var/log/apache2/dvwa_error.log | tail -6'
```
