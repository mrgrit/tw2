...
**핵심 명령** (el34 호스트에서):
```
ssh ccc@10.20.32.100 /var/ossec/bin/wazuh-control status    # daemon 상태
ssh ccc@10.20.32.100 /var/ossec/bin/agent_control -l        # 등록 agent 목록
```
...
ssh att@192.168.0.202  # 비밀번호: 1 → curl http://juice.el34.lab/ ...
...
```bash
ssh ccc@10.20.30.1 nft list tables
```
...
```bash
ssh ccc@10.20.30.1 nft list chain inet six_filter forward
```
...
```bash
ssh ccc@10.20.30.1 nft list table ip six_nat
```
...
ssh att@192.168.0.202 'for h in juice.el34.lab dvwa.el34.lab neobank.el34.lab govportal.el34.lab; do
  code=$(curl -sk -o /dev/null -w "%{http_code}" http://$h/)
  echo "$h: $code"
done'
```
...
```bash
ssh ccc@10.20.31.2 'sudo wc -l /var/lib/suricata/rules/suricata.rules 2>/dev/null || sudo wc -l /etc/suricata/rules/*.rules 2>&1 | tail'
```
...
```bash
ssh ccc@10.20.32.80 'ls /usr/share/modsecurity-crs/rules/ | wc -l; ls /usr/share/modsecurity-crs/rules/REQUEST-94*.conf'
```
...
```bash
ssh att@192.168.0.202 'curl -s -o /dev/null -w "XSS attempt: %{http_code}\n" "http://dvwa.el34.lab/?q=<script>alert(1)</script>"'
sleep 2
...
```bash
ssh ccc@10.20.32.100 'curl -sk -u admin:SecretPassword https://10.20.32.110:9200/_cluster/health | jq
```
...
```bash
ssh att@192.168.0.202 'curl -s -o /dev/null -w "%{http_code}\n" -A "sqlmap/1.5" "http://dvwa.el34.lab/?q=<script>alert(1)</script>&id=1+UNION+SELECT"'
```
...
```bash
ssh ccc@10.20.30.1 'conntrack -L 2>/dev/null | grep -E "dport=80|10.20.32.80" | tail -3'
```
...
```bash
ssh ccc@10.20.31.2 'tail -5000 /var/log/suricata/eve.json | jq -c "select(.event_type==\"http\" and .http.hostname==\"dvwa.el34.lab\") | {ts: .timestamp, src: .src_ip, url: .http.url, status: .http.status}" | tail -2'
```
...
```bash
ssh ccc@10.20.32.100 'tail -500 /var/ossec/logs/alerts/alerts.json | grep -E "web|10.20.30.202" | tail -2 | head -1' \
