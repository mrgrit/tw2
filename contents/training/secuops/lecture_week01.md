...
```bash
docker exec el34-attacker sh -c 'for h in juice.el34.lab siem.el34.lab portal.el34.lab bastion.el34.lab; do
...
```bash
docker exec el34-attacker sh -c 'curl -s -o /dev/null -w "%{http_code}\n" -A "sqlmap/1.5" -H "Host: dvwa.el34.lab" "http://10.20.30.1/?q=<script>alert(1)</script>&id=1+UNION+SELECT"'
```
...
```bash
docker exec el34-ips sh -c 'tail -5000 /var/log/suricata/eve.json | jq -c "select(.event_type==\"http\" and .http.hostname==\"dvwa.el34.lab\") | {ts: .timestamp, src: .src_ip, url: .http.url, status: .http.status}" | tail -2'
```
...
docker exec el34-web sh -c 'tail -500 /var/log/apache2/modsec_audit.log | jq -c "select(.response.status==403 and .request.headers.Host==\"dvwa.el34.lab\") | {client:.transaction.remote_address, status:.response.status, ua:.request.headers.\"User-Agent\"}" | tail -1'
# 매치 룰 ID (error.log)
