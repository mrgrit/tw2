...
5. attacker 컨테이너에서 가벼운 공격 (dalfox XSS / sqlmap 스캔) 을 발생시키고, 같은
...
| **ext** | `el34-attacker` | 192.168.0.202 | nmap, sqlmap, hydra, msfconsole 등 (내부 발판) | attack |
...
# 공격(Red) — 실제 외부 공격자 VM 에서 자연 URL 로 진짜 도구 실행
ssh att@192.168.0.202  # 비밀번호: 1 → 예) whatweb http://juice.el34.lab/ (도달성/핑거프린팅) · sqlmap/dalfox/ffuf 로 공격 ...
...
| 팀 | 책임 | 본 주차 활동 |
|----|------|-------------|
| **Red** | 공격 시뮬레이션 | attacker 에서 dalfox XSS / sqlmap 스캔 |
...
```bash
# dalfox 가 dvwa 에 XSS payload 를 발사 → ModSec 이 941 룰군으로 차단(403). 탐지가 목적이라 dalfox 출력의 WAF 감지 라인만 본다.
ssh att@192.168.0.202 "dalfox url --url 'http://dvwa.el34.lab/?q=FUZZ' 2>&1 | sed -r 's/\x1b\[[0-9;]*m//g' | grep -iaE 'WAF|XSS|403' | head -3"
...
**예상 출력**:
```
[*] Found WAF: ModSecurity (payload 차단 → 403)
941100
941160
```

dalfox 가 쏜 XSS payload 가 ModSec 941 룰군에 걸려 `403` 차단 → audit log 에 `941xxx` 룰 ID 매치.
...
#### 시나리오: "Red 가 attacker VM 에서 sqlmap 으로 dvwa.el34.lab SQLi 스캔"

**Red — 공격 발생 (수 초)**:

```bash
ssh att@192.168.0.202 "sqlmap -u 'http://dvwa.el34.lab/?id=1' --batch --level=2 --risk=2 --disable-coloring 2>&1 | grep -iE 'sqlmap|403|WAF/IPS' | head -3"
```

이 sqlmap 스캔은 자체 UA(`sqlmap/1.x`)와 SQLi 페이로드(UNION / OR tautology / 주석 변형)를 실어 여러 요청을
보내며, 다음 4 도구에 흔적을 남긴다. dvwa 는 ModSec 이 SQLi 룰군(942)으로 `403` 차단한다. (대상은
**차단형 vhost `dvwa`**. `juice` 는 per-vhost DetectionOnly 라 200 통과 — W02/W03 학습용이고, W01 의
차단 검증은 dvwa/neobank 등을 쓴다.)
...
**실측 결과** (el34, 2026-06):
```json
{"ts":"2026-06-20T13:01:16.591749+0000","src":"192.168.0.202","url":"/?id=1 AND 1=1 UNION SELECT NULL,NULL--","status":403}
```

`src` 가 **실제 공격자 192.168.0.202** 로 기록된다 — el34 는 fw 가 SNAT 하지 않아 출처 IP 가 보존된다.
...
**실측 결과** (el34, 2026-06):
```
192.168.0.202 - - [20/Jun/2026:12:53:10 +0000] "GET /?id=1%20AND%201%3D1%20UNION%20SELECT%20NULL,NULL-- HTTP/1.1" 403 438 "-" "sqlmap/1.7"
```
...
> client IP 가 **실제 공격자 192.168.0.202** 인 점에 주목 — el34 는 fw 가 SNAT 하지 않아 출처 IP 가
...
**실측 결과** (el34, 2026-06):
```json
{"client":"192.168.0.202","status":403,"ua":"sqlmap/1.7"}
```
```
id "942100"   id "942190"   id "942260"   id "942360"   id "949110"   id "980130"
```
...
본 케이스의 학습 목표는 attacker VM에서 sqlmap으로 SQLi를 시도하고, ModSecurity가 그 시도를 차단하면서 audit log를 남기고, 학생이 Wazuh Dashboard UI에서 그 audit log를 분석하는 것이다.
...
```bash
# attacker VM 내부 (학습 환경 한정) — sqlmap 이 username 파라미터에 OR 계열 tautology 를 주입
sqlmap -u 'http://dvwa.el34.lab/login' --data='username=admin&password=any' -p username --batch --level=2 --risk=2 --disable-coloring 2>&1 | grep -iE 'sqlmap|403|WAF/IPS|blocked' | head -3
```

sqlmap 이 tautology 페이로드를 보내면 ModSec 이 응답하는 status 를 출력에서 확인한다.

- ModSec이 차단하지 않으면 200 (정상 login 페이지 응답).
- ModSec이 차단하면 403 (sqlmap 은 `403 (Forbidden)` / `WAF/IPS` 로 보고).
