...
2. Defense in Depth 원리에 따라 el34 4-tier (`ext → pipe → dmz → int`) 가 어떻게
   설계되었는지 화이트보드에 재현한다.
3. el34 호스트(`ssh ccc@192.168.0.151`)에서 docker 로 보안 핵심 16 컨테이너 전부에
...
| 0:55–1:05   | 휴식                                                                  | —        |
| 1:05–1:35   | el34 4-tier 토폴로지 + Apache vhost 라우팅 + 패킷 흐름                 | 강의/토론 |
| 1:35–2:00   | 실습 1 — el34 호스트 접속 + 16 컨테이너 가시화                        | 실습     |
...
| **HAProxy** | High Availability Proxy | 대표적 L7 reverse proxy/LB 제품 (el34는 web 의 Apache 가 이 역할 대체) | 안내 데스크 (방 번호 라우팅) |
...
역프록시는 여러 제품으로 구현된다 — **Apache(mod_proxy)**, nginx, HAProxy, Traefik, Caddy 등. **el34 인프라에서는 web 컨테이너의 Apache 가 이 역프록시 역할을 한다.** 게다가 el34의 Apache는 역프록시인 동시에 **WAF(ModSecurity)** 도 겸한다 — 컨시어지가 곧 입구 금속탐지기까지 같이 보는 셈이다.

> **참고 — HAProxy 와의 관계.** 현장에서는 역프록시 전용 제품으로 HAProxy 를 많이 쓴다. 구(舊) 인프라(6v6)는 fw 앞단에 HAProxy 를 따로 두어 L7 라우팅을 시켰지만, **el34 는 이 L7 라우팅을 web 의 Apache vhost 로 통합**했다. 그래서 el34 에는 별도 HAProxy 가 없다 — **fw 는 순수 L3/L4 방화벽(nftables)** 이고, **web 의 Apache 가 L7 라우팅 + WAF** 를 함께 담당한다.
...
| 방문객의 의도 | `Host:` 헤더 (예: `juice.6v6.lab`) |
| 객실 안내 | vhost → 내부 app 컨테이너로 전달 |
| 의도 외 방문 거절 | WAF(ModSecurity) 차단 또는 404 |

학생이 web browser에서 `http://juice.6v6.lab` 을 입력했을 때 일어나는 흐름은 다음과 같다.

1. 학생 host의 DNS(또는 hosts 파일)가 `juice.6v6.lab` 을 **192.168.0.161** 로 해석한다.
...
5. **web 의 Apache**가 요청을 수신하고 `Host:` 헤더에서 `juice.6v6.lab` 을 확인한다.
...
**Reverse Proxy** 는 외부 요청을 받아서 내부 server로 전달하고, 응답을 다시 외부로 돌려주는 중계자다(앞 절의 web Apache 가 그 예다). nginx, HAProxy, Traefik, Caddy 등도 같은 역할을 한다.
...
| Host 헤더 | 응답하는 사이트 | 용도 |
|----------|----------------|------|
| `siem.6v6.lab` | Wazuh Dashboard | 운영자/학생의 분석 |
| `portal.6v6.lab` | 운영자 portal | 운영자 접근 |
| `web.6v6.lab` | Juice Shop | 공격 target |
...
ssh -J ccc@192.168.0.151:2204 ccc@<내부-server>
```
...
ssh ccc@192.168.0.151              # 비밀번호: 1
docker exec -it el34-web bash      # 원하는 컨테이너 이름으로
```
...
el34 는 타깃 VM (192.168.0.151) 1대 안에 docker 컨테이너로 위 4 계층을 모사한다. 이름 매핑:
...
**el34 구현**:
- 운영체제: Ubuntu 22.04
- 도구: **nftables** (Linux 커널 표준, iptables 후속) — filter(`six_filter`) + NAT(`six_nat`)
- 역할: 공인 IP **192.168.0.161** 의 80 / 443 / 9100 을 내부로 **DNAT**. L7 host-header 라우팅은 web 의 Apache 가 담당하므로 **el34 fw 엔 HAProxy 가 없다**
...
el34 의 외부 진입점은 **공인 IP 192.168.0.161** (web 트래픽) 과 **192.168.0.151** (관리 SSH) 두 개다.
...
> 트레이닝 실습에서는 위 외부 포트 대신, 호스트 SSH(`ssh ccc@192.168.0.151`)로 접속해
> `docker exec` 로 각 컨테이너에 진입한다. 외부 포트(80/443/9100/2204)는 학생·운영 web 접근과
> 외부 공격 트래픽용이다.
...
| `juice.6v6.lab` | int juiceshop | ModSec 적용 (WAF) |
| `dvwa.6v6.lab` | int dvwa | ModSec 적용 |
| `neobank.6v6.lab` | int neobank | ModSec 적용 |
| `govportal.6v6.lab` | int govportal | ModSec 적용 |
| `mediforum.6v6.lab` | int mediforum | ModSec 적용 |
| `admin.6v6.lab` | int adminconsole | ModSec 적용 |
| `ai.6v6.lab` | int aicompanion | ModSec 적용 |
| `siem.6v6.lab` | dmz wazuh-dashboard:5601 | 운영 트래픽 (ModSec 완화) |
| `portal.6v6.lab` | dmz portal:8000 | 운영 트래픽 (ModSec 완화) |
| `bastion.6v6.lab` | ext bastion:9100 | 운영 트래픽 (ModSec 완화) |

> 도메인은 구 인프라의 `*.6v6.lab` 명명을 el34 에서도 그대로 유지한다. 학생 host 의 hosts 파일
> 또는 DNS 가 이들을 **192.168.0.161** 로 가리키게 설정한다.

### 3.5 web Apache 의 L7 vhost 라우팅

el34 에는 HAProxy 가 없다. fw(nftables)가 .161 의 80/443 을 web 으로 forward 하면, **web 의 Apache 가
`Host:` 헤더로 vhost 를 선택**해 내부 대상으로 reverse-proxy 한다. 구 인프라(6v6)에서 HAProxy 가 하던
6-backend 라우팅을, el34 는 이 Apache vhost 로 통합했다.

```mermaid
graph TB
    IN["Apache listener<br/>web :80 / :443"]
    IN -->|"Host: juice.6v6.lab"| W1["juiceshop (int) · ModSec ON"]
    IN -->|"Host: dvwa / neobank 등"| W2["기타 취약앱 (int) · ModSec ON"]
    IN -->|"Host: siem.6v6.lab"| OPS1["wazuh-dashboard :5601 · ModSec 완화"]
    IN -->|"Host: portal.6v6.lab"| OPS2["portal :8000 · ModSec 완화"]
    IN -->|"Host: bastion.6v6.lab"| OPS3["bastion :9100 · ModSec 완화"]
...
el34 에서는 두 트래픽 모두 **fw → ips → web → 대상** 의 같은 경로를 탄다 (HAProxy 가 없으니
web 의 Apache 를 항상 거친다). 차이는 web 에서 **ModSecurity 엔진을 적용하느냐** 다.

**학생 트래픽 (`juice.6v6.lab`) — ModSec 적용**:
```mermaid
graph LR
...
차단되는 사고를 방지. (구 인프라 6v6 는 HAProxy 가 운영 트래픽을 web 으로 보내지 않아 hop 수가
달랐지만, el34 는 경로가 같고 ModSec 적용 여부로 구분한다.)

### 3.7 el34 접근 모델 — 호스트 SSH + docker exec

el34 는 모든 보안 컨테이너가 **타깃 VM 192.168.0.151** 한 대 위에서 돈다. 트레이닝 실습의
표준 접근은 호스트에 SSH 로 들어간 뒤 `docker exec` 로 원하는 컨테이너에 진입하는 것이다.

```bash
ssh ccc@192.168.0.151                 # 비밀번호: 1
docker ps --format '{{.Names}}'       # 컨테이너 목록 확인
docker exec -it el34-fw   bash        # 방화벽
docker exec -it el34-ips  bash        # IDS
docker exec -it el34-web  bash        # WAF
docker exec -it el34-siem bash        # SIEM
```
...
> 트레이닝은 단일 호스트 위 docker 구조라, 더 단순·확실한 `호스트 SSH + docker exec` 경로를 표준으로
...
> 컨테이너를 타깃 VM(192.168.0.151) 한 대에 모아두었고, 외부 진입점(bastion, :2204)을 두어
> ISMS-P 2.5 (인증·권한관리) 의 single point of access 원칙을 모사한다.
>
> **이걸 하면 무엇을 알 수 있는가?**
> - 4-tier 토폴로지의 실제 컨테이너 IP·이름·역할 매핑
> - 호스트 SSH + `docker exec` 접근 모델 (§3.7)
...
**Step 1.4 — 4 핵심 컨테이너 진입 검증 (호스트에서 docker exec)**

```bash
# el34 호스트에서
for h in fw ips web siem; do
  echo "--- el34-$h ---"
  docker exec el34-$h hostname
done
```
...
4 컨테이너 모두 hostname 응답 → docker exec 접근 정상.
...
> bastion :2204)로만 가능하고, 컨테이너 작업은 호스트에 들어와 `docker exec` 로 한다.
...
> (el34 fw 엔 HAProxy 가 없고, L7 라우팅은 그 뒤 web 의 Apache 가 한다.)
>
> **이걸 하면 무엇을 알 수 있는가?**
...
```bash
docker exec el34-fw nft list tables
```
...
```bash
docker exec el34-fw nft list chain inet six_filter forward
```
...
el34 fw 는 HAProxy 대신 nftables `six_nat` 으로 공인 .161 의 포트를 내부로 전달한다.

```bash
docker exec el34-fw nft list table ip six_nat
```
...
```bash
docker exec el34-attacker sh -c 'for h in juice.6v6.lab siem.6v6.lab portal.6v6.lab bastion.6v6.lab; do
...
```bash
docker exec el34-ips sh -c 'pgrep -a Suricata; echo "---"; sudo suricatasc -c version; sudo suricatasc -c uptime'
```
...
```bash
docker exec el34-ips sh -c 'sudo tail -500 /var/log/suricata/eve.json | jq -r .event_type | sort | uniq -c | sort -rn'
```
...
```bash
docker exec el34-ips sh -c 'sudo wc -l /var/lib/suricata/rules/suricata.rules 2>/dev/null || sudo wc -l /etc/suricata/rules/*.rules 2>&1 | tail'
```
...
```bash
docker exec el34-ips sh -c 'sudo suricatasc -c dump-counters 2>&1 | jq ".message | {pkts: .[\"capture.kernel_packets\"], drops: .[\"capture.kernel_drops\"], alerts: .[\"detect.alert\"]}"'
```
...
```bash
docker exec el34-web sh -c 'sudo apache2ctl -M 2>&1 | grep -iE "security|proxy|ssl"'
```
...
```bash
docker exec el34-web sh -c 'sudo grep -E "^SecRuleEngine|^SecAuditLog|^SecAuditEngine|^SecAuditLogFormat" /etc/modsecurity/modsecurity.conf'
```
...
```bash
docker exec el34-web sh -c 'ls /usr/share/modsecurity-crs/rules/ | wc -l; ls /usr/share/modsecurity-crs/rules/REQUEST-94*.conf'
```
...
```bash
docker exec el34-attacker sh -c 'curl -s -o /dev/null -w "XSS attempt: %{http_code}\n" -H "Host: dvwa.6v6.lab" "http://10.20.30.1/?q=<script>alert(1)</script>"'
sleep 2
docker exec el34-web sh -c 'sudo tail -3 /var/log/apache2/modsec_audit.log | head -1 | jq -r ".transaction.messages[]? | select(.id | startswith(\"941\")) | .id" | head -3'
```
...
```bash
docker exec el34-siem sh -c 'sudo /var/ossec/bin/agent_control -l'
```
...
```bash
docker exec el34-siem sh -c 'sudo ss -tlnp | grep -E ":(1514|1515|55000)"'
```
...
```bash
docker exec el34-wazuh-indexer curl -sk -u admin:SecretPassword https://localhost:9200/_cluster/health | jq
```
...
```bash
docker exec el34-siem sh -c 'sudo tail -3 /var/ossec/logs/alerts/alerts.json | tail -1 | jq "{ts: .timestamp, agent: .agent.name, rule_id: .rule.id, level: .rule.level, desc: .rule.description}"'
```
...
```bash
docker exec el34-attacker sh -c 'curl -s -o /dev/null -w "%{http_code}\n" -A "sqlmap/1.5" -H "Host: juice.6v6.lab" "http://10.20.30.1/?q=<script>alert(1)</script>&id=1+UNION+SELECT"'
```
...
```bash
docker exec el34-fw sh -c 'conntrack -L 2>/dev/null | grep -E "dport=80|10.20.32.80" | tail -3'
```
...
```bash
docker exec el34-ips sh -c 'sudo tail -300 /var/log/suricata/eve.json | jq -c "select(.event_type==\"http\" and .http.hostname==\"juice.6v6.lab\") | {ts: .timestamp, src: .src_ip, url: .http.url, status: .http.status}" | tail -2'
```
...
```bash
docker exec el34-web sh -c 'sudo tail -5 /var/log/apache2/juice_access.log'
```
...
> el34 의 Apache 는 11 vhost 별 별도 access/error log (예: `juice_access.log`, `juice_error.log`,
> `dvwa_access.log` 등). 단일 `access.log` 가 아니라 vhost 별 분리되어 있어 분석이
> 정밀하다. client IP 가 직전 hop (10.20.32.1 = ips 의 dmz NIC) 인 이유 — Apache 의 첫 컬럼은
> 직전 hop 이기 때문. 내부 attacker(el34-attacker)는 fw 의 NAT 를 거치며 출처가 hop IP 로 바뀐다.

**(4) web modsec_audit.log — JSON 형식**:

> el34 의 ModSec audit log 는 `SecAuditLogFormat JSON` + `SecAuditLogParts ABCFHZ`
...
```bash
docker exec el34-web sh -c 'sudo tail -3 /var/log/apache2/modsec_audit.log | head -1 | jq "{client: .transaction.remote_address, code: (.response.status // .response.http_code), rules: [.audit_data.messages[]? | capture(\"id \\\"(?<id>[0-9]+)\\\"\") | .id]}"'
```
...
`client: 10.20.32.1` 은 ips 의 dmz NIC (web 직전 hop). 내부 attacker(el34-attacker)는 fw 의
NAT 를 거치므로 web 입장에서 출처가 hop IP 로 보인다. **반면 외부 공격자 VM(192.168.0.202)이
공인 .161 을 공격하면 출처 IP `192.168.0.202` 가 ips·web·siem 전 계층에 그대로 보존된다** — 이것이
el34 가 구 인프라(6v6)의 gateway-src 문제를 개선한 핵심이다(W03 에서 심화).
...
```bash
docker exec el34-siem sh -c 'tail -500 /var/ossec/logs/alerts/alerts.json | grep -E "web|10.20.30.202" | tail -2 | head -1' \
...
- **Wazuh** — open-source SIEM이다. el34 에서는 manager (`el34-siem` 컨테이너, 호스트 192.168.0.151) 와 각 컨테이너에 설치된 agent로 구성된다.
...
외부 공격자 VM 내부에서 el34 호스트(192.168.0.151)의 SSH에 10번 시도한다. 학습 환경 안에서만 실행해야 한다.
...
for i in $(seq 1 10); do
    sshpass -p "wrong${i}" ssh -o ConnectTimeout=3 \
        ...
        admin@192.168.0.151 'whoami' 2>/dev/null
done
```
...
- URL: `https://siem.6v6.lab` (공인 .161 경유; 직접 접근은 내부 `el34-wazuh-dashboard:5601`).
...
- URL: `https://siem.6v6.lab` (Wazuh Dashboard와 같은 instance다).
...
```
el34 호스트
  → docker exec -it el34-fw bash       (fw 컨테이너 진입)
...
```bash
# attacker VM 내부 (학습 환경 한정)
curl -G "http://192.168.0.161/login" \
...
5. **el34 접근** — 호스트 SSH + `docker exec` 로 16 컨테이너 진입. 외부에서 내부 IP 직접 접속 불가.
