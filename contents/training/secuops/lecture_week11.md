...
1. **스냅샷(osquery)** 과 **이벤트 스트림(sysmon)** 의 본질적 차이를 비유 없이 1분 안에
   ...
   그것을 잡는지 근거를 댄다.
2. el34 의 sysmon 이 **왜 컨테이너가 아니라 호스트(192.168.0.151)에 설치** 됐는지를
...
## 0.5 신입생 친화 핵심 개념 — "스냅샷은 사진, 이벤트는 녹화"
...
el34 에서는 호스트(192.168.0.151)에 Sysmon for Linux 가 돌며, 프로세스 생성·네트워크
...
el34 에는 **버전 1.5.2** 가 호스트(192.168.0.151)에 설치돼 systemd 서비스(`sysmon`)로
가동된다.
...
```mermaid
graph TD
    HK["el34 호스트 커널 (192.168.0.151)<br/>커널 6.8 · root · 인터넷"]
...
| el34 위치 | 호스트(192.168.0.151) | 컨테이너(el34-web) 내부 |
...
호스트(`ssh ccc@192.168.0.151`)의 `/var/log/syslog` 로 한다. **sysmon 데몬 자체는 영속
