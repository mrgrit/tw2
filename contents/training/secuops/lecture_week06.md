...
ssh ccc@10.20.32.80 'sudo osqueryi ".tables" | head -20'
...
```bash
# 1. 새 사용자 추가
ssh ccc@10.20.32.80 'sudo useradd -m -s /bin/bash -u 1099 fakeintruder'
...
**Cleanup (실험 종료)**:
```bash
ssh ccc@10.20.32.80 'sudo userdel -r fakeintruder'
...
| **② backdoor cron** | `echo '* * * * * root /tmp/.bd.sh' > /etc/cron.d/sync` | `SELECT command, path FROM crontab WHERE command LIKE '%curl%bash%'` | file_events 의 /etc/cron.d/* 의 INSERT | systemd timer / at job 의 미탐지 (cron 만 검사) | crontab + startup_items + systemd_units 의 3-source JOIN |
