...
5. APT 5 단계(대응)에서 el34 가 **SNAT 를 하지 않아 출처 IP(10.20.30.202)가 전 계층에
...
기말도 시험이지만, 본 강의 시간은 시험 직전의 **14주 통합 복습 + 5 단계 판단 프레임워크
정립 + 실기 평가**로 구성한다.

| 시간        | 내용                                                                       | 유형     |
|-------------|----------------------------------------------------------------------------|----------|
| 0:00–0:20   | 이론 — APT 란 무엇인가, 왜 한 단계만 막으면 안 되는가 (다층 방어 총정리)     | 강의     |
| 0:20–0:55   | 이론 — 5 단계 킬체인 + 각 단계의 장비 매핑(14주의 무기를 한 캠페인에)        | 강의     |
| 0:55–1:05   | 휴식                                                                        | —        |
| 1:05–1:35   | 이론 — 출처 IP 보존(SNAT 없음)으로 5 단계를 한 캠페인으로 엮기 + 인텔 격상   | 강의/토론 |
| 1:35–2:00   | 실기 — 점검(전 무기) + ① 정찰(WAF 913) + IDS 룰(sid 9015001) (lab 1–3)      | 시험     |
| 2:00–2:30   | 실기 — ② 웹 익스플로잇(SQLi 403) + WAF anomaly(942→949110) (lab 4–5)        | 시험     |
| 2:30–2:40   | 휴식                                                                        | —        |
| 2:40–3:10   | 실기 — ③ 발판(리스너·계정)+osquery 헌팅 + ④ C2 sysmon + 인텔 룰 (lab 6–9)    | 시험     |
| 3:10–3:30   | 실기 — ⑤ SIEM 수렴·출처 상관 + APT IR 보고서 + 정리 확인 (lab 10–12)        | 시험     |
| 3:30–3:40   | 정리 + 수료 안내 + 보안운영자로서의 다음 단계                               | 정리     |
...
이 시험의 APT 는 외부 공격자(`el34-attacker`, 출처 IP `10.20.30.202`)가 **일관된 출처** 로
5 단계를 진행하는 시나리오다.
...
이번 시험의 시나리오는 한 APT 행위자(`el34-attacker`, 출처 IP `10.20.30.202`)가 fw 의
게이트웨이(`10.20.30.1`)를 통해 `dvwa.el34.lab` vhost 를 노려 5 단계를 진행하는 것이다.
el34 는 SNAT 를 하지 않으므로 출처 IP `10.20.30.202` 가 모든 단계·모든 계층에 그대로
보존된다(§4).
...
IP(`10.20.30.202`)를 가리킴을 확인한다. 그리고 출처 IP + 시각으로 5 단계를 한 캠페인
...
graph TD
    ...
    KEY["출처 10.20.30.202<br/>+ 시각<br/>→ 한 APT 캠페인 타임라인"]
...
| ② 침투 | SQLi(UNION SELECT) | WAF(942 → 949110, 403) | audit 403 + remote 10.20.30.202 |
...
| ⑤ 대응 | — | SIEM 수렴 + 출처 상관 | alerts.json 다계층, src 10.20.30.202 |
...
```mermaid
graph TD
    P1["① 캠페인 개요<br/>출처 10.20.30.202, 5 단계 킬체인"]
...
graph TD
    ...
    A["IDS eve.json<br/>src_ip = 10.20.30.202"]
    B["WAF modsec_audit<br/>remote_address = 10.20.30.202"]
...
fw 가 SNAT 를 하지 않으므로, 안쪽의 모든 장비가 APT 의 **진짜 출처 IP `10.20.30.202`** 를
...
```bash
docker exec el34-fw sh -c 'sudo nft list ruleset 2>/dev/null | grep -c dnat'   # DNAT 규칙 수
```
...
EOF"'
docker exec el34-ips sh -c 'sudo suricata -T -S /etc/suricata/rules/local.rules'   # 문법 검사(-T 테스트, -S 룰 지정)
docker exec el34-ips sh -c 'sudo suricatasc -c reload-rules'                        # 무중단 reload
# 트리거 후: /var/log/suricata/eve.json 에서 sid 확인
docker exec el34-ips sh -c 'sudo sed -i "/sid:9015001/d" /etc/suricata/rules/local.rules; sudo suricatasc -c reload-rules'   # 정리(베이스 보존)
```
...
```bash
docker exec el34-web sh -c 'sudo tail -80 /var/log/apache2/modsec_audit.log | grep -oE "94[29]1[0-9]{2}" | sort -u'
```
...
echo '{"apt_ioc":"apt15-c2","src_ip":"10.20.30.202"}' | docker exec -i el34-siem sudo /var/ossec/bin/wazuh-logtest
...
docker exec el34-siem sh -c 'tail -400 /var/ossec/logs/alerts/alerts.json | jq -rc "select(.rule.groups|index(\"ids\"))|.data.src_ip" | sort | uniq -c'   # 출처별 수렴
```

무엇을 보나 — ips·web agent 의 경보가 한 manager 로 수렴하고, 그 출처가 모두 같은 APT
IP(10.20.30.202)인지. 이것이 5 단계를 한 캠페인으로 보는 통합 가시성의 실체다.
...
> 남는지 본다.
>
> **무엇을 알 수 있는가?** APT(10.20.30.202)의 `sqlmap` UA 요청이 WAF 의 CRS **913 룰군
...
> 한 캠페인에 엮이는지 확인한다(§4).
>
> **무엇을 알 수 있는가?** Wazuh `alerts.json` 의 ids 그룹에 출처 `10.20.30.202` 가 (다단계)
> 수렴함. el34 가 SNAT 를 안 하므로 출처가 전 계층에 보존된다는 사실.
>
> **결과 해석.** 정상: alerts.json 에 출처 10.20.30.202 가 수렴. 비정상: 출처가 fw IP 로
