...
| 시간      | 내용                                                                       | 유형      |
|-----------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25 | 이론 — 왜 평결을 "만드는 것" 만으로는 관제가 아닌가 (조종석의 동기)         | 강의      |
| 0:25–0:55 | 이론 — dashboard / indexer 구조 + 5개 패널 + 가시화 파이프라인              | 강의      |
| 0:55–1:05 | 휴식                                                                       | —         |
| 1:05–1:35 | 이론 — FIM(syscheck) realtime vs 주기 + 변조 격상 + Active Response 설계    | 강의/토론 |
| 1:35–2:00 | 실습 1, 2 — 조종석 3컴포넌트 점검 + syscheck 감시 경로 확인                 | 실습      |
| 2:00–2:30 | 실습 3, 4 — FIM 실시간 탐지(rule 554) + 격상 룰(101010) logtest 검증        | 실습      |
| 2:30–2:40 | 휴식                                                                       | —         |
| 2:40–3:10 | 실습 5, 6, 7 — Active Response 점검 + 다소스 수렴 + web agent ship          | 실습      |
| 3:10–3:30 | 실습 8, 9 — 조종석 종합 보고서 + 정리(베이스 보존) 확인                     | 실습      |
| 3:30–3:40 | 정리 + 과제 안내 + 다음 주차(W11) 예고                                      | 정리      |
...
## 0.5 신입생 친화 핵심 개념 — "조종석은 보고·잡고·되받아치는 자리다"
...
- **서고에 쌓인 책** = manager 가 만든 평결(`alerts.json`) 자체. 분명히 존재하지만, 그
  자체로는 "어제 10.20.30.202 가 일으킨 고위험 사건만 보여줘" 같은 질문에 즉답할 수 없다.
...
```bash
docker exec el34-wazuh-indexer sh -c \
...
docker exec el34-web sh -c 'echo "w10-fim-test" > /home/ccc/w10_canary.txt'
sleep 12
...
docker exec el34-web sh -c 'rm -f /home/ccc/w10_canary.txt'   # self-clean
```
...
삭제)**. `sleep 12` 는 변경이 syscheck → analysisd 를 거쳐 `alerts.json` 에 반영되기까지의
...
docker exec el34-siem sh -c 'ls -la /var/ossec/logs/active-responses.log 2>/dev/null || echo "(AR 로그 — 트리거 시 기록)"'
```
...
대로 el34 는 fw 가 SNAT 하지 않아 **출처 IP(내부 공격자 10.20.30.202)가 전 계층에 보존** 되므로,
...
```bash
docker exec el34-web sh -c '/var/ossec/bin/wazuh-control status 2>/dev/null | grep -E "agentd|modulesd|syscheckd"'
```
...
각 실습은 **4축 설명** 을 포함한다. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서
`docker exec el34-<comp>` 로 실행한다. 커스텀 룰은 **logtest 로드 검증만**, Active Response 는
...
> - 출처 IP(10.20.30.202) 보존으로 두 소스를 한 사건에 묶는 공통 키
