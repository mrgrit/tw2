...
ssh ccc@10.20.32.100 'ls -la /var/ossec/logs/active-responses.log 2>/dev/null || echo "(AR 로그 — 트리거 시 기록)"'
```
...
```bash
ssh att@192.168.0.202 "curl -s -o /dev/null \"http://dvwa.el34.lab/?id=w10%27%20UNION%20SELECT%201--%20-\""
sleep 8
...
```bash
ssh ccc@10.20.32.80 '/var/ossec/bin/wazuh-control status 2>/dev/null | grep -E "agentd|modulesd|syscheckd"'
```
