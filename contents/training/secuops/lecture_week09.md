...
echo 'Jan  1 00:00:00 web sshd[1]: Failed password for root from 9.9.9.9 port 22 ssh2' \
  | ssh ccc@10.20.32.100 sudo /var/ossec/bin/wazuh-logtest
```
...
rule 86601). 출력의 `s:"192.168.0.202"` 가 **출처가 보존됐다는 증거** 다. 조건 폴링(로그 흔적 대기) 은
...
> 조건 폴링(로그 흔적 대기) 후에도 안 보이면 ips agent 수집 또는 analysisd 를 점검.
>
> **실전 활용**
