...
5. 외부 공격자의 SQLi 를 재현해 ips(Suricata) → manager → `alerts.json` 의 **ids 그룹** 으로
   ingest 되는 전 과정을 추적하고, 출처 IP(192.168.0.202)가 그대로 **보존** 됨을 확인한다.
...
|------|-----------|------|------------------|
| fw | conntrack | 텍스트 | `tcp 6 ... src=192.168.0.202 dst=...` |
| ips | `/var/log/suricata/eve.json` | JSON | `{"event_type":"alert","src_ip":"...","alert":{...}}` |
| web (access) | `/var/log/apache2/dvwa_access.log` | combined | `192.168.0.202 - - [..] "GET /?id=..." 403` |
| web (WAF) | `/var/log/apache2/modsec_audit.log` | JSON | `{"transaction":{...},"response":{...}}` |

이 상태로는 "어제 192.168.0.202 가 일으킨 모든 사건을 심각도순으로 보여줘" 같은 질의가
...
echo 'Jan  1 00:00:00 web sshd[1]: Failed password for root from 9.9.9.9 port 22 ssh2' \
  | ssh ccc@10.20.32.100 /var/ossec/bin/wazuh-logtest
```
...
상관이 깨진다. **el34 는 fw 가 SNAT 하지 않아 출처 IP(내부 공격자 192.168.0.202, 외부 공격자
...
ssh att@192.168.0.202 'curl -s -A sqlmap/1.7  \
  "http://dvwa.el34.lab/?id=1%27%20UNION%20SELECT%201,2--%20-"'
sleep 8
...
#   → {"r":"86601","d":"Suricata: ... UNION SELECT","s":"192.168.0.202"}
```
...
rule 86601). 출력의 `s:"192.168.0.202"` 가 **출처가 보존됐다는 증거** 다. `sleep 8` 은
...
```bash
ssh ccc@10.20.32.80 'grep -A1 localfile /var/ossec/etc/ossec.conf | grep log_format'
#  web agent: apache(modsec audit) · json · syslog · full_command
ssh ccc@10.20.31.2 'grep -A1 localfile /var/ossec/etc/ossec.conf | grep log_format'
...
각 실습은 **4축 설명** 을 포함한다. 각 장비에 직접 접속(fw 10.20.30.1/ips 31.2/web 32.80/siem 32.100, 공격 192.168.0.202, root은 sudo)해 실행한다.
...
> - 출처 IP(192.168.0.202)가 전 계층에 보존됨 → W08 상관과 연결
>
> **결과 해석**
