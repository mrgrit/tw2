...
5. **enrichment(평판/지역) + 빈도(반복)** 를 **결합** 하면 같은 경보가 P3 → P2 → P1 로
   ...
   **같은 출처 IP(192.168.0.202)로 보존되어 누적** 됨을 데이터로 확인한다.
...
전 계층에 그대로 보존된다(내부 공격자 192.168.0.202, 외부 공격자 192.168.0.202). 그래서 외부
...
#   →   N  "192.168.0.202"   (같은 출처가 N건 누적 = 빈도 상관의 입력)
```

출력의 `192.168.0.202` 가 N 건으로 묶여 나오는 것이 "출처가 보존되어 같은 공격자로 인식됐다"
...
(각 장비에 직접 접속(fw 10.20.30.1/ips 31.2/web 32.80/siem 32.100, 공격 192.168.0.202, root은 sudo)해 실행한다). 격상 룰은 **logtest 로만 검증**(라이브 restart 금지)
...
>
> **이걸 하면 무엇을 알 수 있는가?**
> - 같은 출발지(192.168.0.202)에서 sqlmap 을 반복하면 web 의 modsec audit 에 여러 건 남음
...
> - 출처 IP(192.168.0.202)가 보존되어 같은 출발지로 N 건 묶임 → 빈도의 입력원
>
> **결과 해석**
> 정상: alerts.json ids 그룹에서 `192.168.0.202` 가 여러 건으로 집계. 조건 폴링(로그 흔적 대기) 후에도 안
