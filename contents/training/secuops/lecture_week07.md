...
`ssh ccc@10.20.32.80`(web victim)에 접속해 osquery 를 던지는** 단순·확실한 경로를 쓴다. 즉 한 사람이
...
```bash
ssh ccc@10.20.32.100 'tail -300 /var/ossec/logs/alerts/alerts.json | grep -iE "syscheck|integrity" | tail -3'
```
...
| "osquery 결과가 비어 있다" | 테이블/컬럼명 오타, 일부는 root 필요 | `sudo osqueryi` 로 질의(root 필요 시), `.schema <table>` 로 컬럼 확인(W06) |
