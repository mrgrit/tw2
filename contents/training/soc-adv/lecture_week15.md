...
**출처 IP**다. el34는 fw가 SNAT를 하지 않아 출처 `192.168.0.202` 가 네트워크·웹·호스트·SIEM 전 계층에 보존
...
| **192.168.0.202** | 출처 IP | el34 공격자(전 계층 보존, 통합 열쇠) |
...
```mermaid
graph TD
    SRC["출처 IP 192.168.0.202<br/>(SNAT 없음 → 전 계층 보존)"]
...
```bash
# el34-ips: 네트워크 흔적
tail -3000 /var/log/suricata/eve.json | grep -c 192.168.0.202     # → 2935
# el34-siem: SIEM 흔적
tail -2000 /var/ossec/logs/alerts/alerts.json | grep -c 192.168.0.202  # → 1772
```

같은 IP `192.168.0.202` 가 네트워크 2935건+SIEM 1772건으로 양 계층에 보인다 = '네트워크에서 스캔하던 그 IP가
