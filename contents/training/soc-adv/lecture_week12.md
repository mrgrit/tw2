...
> (`Failed password ... from 192.168.0.202` → `srcip=192.168.0.202`). **정규화**는 서로 다른 로그의 필드를
...
원시 로그: `Failed password for root from 192.168.0.202 port 22 ssh2`

| 단계 | 결과 |
|------|------|
| **파싱**(디코더) | `dstuser=root`, `srcip=192.168.0.202`, `program=sshd` 로 **추출** |
...
```mermaid
graph TD
    RAW["원시 로그 한 줄<br/>'Failed password for root<br/>from 192.168.0.202'"]
    FMT["포맷 인식<br/>syslog / json / command"]
    DEC["디코더<br/>패턴으로 필드 추출"]
    FIELDS["필드<br/>srcip=192.168.0.202<br/>dstuser=root<br/>program=sshd"]
    RAW --> FMT --> DEC --> FIELDS
...
원시 텍스트가 `dstuser=root`·`srcip=192.168.0.202` 로 분해됐다 = 디코더가 동작한다는 뜻. **룰은 이 분해된
...
```mermaid
graph TD
    L1["ssh 로그<br/>'from 192.168.0.202'"]
    L2["웹 로그<br/>'client 192.168.0.202'"]
    L3["IPS eve.json<br/>'src_ip: 192.168.0.202'"]
    NORM["정규화<br/>모두 srcip=192.168.0.202"]
...
세 소스가 같은 공격자(192.168.0.202)를 각자 다른 표현으로 기록한다. **정규화**가 이를 모두 `srcip` 으로
...
el34 호스트에서 `ssh ccc@10.20.32.100` 로. **인가된 실습 환경(el34)에서만**, 디코더·logtest는 읽기 전용.
