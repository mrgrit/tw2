...
```
   1945 "src_ip":"192.168.0.202"   ← top talker(이상치 후보 = 공격자)
    991 "src_ip":"10.20.32.80"
     44 "src_ip":"10.20.31.1"
      3 "src_ip":"10.20.31.254"   ← 베이스라인(정상은 대부분 소량)
```

192.168.0.202가 압도적으로 폭주 = 베이스라인 이탈 = 이상치다. **룰을 하나도 안 쓰고** "평소와 다름"만으로
...
| **192.168.0.202** | 폭주 출처 | el34 공격자 컨테이너 |
...
대부분의 IP는 수십 건 이하인데 `192.168.0.202` 가 1945건으로 압도적 = **top talker(이상치)**. 시그니처 룰이
