...
진짜 행위를 드러낸다. 그 전에 정적으로 `system()`·`wget http://...` 같은 행위 지표를 미리 뽑아 "샌드박스에서
...
el34-attacker(`ssh att@192.168.0.202`)에서. **인가된 실습 환경(el34)에서만**, 검체는 모의·격리·self-clean.
...
- **무엇을**: strings에서 `system|exec|wget|http` 지표.
