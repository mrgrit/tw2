...
| 시간        | 내용                                                                | 유형      |
|-------------|---------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 자동화인가(알림 과부하), SOAR 정의                        | 강의      |
| 0:25–0:55   | 이론 — Wazuh active-response·액션 카탈로그·트리거                   | 강의      |
| 0:55–1:05   | 휴식                                                                 | —         |
| 1:05–1:35   | 이론 — 플레이북 5단계·사람 검토·MTTR                                | 강의/토론 |
| 1:35–2:10   | 실습 — SOAR 엔진/액션 확인 + 트리거 정의                             | 실습      |
| 2:10–2:40   | 실습 — 플레이북 설계 + 드라이런 매핑                                 | 실습      |
| 2:40–2:50   | 휴식                                                                 | —         |
| 2:50–3:20   | 실습 — 사람 검토 + MTTR + 보고서                                     | 실습      |
...
| **active-response** | — | Wazuh의 알림 기반 자동 액션 | 경보 연동 자동 셔터 |
...
`<command>`가 액션을 등록하고, `<active-response>`가 "어떤 룰/레벨에서 어떤 액션을, 어디서, 얼마 동안"을
정의한다. 알림이 트리거 조건을 만족하면 해당 액션이 자동 실행된다 — 이것이 플레이북의 "실행" 단계다.
...
**보강**은 W05 CTI와 직결된다 — 출처 IP를 CDB list로 조회해 "알려진 악성인가"를 자동 판단한다. **판단**은
악성+고위험이면 자동 실행으로, 모호하면 분석가 큐로 분기한다. **실행**은 차단뿐 아니라 티켓 생성·알림까지
오케스트레이션한다. **검토**는 사람이 결과를 확인하고 오탐 시 롤백한다.

---

## 4. 사람 검토 · MTTR 측정

**사람 승인 게이트(HITL).** 모든 걸 자동화하면 오탐 한 번이 재앙이 된다. 액션의 **위험도·가역성**에 따라
게이트를 둔다.
...
graph TD
    ...
    ACT --> M["중위험<br/>(IP 임시 차단)<br/>→ 자동 + 사후 검토"]
...
종결 비율) 상승, 일관성. 이 지표로 플레이북을 개선하는 순환이 soc 트랙 W14 운영 지표와 이어진다.

---

## 5. 실습 안내 (8 미션)

1. **SOAR 엔진 확인**(active-response). 2. **액션 카탈로그**. 3. **트리거 정의**(command). 4. **플레이북
설계**. 5. **드라이런 매핑**. 6. **사람 검토**. 7. **MTTR 측정**. 8. **보고서**.

> 명령은 el34 호스트에서 `docker exec el34-siem`로. **인가된 실습 환경(el34)에서만**, active-response는
> 읽기 전용(실차단 미발동·드라이런).

---

## 6. 다음 주차 (W11) 예고 — 인시던트 대응(IR)

W10은 자동 대응(SOAR)이었다. W11은 그 위 계층 — 실제 침해 사건이 터졌을 때의 **인시던트 대응(IR)** 전체
수명주기(준비·식별·격리·근절·복구·교훈)를 다룬다.
