...
OSINT라도 **인가 없는 표적 정찰은 금지**다. 본 실습은 el34 인가 표적에만 수행한다. 능동 요청(nc·whatweb)은
...
> 정확하지만 로그에 남는다. 엄밀히 whatweb·nc는 능동(표적에 요청)이지만 단발이라 스캔보다 은밀하다. 실무
...
| HTTP 헤더 | whatweb | 서버·버전·프레임워크 |
| 노출 파일 | nc robots/.git | 숨은 경로·소스 |
...
| 도구 | 보는 것 | 정체 |
|------|---------|------|
| `whatweb` Server 헤더 | `Apache/2.4.25 (Debian)` | **백엔드** dvwa 앱 |
...
공격자 VM(`ssh att@192.168.0.202`)에서 실행한다. **인가된 표적(10.20.30.1)에만.**
