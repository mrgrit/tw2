...
> **피벗(터널)** 으로 뚫는다. 본 주차에 학생은 el34에서 내부망 발판(192.168.0.202)에 서서 실제 내부 호스트
...
외부에선 공인 IP 하나만 보이지만, 발판(192.168.0.202) 안에서는 내부 서브넷이 펼쳐진다.

| IP | 호스트 | 역할 |
|----|--------|------|
| 192.168.0.202 | attacker | **내 발판** |
...
`-D` + proxychains 면 nmap·nc 등 어떤 도구든 `proxychains nmap ...` 로 내부망을 직접 친다. chisel·ligolo-ng는
방화벽 우회 전용 터널 도구다.
...
공격자 VM(`ssh att@192.168.0.202`)에서 실행한다. **인가된 내부 표적(10.20.30.0/24)에만.**

### STEP 1 — 내부 발판
- **왜**: 측면 이동은 내부망에 선 발판에서 시작.
- **무엇을**: 발판(192.168.0.202)의 내부 위치 확인.
