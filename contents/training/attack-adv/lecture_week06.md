...
> sudo 오설정, 쓰기 가능한 root 파일/cron, 흘린 자격증명, 패치 안 된 커널. 학생은 el34에서 이 벡터들을
> LinPEAS처럼 하나씩 열거하고, 작은 오설정 하나가 어떻게 root로 이어지는지 배운다.
>
...
| 시간        | 내용                                                                | 유형      |
|-------------|---------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 권한 상승이란, 왜 열거가 9할                                 | 강의      |
| 0:25–0:55   | 이론 — SUID·sudo·GTFOBins                                           | 강의      |
| 0:55–1:05   | 휴식                                                                 | —         |
| 1:05–1:35   | 이론 — 쓰기파일/cron·자격·커널 LPE                                  | 강의/토론 |
| 1:35–2:15   | 실습 — 현재권한·SUID·sudo                                            | 실습      |
| 2:15–2:45   | 실습 — 쓰기파일/cron·자격·커널                                       | 실습      |
| 2:45–2:55   | 휴식                                                                 | —         |
| 2:55–3:30   | 실습 — 방어 + 보고서                                                 | 실습      |
...
> 사용자, 다음 주 W08 측면 이동과 연결). 둘 다 "현재 권한으로 접근 못 하는 것에 접근"하는 점은 같지만,
...
```mermaid
graph TD
    USER["현재: 일반 사용자<br/>(www-data·탈취 계정)"]
    USER --> ENUM["체계적 열거"]
    ENUM --> V1["SUID 바이너리"]
    ENUM --> V2["sudo 오설정"]
    ENUM --> V3["쓰기 파일/cron"]
...
**SUID** 비트가 붙은 파일은 실행 시 **소유자 권한**(보통 root)으로 돈다. `passwd`처럼 정상적인 것도 있지만,
`find`·`vim`·`python`에 SUID가 붙어 있으면 **GTFOBins** 기법으로 root 셸을 딴다(예: `find . -exec /bin/sh -p \;`).
**sudo -l** 은 내가 sudo로 쓸 수 있는 명령을 보여준다 — `(ALL) NOPASSWD: ALL`이면 즉시 root이고, `less`·`vi`·
`awk` 같은 GTFOBins 바이너리가 허용되면 그 안에서 셸을 탈출한다(`sudo less /etc/hosts` → `!/bin/sh`). **GTFOBins**
(gtfobins.github.io)는 이런 악용 기법을 바이너리별로 정리한 백과다.
...
— 커널 버전을 확인해 알려진 로컬 익스플로잇(DirtyCow·DirtyPipe·PwnKit/pkexec)을 매핑한다. **LinPEAS**는 이
모든 벡터를 한 번에 자동 열거한다.
...
한다. 그리고 **정기 감사**로 새로 생긴 오설정을 잡는다. 탐지 측면에선 soc-adv W06 헌팅(새 SUID 실행·sudo
이상·신규 계정)이 권한 상승 시도를 포착한다 — 공격과 방어가 다시 만난다.

---

## 5. 실습 안내 (8 미션)

1. **현재 권한**. 2. **SUID 열거**. 3. **sudo 점검**. 4. **쓰기 파일/cron**. 5. **자격 수집**.
6. **커널/GTFOBins**. 7. **방어**. 8. **보고서**.

> 명령은 el34 호스트에서 `docker exec el34-attacker`로. **인가된 실습 환경(el34)에서만**, 열거는 읽기 전용
> (실제 상승 미수행).

---

## 6. 다음 주차 (W07) 예고 — C2 인프라
