...
```bash
ssh att@192.168.0.202 "echo -en 'GET /api/Feedbacks HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | grep -oE '@[a-zA-Z0-9.-]+' | head -3"  # nc-ok: 컴플라이언스 PII 노출 점검(공개 API 응답 JSON 검사)
```
...
```bash
ssh att@192.168.0.202 "echo -en 'GET /api/Feedbacks HTTP/1.0\r\nHost: juice.el34.lab\r\nConnection: close\r\n\r\n' | nc -w3 192.168.0.161 80 | grep -oE 'UserId[^,]*' | head -2; echo reident_check"  # nc-ok: 컴플라이언스 PII 노출 점검(공개 API 응답 JSON 검사)
```
