...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 개인정보는 "빌린 것"인가, 개인정보보호법·GDPR 한눈에              | 강의      |
| 0:25–0:55   | 이론 — 개인정보의 정의·범위 + 가명/익명/개인정보의 경계                     | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — 처리 원칙 6 + 정보주체 권리 + el34 Feedbacks 매핑                    | 강의/토론 |
| 1:35–2:00   | 실습 — 점검(대상 도달) + 개인정보 식별(Feedbacks 이메일 조각) (미션 1–2)   | 실습      |
| 2:00–2:30   | 실습 — 처리 원칙 정리 + 정보주체 권리 정리 (미션 3–4)                       | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 — 재식별 위험(UserId 결합) + 가명·익명·마스킹 구분 (미션 5–6)         | 실습      |
| 3:10–3:30   | 실습 — 안전성 확보 조치 + 데이터 보호 종합 보고서 (미션 7–8)               | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W13 — 제3자·공급망 위험) 예고            | 정리      |
...
점검은 `el34-attacker` 에서 `curl` 로 `/api/Feedbacks` 를 **인증 없이** 호출해, 응답 본문에서 개인을
식별할 수 있는 필드(이메일 조각·UserId)가 추출되는지를 본다. 화면(브라우저)으로 보면 피드백 글만
멀쩡히 보이지만, **API 응답 본문 자체** 를 검사하면 정작 화면에 쓰지 않는 식별자가 함께 내려오는 것이
드러난다 — 이것이 개인정보 노출의 증거다.

> **용어 — curl / 무인증(unauthenticated) 점검.** **curl** 은 명령줄에서 HTTP 요청을 보내고 응답을
> 그대로 받아 보는 표준 도구다. `-s`(조용히)·`-k`(self-signed 인증서 무시)·`-H 'Host: ...'`(가상 호스트
> 지정) 옵션을 쓴다. **무인증 점검** 은 로그인·토큰 없이 그냥 호출하는 것으로, **누구나 접근 가능한
> 상태에서 개인정보가 새는가**를 보는 가장 기본적인 점검이다. 무인증으로 개인정보가 나온다는 것은
> 접근 통제가 없다는 뜻이라 위험도가 높다.

### 2.3 el34 에서 어떻게 보이나 — 개인정보 노출 확인

```bash
docker exec el34-attacker sh -c "curl -sk -H 'Host: juice.el34.lab' http://10.20.30.1/api/Feedbacks | grep -oE '@[a-zA-Z0-9.-]+' | head -3"
```
...
```mermaid
graph TD
    REQ["감사자 점검<br/>curl /api/Feedbacks (무인증)"]
...
echo "→ Feedbacks 무인증 노출 시: 정보주체가 모르는 사이 제3자가 열람 → 통제권 상실"
```
...
```bash
docker exec el34-attacker sh -c "curl -sk -H 'Host: juice.el34.lab' http://10.20.30.1/api/Feedbacks | grep -oE 'UserId[^,]*' | head -2; echo reident_check"
```
...
echo "→ Feedbacks: 우선 접근 통제 + 응답 필드 최소화(식별자 미포함)가 정답"
```
...
> el34-attacker` 로 실행한다. 신규 도구 설치는 없으며(curl·grep), **인가된 표적(JuiceShop)** 만
> 점검한다. 노출된 개인정보는 수집·재배포하지 않는다(§8). 합격 임계값은 0.7 이다.
...
>
> **무엇을 알 수 있는가?** `curl` 로 JuiceShop 의 응답 코드(`juice=...`)가 돌아오는지 — 개인정보
