...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:20   | 이론 — 무단 변경은 왜 침해의 신호인가, 변경관리와 FIM 이 한 쌍인 이유       | 강의      |
| 0:20–0:55   | 이론 — 변경관리 7단계 + CAB·긴급변경 + PCI 6.4 / ISMS-P 2.9 매핑           | 강의      |
| 0:55–1:05   | 휴식                                                                       | —         |
| 1:05–1:35   | 이론 — FIM(syscheck) realtime vs 주기 + rule 554/550/555 + whodata         | 강의/토론 |
| 1:35–2:00   | 실습 1, 2 — 점검(대상 도달) + FIM 감시 디렉터리 확인 (미션 1–2)            | 실습      |
| 2:00–2:30   | 실습 3, 4 — FIM 작동 증적(syscheck 경보) + 무단 변경 탐지(canary) (미션 3–4)| 실습      |
| 2:30–2:40   | 휴식                                                                       | —         |
| 2:40–3:10   | 실습 5, 6 — 변경관리 프로세스 + FIM↔변경관리 연계 (미션 5–6)               | 실습      |
| 3:10–3:30   | 실습 7, 8 — 무결성 통제 방어 + 종합 보고서 (미션 7–8)                      | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W11 — 사고대응 컴플라이언스) 예고        | 정리      |
...
```bash
docker exec el34-web sh -c "grep -E '<directories' /var/ossec/etc/ossec.conf | head -8"
```
...
```bash
docker exec el34-siem sh -c 'N=$(tail -8000 /var/ossec/logs/alerts/alerts.json 2>/dev/null | grep -c syscheck); echo "fim_alerts=$N"'
```
...
```bash
docker exec el34-web sh -c 'echo x > /etc/apache2/cp10_canary.txt; test -f /etc/apache2/cp10_canary.txt && echo "fim_canary=created_in_monitored_dir"; rm -f /etc/apache2/cp10_canary.txt; echo cleaned'
```
...
/ 실전 활용. 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에서 `docker exec
...
> 음성 결과가 무의미하다.
>
> **무엇을 알 수 있는가?** `docker exec el34-web` 으로 hostname 이 응답하는지 — 점검 대상이
> 실제 가동·접근 가능한 상태인지.
>
