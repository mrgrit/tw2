...
```bash
ssh att@192.168.0.202 "echo | openssl s_client -connect neobank.el34.lab:443 -servername neobank.el34.lab | grep -iE 'Protocol|Cipher' | head -3"
```
...
```bash
ssh att@192.168.0.202 "timeout 55 nmap --script ssl-enum-ciphers -p 443 neobank.el34.lab 2>/dev/null | grep -iE 'TLSv|least strength'"
```
...
```bash
ssh att@192.168.0.202 'if echo | openssl s_client -connect neobank.el34.lab:443 -tls1_1 2>&1 | grep -qiE "no protocols|alert|handshake fail|no peer cert"; then echo "tls11=blocked"; else echo "tls11=allowed"; fi'
```
...
> 무의미하다.
>
> **무엇을 알 수 있는가?** `openssl s_client` 로 TLS 위에 보낸 raw HTTP 요청이 fw → web 경로를 거쳐
> 응답 코드를 돌려주는지. 즉 점검 대상이 살아있고 도달 가능한지.
>
