...
| 시간        | 내용                                                                       | 유형      |
|-------------|----------------------------------------------------------------------------|-----------|
| 0:00–0:25   | 이론 — 왜 "막기"가 아니라 "준비·절차·증거·기한"인가 (실 사고 3건)            | 강의      |
| 0:25–0:55   | 이론 — NIST SP 800-61 IR 생명주기 4단계 + ISMS-P 2.11 매핑                  | 강의      |
| 0:55–1:05   | 휴식                                                                        | —         |
| 1:05–1:35   | 이론 — 증적 보존(chain of custody) + 신고 의무(72h) + 사고 심각도 분류       | 강의/토론 |
| 1:35–2:00   | 실습 — 점검(대상 도달) + 사고 탐지(SIEM level≥10) (미션 1–2)                | 실습      |
| 2:00–2:30   | 실습 — IR 생명주기 + 증적 보존(chain of custody) (미션 3–4)                 | 실습      |
| 2:30–2:40   | 휴식                                                                        | —         |
| 2:40–3:10   | 실습 — 신고 의무(72h) + 사고 분류(심각도) (미션 5–6)                        | 실습      |
| 3:10–3:30   | 실습 — IR 체계(방어) + 사고대응 보고서 (미션 7–8)                           | 실습      |
| 3:30–3:40   | 정리 + 채점 기준 안내 + 다음 주차(W12 — 데이터 보호·개인정보) 예고          | 정리      |
...
호스트(`ssh ccc@192.168.0.151`, 비밀번호 1)에 접속한 뒤 `docker exec el34-siem`(SIEM 탐지) 등으로
...
감사자는 호스트(`192.168.0.151`)에 한 번 SSH 로 들어간 뒤, `docker exec el34-siem` 으로 SIEM 컨테이너에
...
ccc@192.168.0.151`, 비밀번호 1)에서 실행하며, 신규 도구 설치는 없다.
...
> **진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.151`)에서 `docker exec el34-siem`(탐지) 또는
