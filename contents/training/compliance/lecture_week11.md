...
호스트(`ssh ccc@192.168.0.80`, 비밀번호 1)에 접속한 뒤 `ssh ccc@10.20.32.100`(SIEM 탐지) 등으로
...
graph TD
    ...
    SIEM["탐지 점검<br/>ssh ccc@10.20.32.100<br/>alerts.json (level≥10)"]
...
감사자는 호스트(`192.168.0.80`)에 한 번 SSH 로 들어간 뒤, `ssh ccc@10.20.32.100` 으로 SIEM 컨테이너에
...
```bash
ssh ccc@10.20.32.100 'N=$(tail -3000 /var/ossec/logs/alerts/alerts.json | grep -oE "\"level\":1[0-9]" | wc -l); echo "incidents=$N"'
```
...
> **진행 원칙.** 모든 명령은 el34 호스트(`ssh ccc@192.168.0.80`)에서 `ssh ccc@10.20.32.100`(탐지) 또는
...
>
> **무엇을 알 수 있는가?** `ssh ccc@10.20.32.100` 으로 hostname 이 응답하는지 — 사고대응 점검의 중심
