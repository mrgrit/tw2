...
3. el34 호스트(`ssh ccc@192.168.0.80`)에서 `ssh ccc@10.20.32.80` 로 설정 파일에 **`sha256sum`(무결성)
...
```bash
ssh ccc@10.20.32.80 "sha256sum /etc/apache2/conf-available/security.conf; date -u +%Y-%m-%dT%H:%M:%SZ"
```
...
>
> **무엇을 알 수 있는가?** `ssh ccc@10.20.32.80` 으로 hostname 이 응답하는지 — 증적 수집 대상이 실제
> 가동·점검 가능한 상태인지.
>
