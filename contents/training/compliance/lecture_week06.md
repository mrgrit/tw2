...
192.168.0.202 - - [14/Jun/2026:11:06:44 +0000] "GET /?q=<script> HTTP/1.1" 403 512
```

위 한 줄을 감사 5요소로 읽으면 — **어디서**(출처 IP `192.168.0.202`) · **언제**(시각
...
```mermaid
graph TD
    LOG["access 로그 한 줄<br/>192.168.0.202 ... GET /?q=&lt;script&gt; 403"]
...
> el34-web` 또는 `ssh ccc@10.20.32.100` 으로 실행한다. **신규 설치는 없다** — 이미 가동 중인 로깅
...
>
> **무엇을 알 수 있는가?** `ssh ccc@10.20.32.80` 으로 대상 컨테이너에 접근해 hostname 을 받아오는지 —
> 즉 점검을 시작할 수 있는 상태인지.
>
