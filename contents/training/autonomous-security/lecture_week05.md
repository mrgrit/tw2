...
- **A. 플레이북 정의 실증 (필수, 40점)** — `PLAYBOOK_DEFINED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 플레이북 실행 분석 (필수, 30점)** — `PLAYBOOK_EXECUTED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. 파라미터화 재사용 방어 설계 (필수, 30점)** — `PLAYBOOK_REUSED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(PLAYBOOK_DEFINED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(PLAYBOOK_EXECUTED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(PLAYBOOK_REUSED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **Playbook 자동화: 재사용 가능한 보안 임무 구조화**.
- **플레이북 정의**(`PLAYBOOK_DEFINED`): 트리거·단계·분기·성공기준으로 반복 임무를 구조화한다.
- **플레이북 실행**(`PLAYBOOK_EXECUTED`): 트리거 발동 후 단계를 진행하고 조건에 따라 분기한다.
- **파라미터화 재사용**(`PLAYBOOK_REUSED`): 대상·임계값을 변수로 빼 같은 플레이북을 다른 입력에 재사용한다.
