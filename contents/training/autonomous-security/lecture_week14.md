...
- **A. 인간 피드백 조종 실증 (필수, 40점)** — `POLICY_STEERED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 제약 최적화 분석 (필수, 30점)** — `CONSTRAINED_OPTIMIZED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. 배포 전 평가 방어 설계 (필수, 30점)** — `POLICY_IMPROVED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(POLICY_STEERED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(CONSTRAINED_OPTIMIZED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(POLICY_IMPROVED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **RL Steering과 정책 최적화: 정교한 정책 조종**.
- **인간 피드백 조종**(`POLICY_STEERED`): 사람 선호 평가로 보상을 보정해 정책을 원하는 방향으로 유도한다.
- **제약 최적화**(`CONSTRAINED_OPTIMIZED`): 안전 제약을 위반하지 않는 선에서 보상을 최대화한다.
- **배포 전 평가**(`POLICY_IMPROVED`): 새 정책을 배포 전에 시뮬·과거 데이터로 평가한다.
