...
- **A. 보상 함수 설계 실증 (필수, 40점)** — `REWARD_DEFINED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 정책 개선 분석 (필수, 30점)** — `POLICY_UPDATED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. 보상 해킹 탐지 방어 설계 (필수, 30점)** — `REWARD_HACK_DETECTED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(REWARD_DEFINED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(POLICY_UPDATED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(REWARD_HACK_DETECTED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **강화학습(RL)과 보상: 정책 개선과 보상 해킹**.
- **보상 함수 설계**(`REWARD_DEFINED`): 보안 행동의 결과에 보상/페널티를 정의한다.
- **정책 개선**(`POLICY_UPDATED`): 보상 경험으로 상태→행동 규칙을 갱신한다.
- **보상 해킹 탐지**(`REWARD_HACK_DETECTED`): 보상은 높은데 실제 목표는 못 이루는 패턴을 찾는다.
