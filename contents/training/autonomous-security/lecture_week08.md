...
> 하나의 에이전트**로 통합하는 능력이다: ① **설계** — 인지→판단→행동→학습 루프에 Manager의 하니스 엔지니어링·E.G,
...
- **el34 맥락에서 어떻게**: 인지(로그)→판단(LLM+E.G)→행동(플레이북/SubAgent)→학습(경험)에 가드레일을 얹으면
  `AGENT_DESIGNED`.
...
학습하는 구조를 심화하고, E.G(W03)의 경험 DB가 어떻게 4계층으로 조직되는지 익힌다.
