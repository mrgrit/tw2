...
- **A. 자율 에이전트 설계 실증 (필수, 40점)** — `AGENT_DESIGNED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **B. 임무 수행 분석 (필수, 30점)** — `MISSION_COMPLETED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.
- **C. 안전 검증 방어 설계 (필수, 30점)** — `SAFETY_VERIFIED` 단계를 직접 수행해 실제 명령·출력(또는 아티팩트 분석 결과)을 캡처하고, 무엇을 근거로 판정했는지 서술한다.

## 2.6 평가 기준

| 항목 | 미흡(0) | 보통 | 우수 |
|------|---------|------|------|
| 탐지/실증(AGENT_DESIGNED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 분석(MISSION_COMPLETED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |
| 방어(SAFETY_VERIFIED) | 미수행 | 마커 도출 | 근거·해석·재현까지 |

## 2.7 핵심 정리 (1줄씩)

- 이번 주 주제: **중간고사: 자율 보안 점검 CTF**.
- **자율 에이전트 설계**(`AGENT_DESIGNED`): 루프·하니스·SubAgent·가드레일·자율성 수준을 하나의 에이전트로 결합한다.
- **임무 수행**(`MISSION_COMPLETED`): 생명주기·플레이북으로 실제 임무를 수행하고 결과를 검증한다.
- **안전 검증**(`SAFETY_VERIFIED`): 5대 기둥(가드레일·자율성·감사·보상·결과 검증)을 점검한다.
